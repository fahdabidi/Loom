...
one short.
| Demo app | `app/apps/loom_communities_demo` | **167** (0 skipped) — re-measured 2026-09-13 on a quiet box. Moved 160 → 161 → 162 → 163 → 165 → 167 across five commits on the B25 capture path, every increment attributable: the watchdog race (`9c25054f`, +1), the silent missed tap (`7ba98707`, +1), the transition-frame retry (`43168718`, +1), the speculative-tap removal plus surface assertion (`51bb1cb3`, +2), and the surface-assertion polling fix (`b5bc8880`, +2). **A total that moves up needs its reason named as much as one that moves down** — seven unexplained increments look identical to seven fabricated tests, and this row is the only place that distinction is recorded |
