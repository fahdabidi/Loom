...
one short.
| Demo app | `app/apps/loom_communities_demo` | **165** (0 skipped) — re-measured 2026-09-13 on a quiet box. Moved 160 → 161 → 162 → 163 → 165 across four commits on the B25 capture path, each increment attributable: the watchdog race (`9c25054f`, +1), the silent missed tap (`7ba98707`, +1), the transition-frame retry (`43168718`, +1), and the speculative-tap removal plus surface assertion (`51bb1cb3`, **+2**). **A total that moves up needs its reason named as much as one that moves down** — five unexplained increments look identical to five fabricated tests, and this row is the only place that distinction is recorded |
