...
- Application code changes go through `data/call_implementation_agent.sh`. **Since 2026-09-14 its default engine is the Claude Code CLI, `sonnet` at effort `xhigh`** (`IMPLEMENTATION_ENGINE=claude`; `muse` and `codex` remain selectable). Verified live through the script: a `--fresh` multi-step ticket ran as separate Read/Bash/Write calls on `claude-sonnet-5`, and a follow-up without `--fresh` resumed the same `--session-id` and recalled earlier values with no tool calls. Output is stream-json, so the log grows during a run; the answer is printed between `=== AGENT REPLY ===` markers. **Muse is selectable but `muse-spark-1.3` is refused by the Meta API for this account** as of 2026-09-14 — the server's catalog now lists only 1.2 — and headless Muse resume is `muse exec --session-id`, never `muse resume` (that is the interactive picker).
...
| --- | --- | ---: |
| UX judges | `app/packages/tooling/loom_ux_judges` | **514** (0 skipped) — re-measured 2026-09-14 by me on a settled box; moved 505 → 513 → 514 as the capture tool gained its prebuilt-binary path (`6abb2a4f`, +8) and created-instance identity (`ebe226bd`, +1). — re-measured 2026-09-12; was 500 on 2026-09-10 after the `dead_role_binding` removal (5 obsolete warning tests replaced by 4 regression tests); was 501, and 490 before that |
| App shell | `app/packages/core/loom_communities_app_shell` | **421** (+2 skipped) — re-measured 2026-09-14 by me; +2 from `901b2bc1`'s completed-action tests. — re-measured 2026-09-13; `ec3a7f0c` added one regression test alongside the calendar action-load error capture, the first B25 capture fix to touch app-shell product code. **`flutter analyze` on this package is now clean ("No issues found")** — the long-standing `unnecessary_cast` at `part28…:1882` went with that edit, so a new analyzer finding here is real, not pre-existing noise. **A second environmental timeout lives in this package**: `cjm8_engine_native_tabs_test.dart` has timed out at 30s under load and passes in ~2s isolated (`--concurrency=1`), the same shape as the `v3_milestone_calr2g` case below. Previously 418, re-measured 2026-09-12. **Derived, and the derivation matters:** a run under load 42 gave 417 passed + 1 failed, and that single failure (`v3_milestone_calr2g_live_package_test.dart`, a 30s `TimeoutException`) passes in **1 second** re-run isolated on a quiet box. So all 418 pass; the failure was environmental and was proven so rather than assumed. Was 408 on 2026-09-10; was recorded as 403, and as 375 for four days while the real figure passed 400 |
| Workflow engine | `app/packages/core/loom_workflow_engine` | **330** (+5 skipped) **= 335 cases**, without PG credentials — re-measured 2026-09-13 (0 failures), on a quiet box. **The +10 over the 2026-09-12 figure of 320 is real growth with named causes**, found because a dispatch that never touched this package reported 330 and the number was checked rather than waved through: `3f6812b3` (tri-state guard evaluation), `03b93f1c` (a throwing guard disqualifies only its own transition) and `ab296d99` (P4 Half 2) each added tests since 2026-09-11. Was 310 on 2026-09-10; was recorded as 312. See the note below on that −2 |
| Workflow service | `app/packages/core/loom_workflow_service` | **166 (+1 skipped) = 167 cases** with BOTH credential sets — 2026-09-18. **The +2 over the previous 164 was checked rather than waved through, because a total moving UP can hide a deletion just as a total moving down can.** Declared tests are **167 at HEAD and 167 at the 2026-09-12 baseline commit**, and a per-file diff shows *no file gained or lost a test* — so nothing was added and nothing was deleted; two declared tests simply did not execute in the earlier run. Environmental, not a code change: the only commits touching this package since are `3f6812b3` and `03b93f1c`, both engine fixes with no test files. Previously **164 (+1) = 165**, re-measured 2026-09-12 on a quiet box, exit 0. **The skip count of 1 is the load-bearing part**: it is how you know the PostgreSQL-backed tests actually ran rather than silently skipping. Was 156 on 2026-09-10; was recorded as 153. 146 (+8) with only `LOOM_POSTGRES_PASSWORD`; 142 (+12) with none. **Read the RLS-timeout note below before trusting a red run here.** The one expected skip is `app_access_create_instance_integration_test.dart` ("Set `LOOM_APP_ACCESS_BASE_URL`…"), not a PostgreSQL test |
