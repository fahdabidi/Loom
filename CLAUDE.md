...
one short.
| Demo app | `app/apps/loom_communities_demo` | **175** (0 skipped) — re-measured 2026-09-13 on a quiet box. Moved 160 → 161 → 162 → 163 → 165 → 167 → 168 → 172 → 175 across **eight** commits on the B25 capture path, every increment attributable: watchdog race (`9c25054f`, +1), silent missed tap (`7ba98707`, +1), transition-frame retry (`43168718`, +1), speculative-tap removal plus surface assertion (`51bb1cb3`, +2), surface-assertion polling (`b5bc8880`, +2), entry-gate allowance (`e5c26a57`, +1), disabled-vs-absent split (`093c6c61`, +4), calendar preparation (`ec3a7f0c`, +3). **A total that moves up needs its reason named as much as one that moves down** — fifteen unexplained increments look identical to fifteen fabricated tests, and this row is the only place that distinction is recorded |
