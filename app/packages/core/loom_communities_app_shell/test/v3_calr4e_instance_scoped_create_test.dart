...
Future<_InstalledFixture> _install(String extensionId) async {
  ...
  final temp = await Directory.systemTemp.createTemp('loom-calr4e-$extensionId-');
  ...
  await extension.writeAsString(jsonEncode({
    'schemaVersion': 1,
...
void main() {
  testWidgets('instance create button binds a ballot to its tournament and hides for members', (tester) async {
    final installed = (await tester.runAsync(() => _install('calr4e-instance-create')))!;
    try {
      final eventId = (await tester.runAsync(() async {
        final engine = await workflowEngineForExtensionId(installed.community.extensionId);
        final existing = await engine.queryInstances(
          tabId: 'calendar', personaId: 'tabletop-organizer', limit: 100,
        );
        return existing.items.firstWhere(
          (item) => item.workflowType == 'tournament-event',
        ).instanceId;
      }))!;
      ...
      final ballotEventIds = await tester.runAsync(() async {
        ...
        final home = await engine.queryInstances(
          tabId: 'home', personaId: 'tabletop-organizer', limit: 100,
        );
        return home.items
      ...
      expect(
        find.byKey(
          ValueKey(
            'instance-create-action-$eventId-tournament-ballot',
          ),
        ),
        findsNothing,
      );
