...
void main() {
  test('roleHasPermission derives visibility from transition roles', () {
  ...
  test('roleHasPermission derives visibility from create-action roles', () {
    final experience = _experience({
  ...
  test('roleHasPermission follows responseTable workflow roles', () {
    final experience = _experience({
      'event': _machine(
        workflowType: 'event',
        tabId: 'calendar',
        transitions: const [
          LoomWorkflowTransition(
            ...
            from: ['open'],
            to: 'open',
            guard: WorkflowGuard(allowedRoleIds: ['outsider']),
          ),
        ],
  ...
  test(
    ...
    () {
      ...
      expect(
        roleHasPermission(runtimeGuardedExperience, 'outsider', tabId: 'mixed'),
        isTrue,
      );
      expect(
        roleHasPermission(unguardedExperience, 'outsider', tabId: 'mixed'),
        isTrue,
      );
    },
  );
  ...
  test('roleHasPermission combines acting and read visibility', () {
    final experience = _experience({
      'guarded-workflow': _machine(
        ...
        visibility: const WorkflowVisibility(
          defaultValue: WorkflowVisibilityDefault.guarded,
          readGuard: WorkflowGuard(allowedRoleIds: ['outsider']),
        ),
  ...
  test('runtime-only actor guards leave a bound tab visible', () {
    ...
    expect(
      roleHasPermission(experience, 'member', tabId: 'owned-items'),
      isTrue,
    );
  });
  ...
  test(
    ...
    () {
      ...
      expect(
        roleHasPermission(experience, 'outsider', tabId: 'admin'),
        isFalse,
      );
    },
  );
  ...
  test('an unguarded workflow leaves its custom tab visible', () {
    ...
    expect(roleHasPermission(experience, 'member', tabId: 'unguarded'), isTrue);
    expect(
      roleHasPermission(experience, 'outsider', tabId: 'unguarded'),
      isTrue,
    );
  ...
  test('a declarative tab with no bound workflow stays public', () {
    ...
    expect(
      roleHasPermission(experience, 'member', tabId: 'static-help'),
      isTrue,
    );
  ...
  test(
    ...
    () async {
      ...
      await expectLater(
        engine.queryInstances(tabId: 'private-surface', fanId: 'outsider'),
