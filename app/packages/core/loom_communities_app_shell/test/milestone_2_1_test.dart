...
void main() {
  group('Milestone 2.1 audienceSelector form field', () {
    testWidgets(
      'switches mode inputs and emits matching instanceData',
      (tester) async {
        ...
        expect(find.byKey(const ValueKey('audience-selector-field')),
            findsOneWidget);
        expect(find.byKey(const ValueKey('audience-selector-selected-many')),
            findsNothing);
        expect(find.byKey(const ValueKey('audience-selector-individual')),
            findsNothing);
        expect(submitted, {
          'audienceScope': 'all',
        ...
        expect(find.byKey(const ValueKey('audience-selector-selected-many')),
            findsOneWidget);
        await tester
            .tap(find.byKey(const ValueKey('audience-selector-persona-alice')));
        await tester.pumpAndSettle();
        await tester
            .tap(find.byKey(const ValueKey('audience-selector-persona-bob')));
        await tester.pumpAndSettle();
        expect(submitted, {
        ...
        expect(find.byKey(const ValueKey('audience-selector-individual')),
            findsOneWidget);
        await tester
            .tap(find.byKey(const ValueKey('audience-selector-persona-cora')));
        await tester.pumpAndSettle();
        expect(submitted, {
