...
Future<_PreloadedShellInstallation> _installOverPreloadedShell({
  ...
  final temp = await Directory.systemTemp.createTemp('loom-cjm9-preloaded-$extensionId-');
  ...
  final snapshotCommunity = backend
      .listCommunities()
      .singleWhere((community) => community.communityId == communityId);
...
Map<String, Object?> _experienceFromSource(Map<String, Object?> source) {
  ...
  throw StateError('Missing experience block in fixture community ${source['communityId']}');
}
...
bool _hasMarketplaceBinding(LoomWorkflowStateMachine definition) {
  return definition.renderBindings.any((binding) => binding.tabId == 'marketplace');
}
...
void main() {
  test(
    ...
    () async {
      ...
      try {
        ...
        final gardenToolLoan = experience.workflowDefinitions?['garden-tool-loan'];
  ...
  test('camera shell hydrates canonical binding into marketplace flow', () async {
    final installation = await _installOverPreloadedShell(
    ...
    try {
      ...
      final gearRequest = experience.workflowDefinitions?['gear-loan-request'];
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      try {
        ...
        expect(
          find.text('Exchange is coming to Garden Club'),
          findsNothing,
        );
      } finally {
