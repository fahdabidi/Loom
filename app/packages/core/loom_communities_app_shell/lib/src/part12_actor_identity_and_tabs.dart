...
List<LoomActorIdentity> actorIdentitiesForExtensionId(
  String extensionId, {
  LoomExperienceDefinition? experience,
}) {
  final packageActorIdentities = experience?.actorIdentities;
  if (packageActorIdentities != null && packageActorIdentities.isNotEmpty) {
    return packageActorIdentities;
  }
  return _actorIdentitiesByExtensionId[_legacyDemoCatalogExtensionId(
        extensionId,
      )] ??
      _fallbackActorIdentities;
}

LoomWorkflowRolePolicy rolePolicyForWorkflow(
  String extensionId,
  String workflowId, {
  LoomExperienceDefinition? experience,
}) {
  final packagePolicy = experience?.rolePolicies?[workflowId];
...
LoomRoleWorkflowState roleWorkflowStateFor({
  ...
  if (policy.actorRoleIds.contains(roleId)) {
    return LoomRoleWorkflowState.actor;
  }
  if (policy.receiverRoleIds.contains(roleId)) {
    return LoomRoleWorkflowState.receiver;
  }
  if (policy.readOnlyRoleIds.contains(roleId)) {
    return LoomRoleWorkflowState.readOnly;
  }
  return LoomRoleWorkflowState.disabled;
}

LoomRoleWorkflowView roleWorkflowViewFor({
  required String extensionId,
  required LoomWorkflowDefinition workflow,
  required String roleId,
  required Set<String> completedWorkflowIds,
  required Set<String> receivedWorkflowRoleKeys,
  LoomExperienceDefinition? experience,
}) {
  final policy = rolePolicyForWorkflow(
    extensionId,
  ...
  final completed = workflowCompleted && state == LoomRoleWorkflowState.actor;
  final received = receivedWorkflowRoleKeys.contains(
    workflowRoleReceiptKey(workflowId: workflow.workflowId, roleId: roleId),
  );
  final actorWaiting =
  ...
  final receiverWaiting =
      state == LoomRoleWorkflowState.receiver &&
  ...
  return LoomRoleWorkflowView(
    ...
    roleRationale: _rationaleForState(state, policy),
    waitingText: 'Waiting',
  );
}

List<LoomRoleWorkflowMatrixRow> roleWorkflowMatrixForExtensionId(
  String extensionId,
) {
  final experience = experienceForExtensionId(extensionId);
  final actorIdentities = actorIdentitiesForExtensionId(extensionId);
  return [
    for (final workflow in experience.workflows)
      for (final actorIdentity in actorIdentities)
        LoomRoleWorkflowMatrixRow(
          extensionId: extensionId,
          workflowId: workflow.workflowId,
          roleId: actorIdentity.roleId,
          state: roleWorkflowStateFor(
            extensionId: extensionId,
            workflowId: workflow.workflowId,
            roleId: actorIdentity.roleId,
          ),
          rationale: _rationaleForState(
            ...
            rolePolicyForWorkflow(extensionId, workflow.workflowId),
          ),
          prerequisiteWorkflowId: rolePolicyForWorkflow(
            extensionId,
            workflow.workflowId,
...
List<LoomWorkflowDependency> workflowDependenciesForExtensionId(
  ...
  return [
    for (final workflow in experience.workflows)
      for (final receiverRoleId in rolePolicyForWorkflow(
        extensionId,
        workflow.workflowId,
      ).receiverRoleIds)
        LoomWorkflowDependency(
          extensionId: extensionId,
          workflowId: workflow.workflowId,
          actorRoleId: rolePolicyForWorkflow(
            extensionId,
            workflow.workflowId,
          ).actorRoleIds.first,
          receiverRoleId: receiverRoleId,
          prerequisiteWorkflowId:
              rolePolicyForWorkflow(
                extensionId,
                workflow.workflowId,
              ).prerequisiteWorkflowId ??
              workflow.workflowId,
...
List<LoomDeclarativeTabSpec> _declarativeTabSpecsFor({
  ...
  final packageActorIdentity = _declarativeTabSpecsFromRoleConfiguration(
    appShellConfiguration['roleTabs'],
    roleId: roleId,
  );
  return [...packageGlobal, ...packageActorIdentity];
}

List<LoomDeclarativeTabSpec> _declarativeTabSpecsFromRoleConfiguration(
  Object? value, {
  required String roleId,
}) {
...
bool _roleCanAdministerAnyWorkflow(
  LoomExperienceDefinition experience,
  String roleId,
) {
  ...
  return experience.workflows.any((workflow) {
    ...
    if (state != LoomRoleWorkflowState.actor) {
      return false;
    }
