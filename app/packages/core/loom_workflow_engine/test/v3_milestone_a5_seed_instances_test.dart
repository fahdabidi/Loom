...
void main() {
  ...
  test('parses source metadata and retains it after a definition reload', () async {
    final machine = _definition(schema: <String, dynamic>{
    ...
    expect((await _all(reloaded)).single.instanceData, isNot(contains('remoteCount')));
  });

  test('defers only formulas dependent on an absent declared source', () async {
    final e = await _engine(schema: <String, dynamic>{
      'items': <String, dynamic>{'type': 'list'},
      'available': <String, dynamic>{'type': 'number', 'formula': 'count(items)'},
      'remoteItems': <String, dynamic>{
        'type': 'list',
        'source': 'query(remote-item)',
      },
    ...
    await e.seedInstances([
      _seed('ok', data: <String, dynamic>{'items': <Object?>[1]}),
    ]);
  ...
  test('rejects an unknown function despite an absent declared source', () async {
    final e = await _engine(schema: <String, dynamic>{
      'remoteItems': <String, dynamic>{
        'type': 'list',
        'source': 'query(remote-item)',
  ...
  test('surfaces invalid runtime types for evaluable formulas', () async {
    ...
    await e.seedInstances([_seed('wrong', data: <String, dynamic>{'items': 'wrong'})]);
    expect(_all(e), throwsA(isA<FormulaEvaluationException>()));
  });
}
