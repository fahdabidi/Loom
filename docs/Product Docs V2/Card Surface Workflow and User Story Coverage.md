...
| App Shell navigation/theming | `CommunityAppShellNavigationApi` | As a member or role-specific persona, I can use persona-specific tabs, pinned surfaces, card expansion, theme/customization knobs, shell-owned Messages, and tab-native renderer contracts while Home remains available. | 29 interactions mapped below. |
...
| Marketplace state machine | `CommunityMarketplaceApi` | As a community owner or extension author, I can declare per-listing state machines (loan, sale, trade, giveaway) with persona-gated transitions, effect flags, and linked workflows — the engine derives available actions per persona from declared data without framework built-ins. | 5 interactions mapped below. |
...
| App Shell navigation/theming | `listTabRendererContracts` | Inspect the renderer contracts available for Home, Calendar, Messages, Marketplace, Documents, Workflow Status, Giving, Care, and Admin tabs. | `CommunityAppShellNavigationApi.listTabRendererContracts` |
| App Shell navigation/theming | `resolveTabRendererContract` | Resolve the effective renderer contract for a persona tab after surface assignment, permissions, and pinning policy. | `CommunityAppShellNavigationApi.resolveTabRendererContract` |
| App Shell navigation/theming | `validateTabRendererContract` | Validate that the selected tab renderer has the required anatomy, interactions, states, and evidence plan for the assigned card surfaces. | `CommunityAppShellNavigationApi.validateTabRendererContract` |
| App Shell navigation/theming | `previewTabRendererContract` | Preview the tab-native layout before publishing configuration so Calendar, Messages, Marketplace, Documents, and status flows do not degrade to generic cards. | `CommunityAppShellNavigationApi.previewTabRendererContract` |
| App Shell navigation/theming | `recordTabRendererEvidence` | Record screenshot/evidence links proving that the tab rendered its contract-specific anatomy and state model. | `CommunityAppShellNavigationApi.recordTabRendererEvidence` |
