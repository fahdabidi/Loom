...
| --- | --- | --- | --- | --- | --- |
| platform-message-thread | member | Member sends or reads a concrete message thread with a named participant and decides whether to reply, mute, or archive it. | send message, reply, mark read | mute, unmute, archive, unarchive | Fresh screenshots must show sender/recipient, message history, read/unread state, and the mute/archive state the other participant receives. |
| platform-connection | member | Member sends a connection invite to another member; the invitee decides to accept or decline, the inviter may cancel first, and either party may later block or unblock the connection. | send invite, accept invite | decline invite, cancel invite, block, unblock | Fresh screenshots must show inviter/invitee identity, the invited/connected/declined/blocked state, and the resulting connection status the other party receives. |
