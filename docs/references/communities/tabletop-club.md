...
- **The full Calendar experience — design closed 2026-07-26, audited against Google/Apple/Samsung Calendar
  ...
  - **CAL.Calendar2.1 — generic transition-input dialog.** Today every transition needing `inputs`
    ...
    Round 1 correctly stopped rather than guess at a real cardinality gap: `byDayOfWeek` needs multi-select
    for weekly recurrence but exactly-one-select for the monthly "last/Nth weekday" mode. Resolved with two
    new `transitions[].inputs[]` fields (`maxSelections`, `writesTo`) — see
    `spec-version.json` → `transitionInputWritesToAndMaxSelections`.
  ...
  - **CAL.Calendar2.8 — party size / plus-ones.** Blocked on a real bug found during this audit:
    ...
    JSON locked 2026-07-29: `event-rsvp-response` gained `partySize` (required GAP-1 input on
    `respond-going`, collected via CAL.Calendar2.1's generic dialog); `respond-going`'s capacity guard
    switched from `op: "count"` to `op: "sum", field: "partySize"`; all 14 seeded response rows backfilled
    with `partySize: 1` so the switch never silently undercounts pre-existing attendees. Deliberately
    inert until the `field`-parameter engine fix lands in the same implementation pass — never shipped
    ahead of it, same discipline as every other not-yet-implemented guard usage this session.
...
- **Notifications Experience — new phase, design closed 2026-07-31, JSON locked this revision (PROPOSED —
  implementation not started).** Rendering half of CAL.Notify, rescoped into its own phase after CAL.Notify.4
  hit a real wall (see below). Explicit direction shaping this phase: (1) the notification mechanism must be
  **archetype-agnostic** — any workflow type feeds notifications in, not just `event-rsvp` (already true by
  construction: `createInstance`/`onSuccessEffects` targeting `notification` is ordinary, unrestricted
  grammar); (2) **all four presentation styles** (`bell`/`dedicatedTab`/`fixedCard`/`fab`,
  `render-bindings.md`) ship real and independently tested, not a real/documented-only split — Tabletop Club
  itself only activates `bell`; (3) a real **Calendar reminder-scheduling API** (`reminderOffsetHours`
  already in the fixture; `reminderAt`/`combineDateAndTime` withheld until CAL.Notify2.9's engine work,
  formulas.md); (4) real **native OS notification delivery**, local-only (`flutter_local_notifications`,
  `NotificationDeliveryService`) since this engine has no backend/push infrastructure at all — architected
  behind an interface so a future real backend can swap the implementation without touching call sites. See
  `spec-version.json` → `notificationPresentationGrammar`/`notificationCoreDataLayer`/
  `notificationDeliveryService`/`combineDateAndTimeFormula`/`calendarReminderApi` for the full narrative, and
  the tracker's CAL.Notify2.1-.10 rows for the milestone breakdown. **Dead-code note (tracked, not actioned
  this phase)**: `_NotificationInboxTabSurface`/`LoomNotificationItem`/`_NotificationInboxEngineStore`
  (`part02_tab_shell.dart`) are confirmed dead/disconnected code for every real community — a real cleanup
  candidate, deliberately out of scope here to keep this phase's own diff reviewable.

- **Targeted per-persona notifications — CAL.Notify, design closed 2026-07-26; engine + validator + fixture
  IMPLEMENTED 2026-07-31 (CAL.Notify.1-.3).** App-Shell rendering (CAL.Notify.4/.5/.6) was attempted, hit a
  real architectural wall, and is now RESCOPED into its own dedicated **Notifications Experience** phase —
  see the tracker. Real gap found during the Calendar audit, but cross-cutting
  (Marketplace/Giving would want it too), so scoped as its own initiative rather than bundled into
  CAL.Calendar2. Loom has no way today to notify one specific member (event edited/cancelled, promoted off
  a waitlist) — only a manual, organizer-published broadcast announcement
  ...
  - **New guard kind 10, `actorEqualsField`** (`guards.md`, IMPLEMENTED 2026-07-31): `{ key }`, passing only if `$actor
  ...
  - **`transitionRelated` gains `onSuccessEffects`** (`effects.md` §11, IMPLEMENTED 2026-07-31) — a real, necessary
...
- **Recurring events (2026-07-25, IMPLEMENTED — CAL.Recurrence, 7 milestones, commits `2974310`/`e653074`
  engine, `91ef180` validator, `f5d30b9`/`b4ee5c2` frozen fixture, `7128d9a` App-Shell creation UI,
  `4eb10d8` App-Shell delete-series).** No recurrence concept existed anywhere in the model — every event
  was a standalone instance. Confirmed (twice, independently) there was no JSON-only shortcut:
  `resolveInstanceScopedPrefill`/`createInstance.fields` are interpolation-only, and the formula evaluator
  has no forward-direction date function. **Design: eager instance generation** — a new `make-recurring`
  transition on `event-rsvp` (self-removing after firing once, via `formula: "seriesId == null"`) carries
  a new effect op, `generateRecurringInstances` (`effects.md` §12), which mints one `seriesId`, stamps it
  on the anchor instance (occurrence 0), and creates `count - 1` real, independent sibling instances via
  bespoke Dart date arithmetic (`recurrence_evaluator.dart`) — not the formula evaluator, since the full
  requested pattern set (daily/weekly-by-weekday/monthly-by-date-with-clamping/monthly-by-last-or-Nth-
  weekday) is real month-boundary arithmetic that would over-generalize the formula language if forced
  ...
  sharing `seriesId`, loop the existing `cancel-event` transition) — not a new engine bulk-op. **The
  implementation cycle independently caught and fixed three real, distinct bugs before the feature could
  be trusted**: (1) an omitted optional `recurrenceRule` field resolved to a garbage literal token string
  instead of `null`; (2) the weekly `byDayOfWeek` walk assumed occurrence 0 always equals the anchor,
  false whenever the anchor's own weekday isn't itself requested; (3) the validator never asked to catch
  a monthly `byDayOfWeek` without `bySetPos`, even though the engine itself already rejected it — a real
  validator/engine mismatch the original ticket missed. Independently verified end-to-end:
  `loom_workflow_engine` 159/159, `loom_ux_judges` 119/119, `loom_communities_app_shell` 128/128, both
...
| --- | --- | --- | --- | --- | --- |
| Notifications Experience phase — JSON lock, 2026-07-31 | **yes, a full new phase** — real, flexible, archetype-agnostic notification rendering (four presentation styles, not one Calendar-tab bolt-on), a real Calendar reminder-scheduling API, and real native OS notification delivery, none of which existed before this pass | no new gap found by this JSON-only pass beyond what CAL.Notify.4's own attempt already surfaced (Calendar's date-grid `eventDate` requirement) — confirmed via the real validator that a NEW formula function reference (`combineDateAndTime`) is actively rejected at author time (unlike guard/effect shapes, which tolerate additive-but-inert keys), so `reminderAt` itself is deliberately withheld from the fixture until its engine dependency ships, while `reminderOffsetHours` (harmless alone) is added now | `render-bindings.md` gained `notificationPresentation` (PROPOSED); `formulas.md` gained `combineDateAndTime` (PROPOSED) and corrected a stale "PROPOSED" marker on `subtractHours`/`mapGet` (both actually implemented); `spec-version.json` gained five new `proposedNotImplemented` entries; this doc's §10/§11 updated. Frozen JSON: `notificationPresentation: {"style": "bell"}` added community-wide; `event-rsvp`/`tournament-event` gained `reminderOffsetHours` (live, harmless); `event-rsvp-response` gained `reminderSentAt` (idempotency stamp, inert until CAL.Notify2.9). Both real validators re-run clean (0/0); full `loom_communities_app_shell` suite re-run clean (143/143) confirming all new fields are safely additive | App Shell/engine: nothing in CAL.Notify2.1-.10 implemented yet — tracked as its own set of tracker rows, milestone-ordered | open — JSON/docs updated and approved this revision (JSON/docs only, per standing instruction); implementation tickets not yet dispatched |
| CAL.Notify.4 attempted + rescoped into Notifications Experience phase, 2026-07-31 | **yes, a real one found by actually building it, not by reading docs** — the assumption that `notification` could render via an ordinary `renderBindings[].tabId: "calendar"` entry (same mechanism `event-rsvp` uses) was wrong: Calendar's own date-grid surface requires every instance it renders to carry a valid `eventDate`, unconditionally, baked into `_CalendarEntry`'s own projection logic, not gated by `cardSurfaceFamily` the way the design assumed | the JSON amendment (recipientPersonaIds/audienceScope + two renderBindings) was written, validated clean by both real validators (0/0), and only THEN caught by a real widget test crash — `'fullness guard auto-promotes a waitlisted member through Calendar UI'` timed out the moment a genuine waitlist-promotion notification instance existed. Confirmed via `git stash` bisection: passes clean on the prior commit, fails only with the new renderBindings present. Reverted cleanly (`git checkout`, nothing had been committed) | `spec-version.json`'s `notificationWorkflowType` entry updated to record the attempt and the real root cause found; this doc's §10/§11 updated; tracker's CAL.Notify.4/.5/.6 rows marked rescoped, new **Notifications Experience** phase row added with the user's own direction (research bell icon / dedicated tab / fixed card surface / FAB patterns; design the real generic rendering mechanism; explicitly revisit Calendar's own reminder/notification integration during that phase, not as a bolt-on now) | none landed — the attempted `notification` renderBindings were reverted before any commit; `notification`'s `renderBindings` stays `[]` exactly as CAL.Notify.3 left it | CAL.Notify.4/.5/.6 rescoped, not implemented; new Notifications Experience phase open, awaiting its own design pass |
| CAL.Notify.1-.3 — engine + validator + fixture, 2026-07-31 | no — design was already locked 2026-07-26; this pass implements + activates it, with explicit sign-off on the exact fixture diff per standing instruction | no new gap — `onSuccessEffects` reuses the existing recursive effect-parsing/checking machinery (`then`/`else`'s own pattern) rather than introducing a second, divergent path; `actorEqualsField` mirrors `RelatedAggregateGuard`'s existing shape. Confirmed via real end-to-end tests: `onSuccessEffects` interpolate against the TARGET row's own data (not the source), run exactly once on genuine promotion, and do NOT run on the target's own silent guard-failure path | `guards.md`/`effects.md`: `actorEqualsField`/`onSuccessEffects` (and, opportunistically, two other stale PROPOSED markers this pass found — `cancellationDeadline`/`locationOverlap`, both actually implemented since CAL.Calendar2.2/.9) all moved to IMPLEMENTED with commit references; `spec-version.json` updated to match; this doc's §10/§11 updated. Frozen JSON: `respond-maybe`/`respond-declined`'s existing `transitionRelated` effects gained real `onSuccessEffects` creating a `notification` instance targeting the promoted row's own `personaId`; new `notification` workflow type added (`unread`→`read`, `mark-read` gated by `actorEqualsField`). Both real validators re-run clean (0/0) against the updated fixture | App Shell: `notification`'s Calendar-tab rendering (workflow-type-dispatched `rolesForInstance` lambda) and the client-orchestrated event-changed/cancelled fan-out are not yet implemented — tracked as CAL.Notify.4/.5/.6 | CAL.Notify.1/.2/.3 closed (independently verified: `loom_workflow_engine` 181/181, `loom_ux_judges` 124/124, both real validators clean); CAL.Notify.4-6 remain open |
| CAL.Calendar2.8 JSON lock — party size/plus-ones, 2026-07-29 | no — the design (fix `relatedAggregate`'s `field` param, add `partySize`, switch the capacity guard to `sum`) was already locked in the original Calendar audit; this pass only writes the JSON, after explicit review of the exact diff per standing instruction | no new gap — confirms the existing `aggregate()` method is the right abstraction to extend (adding `field` is pure wiring to an argument that already exists on that method's real signature), not a new ad-hoc computation path, so a future real backend only needs to implement the same `aggregate()` contract | `guards.md`'s `relatedAggregate` correction note extended to describe the now-fixture-present (but still inert) `sum`/`field` usage; `spec-version.json`'s `relatedAggregateFieldParam` updated with the fixture-lock date; this doc's §10/§11 updated; frozen JSON: `event-rsvp-response` gained `partySize` (required GAP-1 input on `respond-going`, `set` effect); `respond-going`'s capacity guard switched `op: "count"` → `op: "sum", field: "partySize"`; all 14 seeded response rows backfilled `partySize: 1` so the switch never silently undercounts pre-existing attendees the moment the paired engine fix lands | App Shell/engine: `RelatedAggregateGuard.field` + `aggregate()` wiring not yet implemented — the JSON is deliberately inert until that lands in the same pass, same discipline as every other not-yet-implemented guard usage this session | open — JSON locked and approved this revision; engine ticket about to be dispatched |
| CAL.Calendar2.1 fix round — input cardinality, 2026-07-26 | **yes, small** — the implementation agent's round 1 build (generic transition-input dialog + `make-recurring` migration, otherwise fully correct) correctly stopped rather than guess at a real grammar gap: `byDayOfWeek` needs multi-select for weekly recurrence but exactly-one-select for the monthly "last/Nth weekday" mode (`recurrence_evaluator.dart` requires `bySetPos` pairs with exactly one `byDayOfWeek` entry), and the locked `transitions[].inputs` grammar had no way to express mode-conditional cardinality on a single input | no engine gap beyond the grammar itself — resolved by adding two small `transitions[].inputs[]` fields (`maxSelections`, `writesTo`) rather than a formula-conditioned cardinality mechanism; also closed an unrelated documentation gap found while writing this up — `visibleWhen`/`options`/`modeGroup`/`modeValue` had been implemented in round 1 but never entered into `workflow-grammar.md` or `spec-version.json` at all | `workflow-grammar.md` gained a full `transitions[].inputs` section (previously undocumented entirely) including `maxSelections`/`writesTo`; `spec-version.json` gained `transitionInputDialogFields` (backfilling round 1's undocumented grammar) and `transitionInputWritesToAndMaxSelections`; this doc's §10/§11 updated; frozen JSON: `make-recurring`'s `byDayOfWeek` input split into `byDayOfWeekWeekly` (unbounded) + `byDayOfWeekMonthly` (`maxSelections: 1`), both `writesTo: "byDayOfWeek"` | App Shell: `maxSelections`/`writesTo` support in `GenericTransitionInputDialog`'s multi-select chip renderer and `_confirm()` — not yet dispatched, tracked as a CAL.Calendar2.1 fix-round | open — spec updated and approved this revision (JSON/docs only, per standing instruction); fix-round ticket not yet dispatched |
