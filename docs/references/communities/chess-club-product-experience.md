...
| Export package | Export club match/ranking data. | Owner | export scope, platform-generated checksum, status, download history, rollback/retry/cancel paths | Generate export / download / rollback |
...
| Export package | export scope, platform-generated checksum, status | ready/generated/rolled back/cancelled | generate export, download, rollback, retry, cancel | effect-written or hardcoded checksum |
...
| `chess-export-package` | owner | Export package review | export scope, platform-generated checksum, status, download history, rollback/retry/cancel paths | `exportWizard`; export-bundle service generates and verifies the SHA-256 checksum | B25 |
...
| `chess-export-package` | owner creates an export, changes its scope, generates it, downloads it, rolls it back, retries it, or cancels it | players may read the guarded status summary while the owner retains export controls | players see export status while the owner sees platform-generated integrity metadata | rollback and download are unavailable until generated; retry is available after rollback | non-owners cannot create, configure, generate, download, roll back, retry, or cancel exports; membership admission is not a workflow |
...
| `chess-club-night` | `statusTimeline` | Workflow engine | RSVP, withdraw RSVP, add reminder, send reminder, cancel night | The real generic timeline renders the scheduled → reminded → cancelled progression and its community-defined actions. |
| `chess-discussion-thread` | `discussionThread` | Workflow engine | reply, mute, archive | The real generic structured-list renderer shows message history and the create/reply/archive affordances. |
| `chess-export-package` | `exportWizard` | Export bundle service / workflow engine | change scope, generate, download, rollback, retry, cancel | The bespoke wizard renders the lifecycle; the export-bundle service writes the real checksum, never a JSON effect. |
| `chess-pairing-queue` | `approvalQueueItem` | Workflow engine | assign pairing, ordered waiting list, close queue | The generic queue surface renders the shared queue container, its order, assignment history, and organizer actions. |
| `chess-rankings-table` | `table` | Workflow engine | publish, revise, retire; sort/search rank rows | The bespoke grid groups ranking-row instances and exposes their generic workflow actions. |
| `chess-rules-documents` | `documentLibrary` | Document/link library surface | open embedded, open external, download, archive | The bespoke library renders the two explicit URL open modes and records document lifecycle actions; this is intentionally a link library, not file upload. |
...
| GitHub-fetch convergence pass (2026-09-02) | yes | no | Corrected stale export-checksum language and six stale B25 archetype-registry rows to match the current 13-family registry and export-bundle service. | No package behavior change; the shipped specVersion-4 package already implements the corrected rows. | fixed |
