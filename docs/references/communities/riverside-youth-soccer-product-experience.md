...
| Documents/waivers | document title, version, acknowledgement, access state, authorized-guardian count, embedded/external open option | unread/read/acknowledged/access-requested | open embedded, open external, acknowledge, request access | hidden waiver link or scalar identity used where the shared-reader model requires a list |
...
| soccer-waiver-document | guardian | Waiver document detail | waiver title/version, embedded/external open, acknowledgement, access state, and authorized guardian share set | Documents/external documents/audit; `documentLibrary` owner-and-shared visibility reads a `fanId[]` field | B14/B25 |
...
| soccer-waiver-document | guardian opens and acknowledges a waiver/policy | coach sees acknowledgement status without protected details | waiver version and acknowledgement history are readable by coaches and guardians listed in the authorized guardian share set | acknowledge disabled after latest version accepted | a guardian not listed in `authorizedGuardianFanIds` cannot read the protected player document; only the scalar `guardianFanId` may perform guardian acknowledgement/access transitions |
...
| soccer-waiver-document | guardian | Guardian opens a concrete waiver or policy, reviews version/source and the authorized-guardian access set, chooses embedded or external open, acknowledges it, and sees season registration impact. | open document, acknowledge waiver, download document | request access, open external, mark unread, ask coach | Fresh screenshots must show title/version/source, embedded/external open options, acknowledgement state, authorized-guardian access proof, access guard, and registration-status linkage. |
