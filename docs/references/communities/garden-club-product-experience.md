...
| garden-export-custom-schemas | coordinator | Export package review | selected garden_event and plant_exchange schemas, redaction preview, checksum, destination, change-scope path, download/export status | Export, documents, audit | B13/B25 |
...
Both channels are offered and both are on by default. Volunteer shifts, tool loans coming due, and
plant exchange pickups are all tied to a day and a place. A reminder that arrives after the shift
...
| garden-tool-loan | member | Member browses available garden tools, requests to borrow one or joins the waitlist when unavailable, and the owner confirms pickup or declines; the borrower later returns the item or flags a condition issue. | browse items, list item, request loan, join waitlist, confirm pickup | leave queue, cancel loan, decline request, pause/resume/delist listing, mark damaged, report lost, return item | Fresh screenshots must show browse/list views, owner/current-holder privacy, queue position, borrower state, custody/condition state, and the pickup/return result both owner and borrower receive. |
| garden-tool-giveaway | member | Member browses items offered for giveaway and claims one to take ownership; the owner lists, pauses, resumes, or delists their own item. | browse items, list item, claim giveaway, schedule pickup | pause listing, resume listing, delist listing | Fresh screenshots must show browse/list views, owner privacy, claimant state, pickup coordination, and the ownership-transfer result the owner receives. |
| garden-export-custom-schemas | coordinator | Admin selects export/import/transfer scope, reviews redaction/checksum/status, and can cancel, retry, or roll back. | export, download export, start transfer, import data | change scope, cancel transfer, rollback, retry, redaction preview | Fresh screenshots must show status, receipt/history/confirmation, and any receiver or continuation state for this persona. |
