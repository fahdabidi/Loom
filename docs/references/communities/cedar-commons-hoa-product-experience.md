...
| hoa-architectural-request | board | Workflow status case | change details, current step, reviewer, requested-changes path, document/payment checkpoint, submitted state | Workflow status/cases/tasks/documents | B14/B25 |
| hoa-committee-decision | board | Workflow status review queue | requester, decision actions, status history, request-changes path, comments, owner receiver state | Workflow status/cases/tasks/audit | B14/B25 |
| hoa-owner-notification | board | Owner notification | sender, audience, body, sent/received state | Notifications/events | B14/B25 |
| hoa-export-evidence | board | Export status | scope/checksum/rollback | Export/provider transfer | B14/B25 |
...
| hoa-document-access-request | member | Member requests access to a specific document they cannot yet open, and the Board decides to grant or deny that one request. | request access, withdraw request | grant access, deny access | Fresh screenshots must show the requester and the requested document, the pending/granted/denied/withdrawn state, and the resulting reader-access state both the member and the Board see. |
...
| hoa-architectural-request | board | Reviewer or requester evaluates a concrete request with requester, details, status, and approve/reject/change paths. | submit request, approve request, send request, review request | reject, request changes, revise, withdraw, edit request | Fresh screenshots must show status, receipt/history/confirmation, and any receiver or continuation state for this persona. |
| hoa-committee-decision | board | User decides a concrete community task with enough context, a semantic primary action, a meaningful alternative, and a durable result. | submit, save, send | edit, change, undo, reject, withdraw | Fresh screenshots must show status, receipt/history/confirmation, and any receiver or continuation state for this persona. |
| hoa-owner-notification | board | User decides a concrete community task with enough context, a semantic primary action, a meaningful alternative, and a durable result. | submit, save, send | edit, change, undo, reject, withdraw | Fresh screenshots must show status, receipt/history/confirmation, and any receiver or continuation state for this persona. |
| hoa-export-evidence | board | Admin selects export/import/transfer scope, reviews redaction/checksum/status, and can cancel, retry, or roll back. | export, download export, start transfer, import data | change scope, cancel transfer, rollback, retry, redaction preview | Fresh screenshots must show status, receipt/history/confirmation, and any receiver or continuation state for this persona. |
...
| `hoa-document-access-request` | [workflow-status](../../CardSurfaces/workflow-status.md) | `CommunityWorkflowStatusApi` | request access, withdraw request, grant access, deny access, requester/document identification, decision history | Demo renderer must show the requester, the specific document requested, the pending/granted/denied/withdrawn state, and the Board's decision history -- same pattern as `hoa-committee-decision`. |
