...
| `documentLibrary` | Categorized document browsing, version history, acknowledgement/access-request tracking | 🔲 PENDING | None new — consumes the `type: "url"` field (`field-types.md`). **Correction 2026-08-12:** `openMode: "external"` is already real; every real fixture's primary document field declares `openMode: "choice"` instead, which is not — implementing `choice` is a real, narrower prerequisite than "the renderer does nothing," see `field-types.md`'s correction note. | Cedar Commons HOA, Chess Club, Book Club, Youth Soccer, Masjid Nur document workflows |
