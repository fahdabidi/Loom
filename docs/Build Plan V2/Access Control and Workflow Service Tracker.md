...
|---|---|---|---|---|
| 🟡 In progress | `implementation-dispatched` | **ROOT CAUSE FOUND: intermittent `authorization_service_unavailable` (503) — the app was sending the package extension id (`ext_*`) in the workflow-service URL where the server expects the canonical community id (`community_*`).** Every read (`_resolveRolesForRequest` in `workflow_service.dart`) does an exact-match group lookup keyed by `community_*`; `ext_*` matches nothing and 503s. Curl reproductions this session used the canonical id and always succeeded — same host, same token, only the path segment differed. Live A/B proof, full mechanism trace (`main.dart:241-242` → `part25:237-240` → `part37:405-409` → `remote_workflow_engine_api.dart:352-357`), and recommended fix (translate extension id → canonical community id client-side, at engine-factory invocation): [root-cause-authz-503-extensionid-vs-communityid-2026-09-06.md](Evidence/B25/root-cause-authz-503-extensionid-vs-communityid-2026-09-06.md). Found via the Muse-based Root Cause Agent's first production dispatch. Fix now dispatched via the implementation agent (GPT-5.6-Terra, xhigh). | Muse Root Cause Agent, this session | 2026-09-06 |
