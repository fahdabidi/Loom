...
|---|---|---|---|---|
| 🔴 Open | `needs-decision` | **Three "closed via live write" B25 rows (Cedar dues payment, Camera Club critique, Chess Club dispute — all 2026-09-04) recorded server timestamps that fall inside a confirmed ~25-hour Postgres outage, and none of the three claimed instances exist in the live database today.** Full evidence, timeline, and what is/isn't proven: [root-cause-postgres-outage-live-write-timestamps-2026-09-06.md](Evidence/B25/root-cause-postgres-outage-live-write-timestamps-2026-09-06.md). Not closed, not reopened, not acted on — three open questions recorded there await the user's read: whether to reopen the three affected rows, whether every other "closed via live write" row should be retroactively spot-checked against the database, and whether the outage itself has a known cause. Found by accident while scoping an unrelated ticket (Cedar document-content upload), not by design — worth noting because it means this class of gap is easy to miss unless someone specifically checks a claimed write against the database rather than the rendered UI. | autonomous backend loop, this session | 2026-09-06 |
