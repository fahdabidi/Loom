...
| ⛔ Blocked (user) | `new-ticket` | **The duplicate Cedar group (`loom_communities_cedar_commons_hoa`, underscored) — separate decision, delete or keep.** Confirmed live 2026-09-06, not just recalled from the row below: it sits beside the real hyphenated `loom_communities_cedar-commons-hoa`, has **2 real active members** (`fan_alice`, `fan_bob`, both joined 2026-08-13 — looks like leftover test data from that day, not a real community), and its `cedar_commons_hoa_admin` role holds only 4 mismatched permissions (`community.manage_members`, `document_library.upload`, `event_rsvp.cancel`, `event_rsvp.create`) rather than the standard 5 `community.*`. Deleting a whole group with live members is more consequential than a role rename, so this needs its own explicit go-ahead rather than being folded into the (already-completed) 10-role migration above. | re-verified live 2026-09-06 | 2026-09-02 |
