...
| ⬜ Open | `new-ticket` | **D7a's create-or-get is implemented but not yet reachable — `_loadActions` must offer actions for a row that does not exist.** Approved 2026-08-14. `_applyTransition` now materializes a missing response row on demand instead of returning `Future.value()` silently (`part28`), which is the half that makes RSVP self-healing for late joiners, partial fan-outs and data repair. It cannot fire yet: `_loadActions` derives response actions by calling `availableTransitionsAsync` **against the viewer's row**, and short-circuits to an empty list when there is none — so the member is offered no RSVP controls and there is no tap to intercept. **Correcting the earlier description in this tracker: it is a *missing* control, not a dead one.** Same outcome (a late joiner cannot RSVP), different mechanism — action derivation, not the transition path. Closing it needs availability computed against a synthetic row in the response workflow's declared initial state, which the surface cannot reach today because `WorkflowEngineApi` exposes no way to read a definition (`queryInstances`, `availableTransitions(Async)`, `applyTransition`, `createInstance(s)`, `updateInstanceFields`, `aggregate`, `dueNotifications` — no definition accessor). A skipped regression test is already written and goes green when this lands. | approved D7a | 2026-08-14 |
