...
| ⬜ Open | `new-ticket` | **9 app-shell test failures** — baseline now `+229 -9`, down from `+227 -11`. **Correction 2026-08-14: these were never "unrelated to this effort", as this row originally claimed.** Decomposed by running them: **7** are the unimplemented eager response-row fan-out (`v3_milestone_a11_event_rsvp_archetype_test.dart` ×6, plus `v3_milestone_calr2g_live_package_test.dart`) — the row above; **2** remain unexamined (`cjm9` hydration, `phasee` purchase proposal, the latter already tracked separately). The two `cjm8` failures are **closed** — see below. | independent verification during cleanup; decomposed 2026-08-14 | 2026-08-14 |
