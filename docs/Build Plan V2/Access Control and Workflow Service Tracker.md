...
| 🔵 Queued | Shell UI bug | **`Local package details` disclosure header renders white-on-white (1.00:1 contrast) — shell-wide, every community.** Found and root-caused during the Platform Social B25 walkthrough, independently verified by eye (header text fully invisible on-device, only the chevron/icon glyph visible). Measured on the raw screencap: header glyph pixels ~(249,249,255) against a (248,249,255) surface; the same tile's own expanded children measure 16.29:1 and 8.86:1 against the identical background, so this is not a scroll/animation artifact. Root cause: `app/packages/core/loom_communities_app_shell/lib/src/part01_local_extension_screen.dart:1826-1832` hardcodes `collapsedTextColor`, `collapsedIconColor`, `textColor`, `iconColor` to `Colors.white` on a light scaffold, in both collapsed and expanded states. Fix: derive these from the theme (or drop the hardcoded overrides entirely) so the label is legible against whatever background the shell actually renders. Affects the control users must tap to expand the section — not decorative. | B25 Platform Social walkthrough | 2026-09-04 |
| 🟡 In progress — fix REVERTED, root-cause dispatched | Shell UI bug | **The role radio in "Account role and permissions" appeared to silently no-op in 3 B25 walkthroughs, but a first fix attempt broke an existing test that relies on it WORKING — the real mechanism is not yet understood.** First attempt (2026-09-04): disabled any role option other than the signed-in account's own role, based on `_activeRoleId` at `part01_local_extension_screen.dart:221-224` always preferring `session?.account.roleId`. **This broke `v3_milestone_phasec_marketplace_archetype_test.dart`'s `'paid-up members see Request loan...'` test**, which taps `actor-identity-option-tabletop-organizer` mid-test and asserts the switch DOES take effect (the borrow action disappears). Independently confirmed via `git stash`/re-run: that test passes cleanly on unmodified `main` and fails specifically with the fix applied — a genuine regression, not a false positive. **The fix was reverted** (kept only the unrelated white-on-white header fix from the same ticket); nothing broken was committed. Dispatched a root-cause investigation (`data/call_root_cause_agent.sh`) to reconcile the two observations — does role-switching actually work via a different code path than `_activeRoleId` (making the B25 observation a narrower display-only bug), or does the test's fixture not accurately model production sign-in? Report will land at `docs/Build Plan V2/Evidence/B25/root-cause-role-picker-2026-09-04.md`. Do not re-attempt the "disable other roles" fix until that report lands. | B25 Book Club / Camera Club / Platform Social walkthroughs; regression caught by independent verification | 2026-09-04 |
