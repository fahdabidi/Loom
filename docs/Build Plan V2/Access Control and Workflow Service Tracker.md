...
|---|---|---|---|---|
| ✅ Closed (**Parts A + A4 + B + C ALL DONE — permission catalog publishing is hardened**) | `implementation-dispatched` | **PART C DEPLOYED AND PROVEN 2026-09-08.** `loom/app-access:0.3.10` built, imported, manifest bumped and **committed before** `kubectl apply` (`047e0be`, pushed), rollout clean. **Measured in the live DB against the before-snapshot committed in the ticket: `permission` 127 → **137** (predicted 137), `role_permission` **425 → 425 unchanged**, admins at exactly 5 `community.*` **11 → 11**, `permission_catalog_version` `2026-08-26.2` → the generated hash `4-d15e5ea2…`.** All 9 `calendar.*` present with real display names (0 blank), `event_rsvp.send_reminder` present, `community.*` still 5 — the hand-granted governance rows were NOT dropped, which was the specific fear. The 21 retired ids are retained, not deleted. **Second boot is a true no-op proven by state rather than the log line: `catalog_published_at` is byte-identical across a restart (`06:49:06.134546+00`), counts still 137/425.** **Calendar blocker structurally closed:** `ensurePermissionsExist` 400s only on a derived id absent from the catalog, and the deriver can only emit ids its bundled vocabulary names — so vocabulary ⊆ catalog is decisive: 116 vocabulary ids, **116 present, 0 missing**, and the 137−116 = **21** remainder is exactly the retired set. (Run with a control, which earned its keep: the first attempt returned "0 derived ids" and the control showed that was a broken grep, not an empty set — permissions are derived server-side and never appear in the client plan.) **Deliberately NOT done: the end-to-end re-install.** `installCommunityPackage` replaces declared roles' permissions and deletes undeclared group-scoped roles — it would destroy all 11 community admin roles unrecoverably. That is the open **role deletion** decision, not this ticket's to resolve; running it to "prove" the catalog fix would trade a verified blocker for an unrecoverable one. Post-deploy audit clean: deployed == manifest == HEAD, tree clean, spec parity 8/8 + artifact 1/1, all pods ready with no new restarts, and app-access serving (401, auth enforced). PART B LANDED AND VERIFIED 2026-09-07 (`80bc61f`, pushed).** `PermissionCatalogReconciler` upserts the live catalog from the classpath copy of `permissions-vocabulary.json` at startup, keyed on the generated `catalogVersion`. **Safety re-read by me in the committed source, not taken from the report:** there is no delete call anywhere in the class — retired ids are *reported* only; and when `changedPermissions` is empty it returns *before* any write, so a second boot is a true no-op rather than a version re-stamp. Re-confirmed against the live schema this session that `role_permission` has FKs only to `app_role` (`role_permission_role_fk`) and **none to `permission`**, so a reconcile cannot strip or block a grant even in principle. **My own verification:** the suite was re-run against the real port-forwarded `loom_app_access_test` DB (not the dispatch's throwaway container, which it deleted, making its own result unreproducible) — **72 tests, 0 failures / 0 errors / 0 skipped, BUILD SUCCESS**; the skip count is zero, so nothing was silently not-run. Parity gate proven **both** ways by deliberately drifting the generated twin: drifted → `check_spec_parity.sh` exits 1 *and* `build.sh` refuses with "refusing to invoke Maven with drifted artifacts" before Maven starts; restored → exits 0, tree clean. Spec twins 8/8 identical, generated artifact 1/1. Test diffs grepped for weakened assertions and `@Disabled`: none — the changes are a pure fixture refactor off the deleted `spec/loom-communities-permission-catalog.json` onto the generated vocabulary. **Part C before-snapshot re-confirmed live, matching the one committed in the ticket: `permission` 127, `role_permission` 425, admins at exactly 5 `community.*` 11 (of 12 admin roles — the 12th is the known `masjid-nur-admin` outlier), `permission_catalog_version = 2026-08-26.2`, deployed `0.3.9` == manifest == HEAD.** Next: Part C. PART A LANDED 2026-09-07 (`ea18fb23`), independently verified — not taken from the dispatch's report.** `ArchetypeAction` typed records now carry `id`/`displayName`/`description` for every bespoke, generic and governance action (an action without a display name does not compile); every existing accessor kept its old type as a derived view, so no caller changed. The generator emits, additively, a per-family `catalog[]` of `{permissionId, displayName, description, category}` plus a deterministic top-level `catalogVersion` (`4-<sha256 of content>`). **My own verification, re-derived:** `dart run bin/generate_permissions_vocabulary.dart --check` exits 0 ("up to date" — the committed artifact genuinely matches the resolver); artifact sha256 `ccc72210…`/39,451 bytes matches the dispatch's claim exactly; 15 `catalog` arrays present and the Java parser's dependencies intact (15 `permissionPrefix`, 1 `adminRole`); suites re-run by me — ux_judges **501** (498 + its 3 new tests), engine **308 +5 skipped**, app_shell **381 +2 skipped**, all at baseline; commit scope is exactly the 4 intended files with zero weakened assertions. **A4 also done (mine, outside the dispatch's sandbox):** vocabulary copied into `loom-backend`, `cmp`-verified byte-identical, `check_spec_parity.sh` reports full parity (8 spec twins + generated artifact, 0 drift), committed backend-side as `72fde78`. **Next: Part B** (backend startup upsert-only reconcile, parity gate in `build.sh`, delete the hand-maintained catalog file, integration tests) then Part C (deploy + prove against the committed before-snapshot). ORIGINAL: **Permission catalog publishing is unhardened — the real, remaining cause of the calendar blocker.** Ticket: [GAP-permission-catalog-publishing](Tickets/GAP-permission-catalog-publishing.md). Live catalog is stale (published 2026-08-26 from a hand-maintained file that was never re-published): missing the 9 `calendar.*` + `event_rsvp.send_reminder`; 21 retired ids linger (zero grants). `ensurePermissionsExist` correctly 400s any install that derives a missing id — so calendar communities are uninstallable. **Design ruling (user, 2026-09-07):** single source of truth — display names typed in `ArchetypeResolver`; catalog generated from the vocabulary; backend reconciles the live catalog from its bundled vocabulary on startup (deploy == publish); parity of the vocabulary twin gated in the backend `build.sh`; the hand-maintained `spec/loom-communities-permission-catalog.json` deleted. **Plan:** Part A (Loom: typed action records, additive `catalog[]` + `catalogVersion` in the generator, tests, twin synced) → Part B (backend: startup upsert-only reconcile that reports never deletes, parity gate, delete the file, integration tests) → Part C (me: deploy, commit manifest bump, prove reconcile in DB, re-install one calendar community end-to-end). Safe by construction: no FK from grants to catalog, all retired ids have zero grants, nothing in A/B deletes a row or a grant. Closes the calendar rows in this table when C lands. | verified live against code + DB, this session | 2026-09-07 |
| ✅ Closed (`9a7b2760`) | `fixed` | **FIXED: intermittent `authorization_service_unavailable` (503) — the app was sending the package extension id (`ext_*`) in the workflow-service URL where the server expects the canonical community id (`community_*`).** Root cause found via the Muse-based Root Cause Agent's first production dispatch (live A/B proof, full mechanism trace): [root-cause-authz-503-extensionid-vs-communityid-2026-09-06.md](Evidence/B25/root-cause-authz-503-extensionid-vs-communityid-2026-09-06.md). Fixed via the implementation agent (GPT-5.6-Terra, xhigh): the remote engine factory (`part37_remote_auth_session.dart`) now requires and uses the canonical `communityId`, threaded from the community package through `LoomExperienceDefinition` and every `experienceForExtensionId` call site; throws loudly if missing/blank instead of silently falling back to the extension id. The local, in-memory engine is untouched — still namespaced by `extensionId`, exactly as before. Verified independently, not just from the dispatch report: `loom_workflow_engine` 308 passed/5 skipped, `loom_communities_app_shell` 381 passed/2 skipped, both green; new/updated regression test asserts the outgoing request path carries the canonical id and does not contain the extension id. | Muse Root Cause Agent + implementation agent, this session | 2026-09-06 |
| ✅ Closed (decided) | `decided` | **MAJOR finding, DECIDED 2026-09-07 (user): intentional, not a defect.** Community seed data (`workflowInstances` declared in every package) never reaches the remote backend — confirmed structurally in `part25_engine_native_community_store.dart:290`. **User's decision: leave it unseeded by default** — real communities against the remote backend are meant to start empty and be populated by real member activity, not package-declared seed content. No server-side seed/install endpoint will be built. **Consequence for verification, not the product**: any future B25 walkthrough or UX judge run against the real backend must perform the real role-based actions to create needed data live (sign in as the relevant role, actually create/submit/publish), rather than assume seeded content is present — an empty community on open is the expected state, not a bug. This also fully closes the earlier "why did 16 walkthroughs show populated seed data" question: they were running the local demo engine, the only engine seeding ever reaches. Full writeup and decision: [root-cause-seed-instances-never-reach-remote.md](Evidence/B25/root-cause-seed-instances-never-reach-remote.md). | autonomous backend loop, this session | 2026-09-07 |
| 🟡 In progress (**5 of 10 re-verified: Cedar ✅, Camera Club ✅, Chess Club ✅, Masjid Nur ✅, Youth Soccer ✅; Book Club ⛔ blocked on one credential**) | `needs-reverification` | **CEDAR RE-VERIFIED AND CLOSED 2026-09-08 under the dual-proof standard** — manifest `5e28b13f`, [cedar-dues-live-write-2026-09-08.md](Evidence/B25/cedar-dues-live-write-2026-09-08.md), superseding the reopened 09-04 claim. **Verified by my own query, not the agent's report:** baseline immediately before dispatch was 4 instances with **zero** `hoa-dues-payment`; afterwards two exist, both `created_by_fan_id = fan-hoa-board-1` — `..._m5k32qsqumkb` (`processing`) and `..._cd93iaf2sfem` (**`paid`**, the exact terminal state the original claim asserted via `record-offline-payment`). `updated_at` moved well after `created_at` on both, so real transitions executed server-side rather than a bare insert. Role escalation checked against the shipped package rather than accepted: `"kind": "create", "label": "New dues charge", "byRoleIds": ["hoa-board"]` — a Homeowner genuinely has no dues create path, so board creation + member transition is correct, not a shortcut. **I corrected the agent's own report in one place:** it declined to attribute the 07:57:59Z row to itself for predating its first DB query, but my pre-dispatch baseline proves it new — it created the row before it first queried. Its caution erred safe. **What unblocked this campaign:** the first attempt stalled **44 minutes** at the Keycloak form because its brief omitted credentials; the root cause agent found them documented as a convention (`loom-<slug>`/`fan-<slug>`/shared test password, tracker line 1783), and I verified a password-grant token returns HTTP 200 before re-dispatching. No credential was created or reset. **Product/harness findings banked:** (1) the app shows **`BACKEND UNREACHABLE` for `LoomAuthNotLoggedInException`** while the cluster is fully healthy — a real diagnostic defect that sends an investigator to `kubectl` instead of the login screen; (2) `adb shell input text` **silently truncated** `fan-hoa-member-1` to `fan-hoa-memb`, which would have persisted a valid-looking instance addressed to a nonexistent fan; (3) `uiautomator dump` returned stale trees and omitted a create FAB that screenshots showed. (2) and (3) are folded into CLAUDE.md's evidence rules because both manufacture false evidence silently. **CAMERA CLUB RE-VERIFIED 2026-09-08** — manifest `acf47bfb`, [camera-club-critique-live-write-2026-09-08.md](Evidence/B25/camera-club-critique-live-write-2026-09-08.md). Verified by my own query against a control of **zero** `critique-submission` rows: `..._hqn35lcajo7l`, `created_by_fan_id = fan-camera-member-1` (the nominated identity, no escalation needed), `draft` → **`submitted`**, `updated_at` 09:19:27 vs `created_at` 09:18:18 — a real transition, not a re-save. Honest limit stated by the agent and accepted: `reviewed` needs an organizer and `withdrawn` would destroy the row, so `submitted` is the advanced-but-not-terminal state the standard permits. **Its most valuable output was a defect, not the row:** a **stale Keycloak SSO cookie silently re-authenticated the previous fan** — "Sign in securely with Loom…" showed no login form and returned a green "You're signed in" while re-issuing a token for `fan-hoa-board-1`. Only the app's anti-impersonation guard caught it. That is the likeliest way to bank evidence attributed to the wrong person, it fails green in every direction, and it is now in CLAUDE.md and in every generated brief. Also filed: a required `type: image` field rendering as a plain text box, so a photo-critique community cannot carry a photograph. **CHESS CLUB RE-VERIFIED 2026-09-08** — manifest `2dc82e9c`. The reopened claim was a *dispute*, and `dispute-result` is a destructive transition on `chess-match-result`, not a workflow of its own, so the agent had to create and advance a match first (the package's seeded `final` instance is local-only; remote starts empty). **Verified by my own query against a control of zero `chess-match-result` rows:** `..._8mmrvshrps22`, `created_by_fan_id = fan-chess-member-1`, `draft` → `submitted` → **`disputed`**, created 09:48:16.472 → updated 09:51:16.372, matching the on-screen history to the microsecond. No escalation needed — `chess-member` covers create, submit and dispute. **The SSO hardening from Camera Club paid off immediately:** this run hit Keycloak's logout endpoint first, which answered "Do you want to log out?" — proving the prior Camera Club session was still live and would otherwise have re-authenticated it as the wrong fan. **Three findings filed** (see TODO.md): the entry gate can deadlock a user out of sign-in entirely because the login affordance renders only in an error branch; app-access returns 500 for unknown routes (`NoResourceFoundException`, an ordinary 404); and `participantFanIds` mixes role ids into a `fanId[]` field — the identifier-space hazard `keypatterns.md` records first, which nothing detected. **BOOK CLUB BLOCKED 2026-09-08, honest negative with controls** — manifest `1d4738ee`. `workflow_instances` stayed **8 → 8** with **zero** `book-nomination`, and a control query for `chess-match-result` returned its row, so the empty result is a real negative rather than a broken query. Cause, confirmed by me against Keycloak's admin API rather than taken from the report: **`loom-book-member-1`'s password credential was replaced on 2026-09-04** (createdDate `1788489988859`) while every other seeded account dates to the 2026-08-31 run; it returns **401** with the documented password while `loom-book-organizer-1` returns **200** through the identical client and flow. **I pre-flighted all 13 remaining seeded accounts: every one returns 200, so this is the only broken credential in the realm** and no other community is at risk. **Escalation genuinely cannot rescue it** — `group_membership_role` confirms `book-member → fan-book-member-1` is the sole holder, and that role gates the create FAB, the `draft` editGuard and the `submit-nomination` guard; on-device, an organizer sees only "New ballot" with "Nominate a book" absent. The agent refused to call it "completed as organizer", which was correct. **Filed as `blocked-on-user` in TODO.md** with three unblocking options; I did not reset the credential, since changing one was correctly blocked as privileged earlier and the 2026-09-04 change may have been deliberate — its rationale is recorded nowhere, which is itself worth closing. **MASJID NUR RE-VERIFIED 2026-09-08** — manifest `d25e0a9e`. **Verified by my own query against a control of zero `mosque-donation-payment` rows:** `..._xwr38n4tfb7z`, `created_by_fan_id = fan-masjid-member-1` (the nominated identity, no escalation), state `pending`, 10:40:22Z; total 8 → 9. **Its finding is larger than the row and is a live production gap, not a test-data gap:** Masjid Nur's package role `owner` **was never provisioned at all**. I confirmed this directly in `app_role` with a control — the group holds only `community-member` and the generated `masjid-nur-admin`, while every peer community has its package role (`chess-owner`, `soccer-owner`, `portability-owner`, `ad-off-owner`). Four transitions are gated by `"byRoleIds": ["owner"]` alone, so **no existing account can reach any of them**: a donation can never be recorded as `paid`, no receipt can issue, `mosque-donor-visibility`'s only writer can never fire, and the `paid`-only home-tab binding can never render — `cancelled` is the only reachable terminal state. The agent correctly refused to substitute `masjid-nur-admin`, which is generated governance rather than the package's domain role. **I then checked whether this is systemic: diffing every package's declared roles against live `app_role`, `masjid-nur :: owner` is the ONLY missing declared role across all 10 communities** (control passed), so it is an isolated miss — but nothing detects this class of drift, and a declared-vs-provisioned parity check would catch it cheaply. **YOUTH SOCCER RE-VERIFIED 2026-09-08** — manifest `85a7859f`. **Verified by my own query against a control of zero `soccer-registration-payment` rows:** `..._831tpmuqct6u`, `created_by_fan_id = fan-soccer-guardian-1`, state `processing`, 11:38:44Z. **One guardian action legitimately created three instances** — `soccer-guardian-join-approval` (submitted), then `soccer-registration-payment` and `soccer-waiver-document` **6 ms apart**, i.e. a `createInstance` effect firing cross-workflow, observed live rather than inferred; total 9 → 12. The payment has no create form of its own, so the empty Payments tab is correct rather than a defect. **Finding, confirmed by me in the shipped package: `paid` is unreachable by anyone.** `record-offline-payment` is `"to": "paid"` guarded `["soccer-coach"]`, while the `giving` tab — the payment's only primary binding — is `"visibleRoleIds": ["soccer-guardian"]`, and the `home` summary covers only `paid`/`refunded`. The tab immediately above declares both roles, so omitting the coach reads as an authoring slip. The agent escalated to the seeded `loom-soccer-coach-1` (named; not the generated admin) and confirmed on-device that no coach-visible surface renders the instance. Filed in TODO.md. **This is the second consecutive community whose completion flow no account can finish** — after Masjid Nur's unprovisioned `owner` — different causes, same shape, and **neither is visible in a validator run**. The remaining briefs now ask each agent to check terminal-state reachability explicitly. **Also of note:** the anti-impersonation guard again refused a stale session (`accountNotFound`) before sign-in, and the `adb input text` truncation trap fired twice more and was caught by reading values back — both hardenings working. **Remaining 4 communities** (Garden Club, Ad-Free, Export Migration, Platform Social), one dispatch at a time, briefs generated by `Tools/code/make_b25_brief.sh` so every trap above travels with each dispatch. **Note: the device is left signed in as Board**, so the next dispatch must re-run sign-in. ORIGINAL: **DECIDED 2026-09-07 (user): reopen and re-verify all 16 live-write dispatches (10 distinct communities), now.** None of `cedar-proof`, `cameraclub`, `chessclub`, `bookclub`, `masjidnur`, `youthsoccer`, `gardenclub`, `adfree`, `exportmigration`, `platformsocial`'s claimed live writes have any corresponding row in the live database today (the entire `workflow_instances` table holds only 3 pre-existing Cedar rows from 2026-08-26); 3 of the 10 (Cedar dues, Camera Club critique, Chess Club dispute) additionally have claimed timestamps falling inside a confirmed ~25-hour Postgres outage. **New proof standard**: re-verification must drive the real workflow live as the actual role AND independently query `workflow_instances` for that exact instance in the same session — a rendered-UI screenshot alone is no longer sufficient. **UNBLOCKED 2026-09-08 (user confirmed; verified by me with a live `claude -p` probe returning LANE_OPEN).** The earlier blocker was real -- the first Cedar dues attempt failed immediately with "You've hit your monthly spend limit" — `call_live_verification_agent.sh` and `call_ux_judge_agent.sh` both invoke the Claude Code CLI directly, gated by the same account-level limit as this session. No partial work was done, nothing needs cleanup. Codex/Muse-based dispatches are unaffected. **On hold until the user raises the limit or it resets.** Execution plan and full evidence: [root-cause-postgres-outage-live-write-timestamps-2026-09-06.md](Evidence/B25/root-cause-postgres-outage-live-write-timestamps-2026-09-06.md)'s "Decided"/"BLOCKED" sections. Until each community is individually re-closed under the new standard, treat its live-write B25 row as not proven regardless of its current tracker status. | autonomous backend loop, this session | 2026-09-07 |
| ✅ Closed (`68e4fee8`) | TODO.md line 199 | **Masjid Nur's calendar tab was a list, not a calendar — fixed, separate from the owner-role regen below.** `mosque-volunteer-signup` bound a `formEntry` "summary" onto the calendar tab alongside `mosque-event-rsvp`'s real `event-rsvp` binding — `docs/references/archetypes/calendar.md` §2 documents exactly this rule ("a tab must bind exactly one tab-native archetype... Masjid Nur lost its month grid this way for a long time, silently") and names Masjid Nur by name as the historical example. The just-completed `owner`-role regeneration (row below) didn't touch this — that dispatch was scoped narrowly to the version/role migration. Fixed via a second, narrow surgical Skill dispatch (removed the one renderBindings entry, nothing else — confirmed via diff and independently re-run validator: 0 errors, same 9 pre-existing warnings). Broke one demo-app test that had encoded the bug as expected behavior (`b46_mosque_engine_migration_test.dart`'s own inline comment read "Calendar mixes event-rsvp and volunteer form bindings") — fixed via the implementation agent to assert the real calendar-native root widget instead. Also corrected the product doc's own §6 row, which claimed a now-impossible "Calendar summary" surface for volunteer-signup. Verified independently: ux_judges 492/0, app_shell 376/0 (+2 skipped), demo-app 160/0. | 2026-09-05 |
| ✅ Closed (`14d6e1a6`) | Thread 2 / owner-role | **masjid-nur regenerated through the Skill — supersedes every prior masjid-nur row below (233/236/239/240/249) and closes the `owner`-role gap named at line 283 (11-row `owner` vocabulary decision) and line 256/257 (masjid-nur-admin's 28-grant anomaly).** Root cause (this session): masjid-nur was authored 2026-08-11, before the roleId/fanId split, the `3.0.0` reserved-`owner`-role convention, and every minor spec change since; its only touches after that were narrow surgical edits that correctly never re-derived structure (by design — see `milestone2_surgical_comment_cleanup_pattern`), so the version gap accumulated silently. Built a general fix first, not a one-off: every community package now carries a `skillVersion` stamp, `docs/references/reference/skill-versioning.md` holds the full version log + migration instructions, both Skill-dispatch channels now check-and-migrate before any edit (even a single-field one), and a new `stale_skill_version` validator finding makes the check machine-enforced rather than instruction-only. Backfilled all 11 packages' stamps from git history (5 needed a same-day correction, 3.5.0→3.6.0, since 3.6.0 required no migration). Then regenerated masjid-nur itself: applied the owed `3.0.0` `owner`-role migration (`masjid-admin`→`owner` in all 56 role-shaped references; every fan-identity-shaped `masjid-admin` string, e.g. `createdByFanId`, correctly left untouched) plus `3.4.0`–`3.6.0`. Verified independently, not the dispatch's self-report: real validator run (pass, 0 errors, 9 justified warnings), 69/69 transition ids and 10/10 seeds byte-diffed against the shipped package with zero drift. The role rename broke 4 demo-app tests + a real lib dependency (`part15_evidence_catalog.dart`) hardcoding the old role id as a literal — fixed via a second, precisely-scoped implementation-agent dispatch (the actor-identity widget key itself needed no change; it was already parameterized by the real `roleId`). Regenerated the derived `b25_semantic_interaction_models.json` asset and re-ran the provenance updater. **All five verification suites reconfirmed clean**, matching documented baselines: ux_judges 492/0, app_shell 376/0 (+2 skipped), demo-app 160/0, workflow_engine 312/0 (+5 skipped, no PG creds), workflow_service 142/0 (+12 skipped, no PG creds). **What this does NOT yet prove:** the package now correctly *declares* `owner`, matching the working pattern already used by `ad-off-owner`/`soccer-owner`/`chess-owner` (a separate real app_role holding community-derived permissions, distinct from the system `<handle>-admin` role which stays baseline-only) — but the currently-*deployed* `masjid-nur-admin` App Access role still holds its old 28-grant over-grant until a real package install/reinstall runs against this new JSON. That reinstall, and confirming the permission union lands correctly for the live role, is the next real step, not assumed done by this commit. | 2026-09-05 |
| ✅ Closed (backend `24bd082`, `b065227`, `6d4c843`) | Superuser sweep | **COMPLETE — all four services off the `loom` Postgres SUPERUSER (user-approved keycloak 2026-09-04). Protects the RLS boundary.** Rationale (found while scoping): every service pod carrying the `loom` superuser (`BYPASSRLS`) password could open a fresh connection to `loom_workflow_service` and read every community's data, bypassing the RLS just deployed. **app-access + fan-passport DONE** (`24bd082`, `b065227`): each now runs its runtime datasource as a least-privilege role (`loom_app_access_app` / `loom_fan_passport_app`, `NOSUPERUSER NOBYPASSRLS`), with Flyway migrations still as admin `loom` via `SPRING_FLYWAY_USER/PASSWORD` — a **manifest-only** change (Spring binds the env; no rebuild). Verified live: runtimes connect as the restricted roles; as those roles `loom_workflow_service.workflow_instances` is **"permission denied"** (the superuser saw all rows); pods start clean + serve (401). Provisioning SQL committed for reconstructability. **KEYCLOAK DONE (`6d4c843`, user-approved):** now connects as `loom_keycloak_app` (`NOSUPERUSER NOBYPASSRLS`) which OWNS the keycloak schema objects so its own Liquibase DDL still works (ownership transferred via targeted `ALTER OWNER` — blanket `REASSIGN OWNED` fails on system objects). Verified live: keycloak starts clean, cert + token endpoints work (200 / 401), and as `loom_keycloak_app` `workflow_instances` is "permission denied". All four services (`loom_workflow_app`, `loom_app_access_app`, `loom_fan_passport_app`, `loom_keycloak_app`) are now least-privilege — no service pod carries the `loom` superuser/BYPASSRLS password. | 2026-09-03 → 2026-09-04 |
| ✅ Closed (Loom `5a90042e`, backend `d84e09a`) | RLS activation | **Two-role split DONE — the RLS backstop now ENFORCES live (GAP-rls-restricted-role complete).** After a design review confirmed the API layer is the real boundary (auth signed per-user, role server-authoritative, experience role-driven, guards backend-enforced) and RLS is defense-in-depth, the user approved the two-role split — mainly to get the service off the Postgres superuser (a least-privilege win). **Step 1 DONE:** `loom_workflow_app` provisioned (`NOSUPERUSER NOBYPASSRLS`, DML-only grants), credential in secret `postgres-workflow-app-credentials`, provisioning SQL committed (loom-backend `364be85`) so it's reconstructable. **Found:** the DEPLOYED workflow-service is PRE-RLS (`workflow_instances relrowsecurity=f`) — the migration never ran live because the running image (`loom-workflow-service:1.0.2`) predates `0bcaaf32`; so activation requires a redeploy. **Step 2 DONE (`5a90042e`):** code split — migrations run once as admin `loom`, then the runtime pool connects as `loom_workflow_app`; the pool is code-hardened to refuse DDL. My own oracle: the isolation test now RUNS+PASSES under the restricted role (0/2 without context), suite 153 passed / **1 skipped (down from 2)** / 0 failed, FOR UPDATE 3/3, analyze clean. **Step 3-4 DONE (`d84e09a`):** built + imported `loom-workflow-service:1.0.3`, added `LOOM_POSTGRES_APP_*` to the manifest, rolled out (cluster held, all pods 1/1). **LIVE PROOF:** on the deployed DB `FORCE ROW LEVEL SECURITY` is now `t\|t` (was `f\|f` — the 1.0.2 image predated the migration); as `loom_workflow_app` a no-context query returns **0 rows**, with the correct community context **3 rows**, admin **3** (control). Post-deploy drift audit clean (deployed==manifest==1.0.3), spec parity passes. Follow-on: sweep app-access/fan-passport/keycloak off the `loom` superuser too. | 2026-09-03 |
| ✅ Closed | Thread 4 (#3) | **idempotency unified into one race-safe primitive — `836100b8`. BATCH v4 COMPLETE (all 4 architectural threads).** `runIdempotent<T>` (atomic `ON CONFLICT DO NOTHING RETURNING` + reload, one txn) now backs doc-create/doc-revision/export-bundle; queue is the untouched reference. **Fixed a real TOCTOU→HTTP-500 bug** (concurrent identical requests raced on a 2-transaction SELECT-then-INSERT; the loser'`'`s 23505 was unhandled). New live concurrency test + suite **152 passed / 2 skipped / 0 failed**, analyze clean — verified with my own oracle. Contract preserved (the doc-revision 201-on-replay inconsistency flagged, not changed). **Batch v4 recap:** T1 RLS backstop (`0bcaaf32`, inert pending restricted role — decision parked with user), T2 fan→communities lookup (`0091e0d`+`5de14eeb`), T3 server-authoritative default (confirmed, `922d9723`), T4 idempotency (`836100b8`). | 2026-09-03 |
| ✅ Confirmed (no code change) | Thread 3 (#1) | **server-authoritative default CONFIRMED by composition — it was already the production default.** At the app entrypoint (`loom_communities_demo/lib/main.dart`) `LOOM_ENV` defaults to `dev`, so `configureLoomRemoteServicesFromEnvironment()` returns non-null and the global engine factory flips to REMOTE; local is opt-in only via `--dart-define=LOOM_ENV=local`. Writes are server-first (`part49 _write` calls remoteEngine first, NEVER writes the replica). Verified with MY OWN oracle: ran the three default-assertion suites (`remote_service_configuration_environment_test` unconfigured⇒remote, `remote_auth_session_test`, `offline_replica_coordinator_test` writes-server-first) — **21 passed, 0 failed**; plus the workflow-service PG integration tests (149 passed) prove the server persists writes. **Two findings, no change made:** (a) two-layer default — library default is LOCAL (`part25:151`, so unit tests are deterministic) while entrypoint is REMOTE; leave as-is (flipping the library default breaks every local-assuming unit test). (b) there is NO `prod` environment — only `dev`→`192.168.56.10:30083` (the VM, cleartext HTTP); the VM's k3s IS the current backend so dev/VM is the intended target, add a `prod` entry only when a separate cluster exists. **Deferred:** the single end-to-end on-device live smoke-test (`integration_test/on_device_remote_backend_proof_test.dart`) needs the full B25 live-verification rig (build-on-VM + emulator-on-Windows + Keycloak test creds + host-only networking) — run it there. | 2026-09-03 |
| ✅ Closed (deploy `2500208`, app `b140b429`) | Thread 2 adoption | **fan→communities endpoint DEPLOYED + the app now USES it (user-approved 2026-09-04).** Built + deployed `loom/app-access:0.3.9` (drift audit clean, cluster held) so `GET /v1/fans/{fanId}/communities` is live (verified 401). Then the app was wired OFF its compile-time `communityId→groupId` map (`part40`) onto the endpoint (new `part51_fan_community_membership_client`): on sign-in it fetches the fan's community directory once per session (server-authoritative `groupId`/`roleIds`/state), with the compile-time map kept as a **logged fallback** so a server hiccup can't lock a user out. The server-authoritative role + the community entry gate (`part01`, untouched) are preserved. My own oracle: app-shell 372/2/0 (incl. fallback-on-error + server-role tests), remote-auth 10/0/0, demo 160/160; no assertion weakened; analyze clean. End-to-end live sign-in confirmation folds into the B25 pass. | 2026-09-04 |
| ✅ Closed | Thread 2 (#4) | **fan→communities single-call lookup SHIPPED — backend `0091e0d`, Loom spec twin `5de14eeb`.** New `GET /v1/fans/{fanId}/communities` (`listFanCommunities`) in app-access: the denormalized join that folds each community group's identity into the membership row, replacing the old N+1 (`listFanGroups` + `getGroup`-per-group) — the app didn't even do that, it used a compile-time `communityId→groupId` map. `FanCommunityMembership` = `allOf GroupMembership + {communityId, displayName}`; `communityId` from `externalRef.resourceId`; only `externalResourceType=="community"` groups included; empty→200. Handler mirrors `listFanGroups`' query + auth exactly, batch-loads groups (no N+1). Verified with MY OWN oracle: `check_spec_parity.sh` re-run (twins byte-identical, exit 0); read the handler filter, the auth mirror, and the new integration test (hasSize(2)/(3), correct communityIds, empty→200); app-access **69 passed/0 failed** (was 22→23 tests), fan-passport 31, engine 312, demo 160; app-access still **serves** post-build (401 = up + auth-enforcing). App adoption (replace the compile-time map) + deploy are deliberate follow-ons. | 2026-09-03 |
| ✅ Closed (`1ceb8d6d`) | `new-ticket` | **The 3 pre-existing test failures were STALE GENERATED ARTIFACTS, not real drift — fixed by regeneration.** (a) UX judges (2): the tool-maintained provenance manifest `docs/references/_meta/community-provenance.json` was never refreshed after the AdFree/Cedar/DataPortability/Garden regen adoption (`56dc7998`) — regenerated via `dart run tool/update_community_provenance.dart`. (b) app-shell (1): the B25 interaction-model asset was stale because product docs were edited after its last regen (drift in the Chess Club rows at offset 23,493) — regenerated via `dart run bin/generate_b25_interaction_model_asset.dart`. Both are sanctioned local tool runs (NOT hand-edits, NOT Skill, NOT dispatch; no hard-locked doc touched). Verified: `community_package_provenance_test` 2/2 + `b25_interaction_model_asset_conformance_test` 1/1 now pass. | 2026-09-03 |
| ⬜ Superseded — see the two ✅ Closed rows above (RLS activation, Superuser sweep) | Thread 1 | **STALE as of 2026-09-05 — this row's own blocker was resolved by later work above it and never marked closed.** Re-verified live, not recalled: `pg_stat_activity` right now shows all four services connected as their restricted roles, not the `loom` superuser — `loom_workflow_app` (1 conn), `loom_app_access_app` (2), `loom_fan_passport_app` (2), `loom_keycloak_app` (1) — and `loom_workflow_app` itself has `rolsuper=f, rolbypassrls=f`. The RLS backstop this row calls "inert" is therefore live and enforcing, exactly as the "RLS activation" and "Superuser sweep" rows above already state. ORIGINAL: **RLS community-isolation backstop BUILT & COMMITTED (`0bcaaf32`) — inert pending a restricted DB role.** `FORCE ROW LEVEL SECURITY` + a `community_isolation` policy on all 8 workflow-service tenant tables, per-request `SET LOCAL` context, cross-community publisher exempted. My own verification caught a FOR UPDATE lock regression the session-context refactor introduced (pool test failed 3/3 in isolation) → fixed via a targeted dispatch, assertions unchanged; workflow-service suite **149 passed / 2 skipped / 0 failed** with PG creds. **The backstop is INERT because all backend services connect as the `loom` Postgres SUPERUSER (`BYPASSRLS`) — confirmed live.** Enforcement (a `NOSUPERUSER`/`NOBYPASSRLS` runtime role + credential rotation) is tracked in `Tickets/GAP-rls-restricted-role.md`; the decision is parked with the user (it rotates a live service's DB credentials). The isolation test skips with that documented reason under a bypass role. Two dispatches (RLS build + regression fix), verified with my own oracle, not the agent report. | 2026-09-03 |
| ✅ Closed | Thread 2 | **neighborhood-book-club regen ADOPTED (`7ebfc3fc`) — the earlier "conformance failures" were MY adoption filename bug.** docs/references/communities/ mirrors use different names than app assets for 3 communities (docs: MasjidNur/NeighborhoodBookClub/RiversideYouthSoccer; assets: Mosque/BookClub/YouthSoccer). I copied regens to the app-asset name in the docs dir → duplicate .jsonc → walkthrough-conformance file-set check failed. With the correct docs name, book-club passes demo **160/160**, mute kept. **Lesson: verify the actual failure before diagnosing** — I twice mis-root-caused this as a response-modeling defect (pattern 24, now RETRACTED `2d93bc5e`) when book-club passes fine with response rows + unbound states (shipped has them too). | 2026-09-03 |
| 🅿️ Phase A.1 migration | Thread 2 | **masjid-nur adoption = the Phase A.1 arrays→rows migration, deferred.** Its regen correctly ADDS `mosque-event-rsvp-response` (migrating masjid from hand-written arrays to response rows, exactly Phase A.1's plan). But the harness/catalog were built against masjid's shipped ARRAY model, so the new workflow reads as "larger than expected" in `product_community_walkthrough_conformance_test`. Adopting masjid therefore requires completing the **Phase A.1 migration** (regen + updating the harness/catalog to expect the response-row workflow), not a plain regen-adopt. Tracked under the open Phase A.1 row; shipped masjid (arrays) kept and working until then. NOT a bug — the intended migration, correctly surfaced. | 2026-09-03 |
| ✅ THREAD 1 substantially complete (2026-09-03) | staleness sweep | **Tracker de-staled; it was less stale than first claimed.** Verified across all four trackers by grepping code / querying the DB / checking git: closed ~14 rows describing work shipped or decided this session (external_resource, platformSource, deliver_reminder, passport identity, admin migration, provisioning-destructiveness, dispatch-pipeline, fan-identity-ownership); **RE-OPENED 2** the sweep caught being wrongly pre-closed — the `calendar.*` **catalog** rows (vocab has calendar.*, deployed catalog has 0, DB-verified → real gap: publish the catalog with calendar.*). Everything else checked (architectural items, walkthrough/auth blockers, Community-JSON polish, the Phase G/E/A milestone rows) is genuinely open or accurate partial-progress — NOT stale. The open list now reflects reality. Genuine remaining work is catalogued in the plan-v3 §, the architectural §8 rows, and the two post-production tickets. | 2026-09-03 |
| ✅ Reconciled 2026-09-03 | `needs-decision` | **A 2026-08-14 decision ("event-rsvp response rows become canonical") DIRECTLY CONTRADICTS solved-pattern #24 (2026-09-03).** Pattern 24 forbids per-response `*-response` state machines (responses are DATA lists on the event/ballot); the older decision made response *rows* canonical. This older decision is the likely root of the masjid/book-club `*-response` regen breakage. **Reconcile before the next regen pass:** which stands — responses-as-data (pattern 24, the shipped packages' model) or response-rows-canonical (the 2026-08-14 milestone)? The shipped packages use responses-as-data, so pattern 24 is consistent with production; **RESOLVED 2026-09-03:** corpus measured — 4 of 5 RSVP communities (BookClub/CameraClub/GardenClub/YouthSoccer) use response ROWS; only masjid uses arrays. **Phase A.1 (response rows canonical) STANDS.** Pattern 24 REWRITTEN (`2cea3707`): rows are canonical + every reachable response state must be render-bound; the masjid/book-club regen bug was UNBOUND states, not the row model. The masjid/book-club fix on the next regen pass is render bindings (+ migrate masjid arrays→rows per Phase A.1), not responses-as-data. | 2026-09-03 |
| 🅿️ Deferred to next regen pass | Thread B | **masjid-nur + book-club regens deferred; skill fix (pattern 24) left in place (user 2026-09-03).** Root cause found and fixed at the source: the regens spawned per-response `*-response` state machines (going/maybe/declined) instead of response data lists — a states-vs-data violation whose unbound states broke `product_community_walkthrough_conformance_test`. **Solved-pattern #24** (`4bb90fb0`) now forbids this (STOP on any `*-response` type / answer-valued states; model as data on the single event/ballot). Verification (re-dispatch + suite) intentionally NOT run now — the fix applies on the next regeneration pass. **Shipped masjid + book-club packages remain in place and working, untouched.** The 4 clean regens (ad-free, cedar, data-portability, garden-club) stay adopted. | 2026-09-03 |
| ✅ THREAD C COMPLETE (2026-09-02) | Thread C | **Persistence path verified end-to-end by composition — the app IS server-authoritative when configured.** (a) SERVER: the deployed workflow-service serves the full remote-engine contract — POST create-instance → 400, POST transition → 400, GET changes → 400 (routes present + guarded). (b) CLIENT: app-shell remote-path tests pass **25/25** (offline_replica_coordinator, visible_change_replica, remote_service_configuration, replica_sync_policy) — the remote engine writes-to-server-first + replica-cache + change-feed-sync logic. (c) WIRING: part49 `LoomReplicaFallbackWorkflowEngineApi` routes every write to the remote workflow-service (confirmed in code). (d) ACTIVATION: `main.dart` enables the remote path when `configureLoomRemoteServicesFromEnvironment()` finds the service URIs. **The in-memory engine is only the local/demo default; the server-authoritative path is built, tested, deployed, and env-activated.** Remaining optional: one live app-run smoke-test (remote-configured app → deployed stack → Postgres → cross-device sync) for belt-and-suspenders. | 2026-09-02 |
| ⚠️ Not adopted (regen issue) | Thread B | **neighborhood-book-club re-regen NOT adoptable — still drops mute + render-binding gaps.** Unlike masjid, the enhanced skill did NOT keep mute here (count 0) — pattern #23 was **inconsistently applied**. Also 6 `no_render_binding_for_reachable_state` + 9 `noop_affordance` warnings (the masjid conformance-failure signature). Shipped book-club package KEPT. | 2026-09-02 |
| ✅ THREAD B COMPLETE (2026-09-02) | Thread B | **4 of 6 divergent regens adopted; 2 mute-droppers need a focused fix.** ADOPTED (validated pass + demo 160/160, zero regression, chmod 444): ad-free, cedar-commons-hoa, data-portability, garden-club (`56dc7998`). NOT adopted — shipped kept: **masjid-nur** (enhanced skill KEEPS mute+warns ✓ but regen breaks `product_community_walkthrough_conformance_test` + 7 render-binding gaps) and **neighborhood-book-club** (skill still DROPS mute — pattern #23 inconsistent — + 6 render-binding gaps). member-social-space stays post-production. **FINDING for a focused look:** the two mute-dropping communities' regens have (a) inconsistent keep-but-warn application and (b) render bindings that drop for reachable states, breaking harness/catalog conformance. Not a quick autonomous fix — needs dedicated skill + doc investigation (why render bindings aren't emitted for all reachable states, and why #23 didn't fire for book-club). | 2026-09-02 |
| ⚠️ Not adopted (regen issue) | Thread B | **masjid-nur re-regen KEEPS mute now (Thread A worked) but is NOT adoptable — regen breaks conformance.** The enhanced skill correctly kept `mute-thread`+`unmute-thread` with `// NEEDS IMPLEMENTATION` comments (pattern 23 ✓, validates pass/0-errors, fewer dead affordances than shipped). BUT the regen introduced **7 `no_render_binding_for_reachable_state`** warnings (RSVP response states shipped rendered) and **failed the demo suite (159 +1)** — `product_community_walkthrough_conformance_test` (docs/packages/assets/harness/catalog agree) with a Set mismatch, so the regen changed something structural the harness/catalog expects. **Shipped Mosque package KEPT; regen not adopted.** masjid is a repeat problem child (v1: dropped mute; v2: keeps mute but breaks conformance) — needs a focused look at what its regen restructures. | 2026-09-02 |
| ✅ THREAD A COMPLETE (2026-09-02) | Thread A | **Skill + validator correctness enhancements done.** (1) solved-patterns **#22** (choose archetype by interaction shape — a resource booking is `calendar`, not `event-rsvp`) and **#23** (a doc-promised affordance with no backing service is KEPT + marked `not_implemented`, never faked as a no-op nor dropped) added + bundle-mirrored (`1c0148d4`). (2) New validator warning **`noop_affordance`** (a `to:null` transition with no effects = dead affordance): registered in 05-validation.md + mirror (`a8ae9eda`), rule implemented + 4 tests (`fbf9094c`) — verified my own oracle: judges **490/490, 0 skipped**, finding-code **conformance green** both ways; validator on :8787 restarted and serving it (health 200). (3) stale `deriver_test` fixed — Cedar facility reservation now expects `calendar` (`0e137d3a`), provisioning suite **15/15**. Every future regen now has the archetype + keep-but-warn guidance at the source. **Next: Thread B** (re-dispatch + validate + demo-suite + adopt the divergent regens; masjid/book-club re-regenerate so mute is kept+warned). | 2026-09-02 |
| 📈 Regression quantified | Thread 1 | **ad-free regen adoption = ZERO regression.** The regen validated **status pass, 0 errors, 0 warnings** (live validator), and with it adopted onto the shipped asset the **demo-app suite passed 160/160** (`EXIT=0`, no failures) — the added notificationInbox + reminder render cleanly. Since the divergences are almost all add-only completions of the docs (and notificationInbox is a real rendered archetype), this strongly implies the other add-only regens (cedar, data-portability, garden-club, book-club) adopt cleanly too, each verifiable the same way. Adoption reverted pending the user's all-7 decision. **Recommendation: adopt the add-only regens (validate + demo-suite each); the masjid mute-drop is handled by the messaging ticket; member-social-space stays post-production.** | 2026-09-02 |
| 🅿️ Post-production (ticketed) | Thread 1 | **Messaging/mute API gap ticketed → Tickets/GAP-messaging-api.md (user 2026-09-02).** "messages" is not built, so the doc-promised **per-member mute** button is a dead affordance; the skill correctly drops it on regen. Interim: validator to WARN when an affordance's backing API is missing (see skill/validator enhancement). Real fix (messaging API incl. per-member mute) deferred to post-production. Supersedes the masjid mute-drop row. | 2026-09-02 |
| ✅ THREAD 2 COMPLETE (2026-09-02) | Thread 2 | **external_resource mapping shipped.** Spec (`communityId` on InstallCommunityPackageRequest, both repos byte-identical, `f9e3ade`/`e436c95`); backend (`installCommunityPackage` sets group `externalRef {community, communityId}`, `6d5f77d`, verified by a DB-querying integration test, deployed `0.3.8`/`39bd57e`, drift audit clean); provisioning sends `communityId` (`316a9c59`); backfill of the **10 real groups** done (cruft stays NULL). The group→community mapping is now STORED and RETURNED, not derived — closing the underscored/hyphenated hazard. **Part B (app reads externalRef) deferred:** the app shell has no fetched-groups lifecycle, so resolving from externalRef needs an async bootstrap before auth-API creation — a broader refactor; the injected config map still works meanwhile. **Pre-existing finding surfaced:** `deriver_test` expects Cedar facility reservation = `event-rsvp` but the shipped package derives `calendar` — stale corpus assertion, untouched, worth a follow-up. | 2026-09-02 |
| ✅ Near-identical | Thread 1 | **riverside-youth-soccer regen ≈ shipped — adoptable.** No keys added/dropped; the only diff is +3 `// NEEDS IMPLEMENTATION` honesty comments (checksum/transfer-id/receipt-id are platform-service gaps, not to be fabricated). Doc-honesty improvement, no behavior change. Essentially a clean provenance confirm. | 2026-09-02 |
| 📊 THREAD 1 COMPLETE (2026-09-02) | Thread 1 | **All 10 communities regenerated + diffed.** CLEAN (provenance restored / adoptable): Chess, camera-club, riverside-youth-soccer (3). ADD-ONLY divergences (regen completes doc features shipped lacks — notificationInbox, queues, audit, reminder-delivery): ad-free, cedar-commons-hoa, data-portability, garden-club, neighborhood-book-club (5, book-club also drops unbacked mute). DROP: masjid-nur (drops unbacked mute). GRAMMAR-BLOCKED: member-social-space (needs socialGraph+adSlot archetypes; skill refused force-fit). **Three findings:** (1) shipped packages under-implement their m15-era docs — most regens ADD doc-described features; (2) per-member MUTE has no backing (needs Fan Passport) so the skill drops it; (3) member-social-space force-fits nonexistent archetypes. **Awaiting user decisions before adopting any divergent regen (product changes).** | 2026-09-02 |
| ⚠️ Divergence (mixed) | Thread 1 | **neighborhood-book-club regen ≠ shipped — big reshape, set aside.** +61 lines. **Drops** `Mute`/`Unmute` (same no-backing mute as masjid — consistent skill behavior) and `set_reminder`; **adds** a queue/waitlist (`queuedFanIds`, `queueLength`, `myQueuePosition`), an access-request flow (`accessRequestStatus`, `approvedAccessorFanId`), and `deliver_reminder` + notification title/body. Mostly completing the doc (queue, access, reminder delivery) minus the unbacked mute. Held for batch policy. | 2026-09-02 |
| 🅿️ Post-production (ticketed) | Thread 1 | **[GAP ticket filed: Tickets/GAP-socialgraph-adslot-archetypes.md] Deferred to post-production by user 2026-09-02. member-social-space CANNOT regenerate — needs new archetypes; skill correctly refused to force-fit.** The regen agent produced NO package, reporting that the doc requires two `cardSurfaceFamily` families absent from the 13-family registry: a **social-graph** surface (connection suggestions, invite/accept/decline/cancel/**block/unblock/appeal**, per-relationship privacy, block-disables-messaging enforcement) and an **advertising-slot** surface (in-stream sponsored placement, banner reservation/**no-fill**, sensitive-context suppression, non-collapsing layout). It noted the **shipped package force-fits** these (connections→`approvalQueueItem`/`statusTimeline`, sponsored/no-fill→`notificationInbox`) and refused to reproduce the force-fit per the mandatory archetype-fit rule (correct: "if the grammar cannot express it, STOP and report the gap"). **Decision needed (grammar change → stop-and-ask):** (a) add real `socialGraph` + `adSlot` archetypes (platform work), or (b) accept the force-fit and keep the shipped package as-is (its provenance stays a documented exception). NOT adopted; NOT provenance-restorable as-is. | 2026-09-02 |
| 🔴 Divergence (DROP) | Thread 1 | **masjid-nur regen DROPS a doc-required feature — set aside, needs attention.** Unlike the add-only divergences, the regen **removed** the `mute-thread` transition (mute count 3→0, dropped `notifications_off`) while adding `eventHistory`/`unreadKeptAt`. The **doc requires it** (§158 `mosque-discussion-thread` lists "mute" as an action; §140 describes mute semantics), yet the regen omitted it — the classic "deletion invisible in a validator run" (regen still validates). **Root likely = a known gap:** per-member mute state needs Fan Passport support that does not exist (flagged in the ad-free regen's own gaps), so the shipped mute button may have no real backing and the skill declined to fake it. Decision needed: (a) implement per-member mute properly (needs the Fan Passport mute API — ties to the persistence/notifications work), or (b) accept mute as a display-only affordance and teach the skill to keep it. NOT adopted. | 2026-09-02 |
| ⚠️ Divergence (minor) | Thread 1 | **garden-club regen ≠ shipped — set aside.** +47 lines, **nothing dropped**, adds `muted`, `pickupDate`, `pickupTime`, `event`. Same add-only shape. Held for batch policy. | 2026-09-02 |
| ⚠️ Divergence (add-only) | Thread 1 | **data-portability-community regen ≠ shipped — set aside.** +241 lines, **nothing dropped**, adds a **notificationInbox** workflow (`notificationInbox`, `recipientFanId`, `read`/`unread`, `createInstance`) + audit fields. Third consecutive divergence adding a notificationInbox the shipped package lacks — shipped packages predate the archetype; regens complete it. Held for batch policy. | 2026-09-02 |
| ⚠️ Divergence (minor) | Thread 1 | **cedar-commons-hoa regen ≠ shipped — minor add-only, set aside.** Regen +42 lines/+2571 B, **nothing dropped**, adds just 3 fields (`acknowledged`, `engagementAudit`, `saved`) + 1 reminder; platformSource/notifications/audience/renderBindings all preserved. Same shape as ad-free: `regen ⊇ shipped`. Held for the batch policy. | 2026-09-02 |
| ⚠️ Divergence (user policy) | Thread 1 | **ad-free-community regen ≠ shipped — set aside, NOT adopted.** The skill regeneration is a **superset** of the shipped package (regen 87980 B / 2569 lines vs shipped 82521 / 2403), **nothing dropped**. It ADDS: a **notificationInbox** workflow (`notificationInbox`, `recipientFanId`, `read`/`unread`/`readAt`, +1 notifications, +1 renderBinding) — which the **doc justifies** (it has a "### Notification Delivery" section, so the *shipped* package was missing a doc feature); and a **reminder** workflow (`reminder` ×8, `reminderEnabled`, `leadHours`, `anchorDateField`) whose **only doc basis is an authoring note** ("AP-6 reminder for whoever authors the JSON"), so it looks **over-generated**. Adopting = a product change (new surfaces) needing UX + suite verification, not a byte-identical provenance confirm. Held for the batch policy decision. Chess was byte-identical; ad-free is the first divergence. | 2026-09-02 |
| ✅ Closed | `new-ticket` | **Change feed VERIFIED DEPLOYED & serving (2026-09-02) — the row above was stale.** Live evidence on the running workflow-service (`:30083`): `GET /v1/communities/{id}/changes` returns **400** (route present, validates per the spec's missing-correlation-id rule) while garbage routes return **404** (control) — so the route exists. And the deployed image `loom-workflow-service:1.0.2` was **built 2026-08-30**, *after* the change-feed commit `86f79b0d` landed **2026-08-29**, so the running image contains `listVisibleChanges`. Note `1.0.3` shares 1.0.2's image id (`d8857f49049b`) — same bytes, so no newer build is pending. Nothing to deploy; this closes B5's deploy question. (Full valid-request proof still needs an auth token; routing + spec validation + build-timeline are conclusive that it is deployed.) | verified live | 2026-09-02 |
| ✅ Closed | `new-ticket` | **Admin-role migration DONE & verified (2026-09-02) — Option A steps 4/5 substantially complete.** All 10 non-canonical admin roles migrated to canonical `<handle>-admin` via per-community DB transactions (create canonical, copy grants, re-point the `group_membership_role` member, delete old); `camera-club-admin` was already canonical. Verified with my own oracle + controls: every canonical id now equals `substr(group_id, len('loom_communities_')+1) || '-admin'` (all `t`), each community retains its exact member and grant count (5 each; masjid 28), **0 old-style ids remain**, and no dangling refs (admin roles were only in `group_membership_role`; `app_access_role`/`group_invitation_role` were empty). This removes the latent hazard: the canonical ids now match what `installCommunityPackage` creates, so a future apply **spares** them instead of orphaning admins. Step 5 (catalog) was found unnecessary — `community.*` already in the live catalog. app-access unchanged/stateless, authorization provably identical (permissions resolve from the same `role_permission` rows). | approved 2026-09-02 | 2026-09-02 |
| ⛔ Blocked (user) | `new-ticket` | **2 admin anomalies await your decision (NOT auto-touched).** (1) **`masjid-nur-admin` has 28 grants** — the 5 `community.*` plus **23 archetype workflow permissions** (approval_queue_item.*, discussion_thread.*, document_library.*, event_rsvp.*, form_entry.*, search_ai_answer.*). No other community's admin has these. The next `installCommunityPackage` would **wipe them to 5** via `setRolePermissions`. Decide: (a) trim to the canonical 5 now, (b) promote the 23 into a proper archetype-derived role for that admin, or (c) leave (they vanish on next apply). (2) **Duplicate underscored group `loom_communities_cedar_commons_hoa`** (roles `cedar_commons_hoa_admin` [4 grants] + `cedar_commons_hoa_member`, members `fan_alice`/`fan_bob`, created 2026-08-13) sits beside the real hyphenated `loom_communities_cedar-commons-hoa`. Looks like test cruft. Decide: delete the whole duplicate group, or keep. | anomalies from migration | 2026-09-02 |
| ⬜ Superseded — see row 257 (already closed) | `new-ticket` | **STALE as of 2026-09-06 — the 10-role rename this row and row 260 describe as blocked was already completed and closed same-day as row 257, and these two rows were simply never removed after.** Re-verified live, not recalled: `group_membership_role` shows all 10 formerly-irregular roles now in canonical `<handle>-admin` form (`hoa-admin`→`cedar-commons-hoa-admin`, `masjid-admin`→`masjid-nur-admin`, `soccer-admin`→`riverside-youth-soccer-admin`, etc.), each still with exactly 1 member. **The only genuinely outstanding piece is the duplicate Cedar group** (`loom_communities_cedar_commons_hoa`, underscored) — confirmed still present, 2 real members (`fan_alice`/`fan_bob`, joined 2026-08-13), its `cedar_commons_hoa_admin` role holding only 4 mismatched permissions, not the standard 5. That is a separate decision (delete a whole group with live members, not a role rename) and is tracked as its own row below rather than folded into this one. ORIGINAL: **UPDATE to the row above — the remaining Option A cleanup is low-stakes, and step 5 is UNNECESSARY.** Read-only DB facts: (a) the live `loom_communities` catalog **already contains all 5 `community.*`** (127 ids total) — so there is **no catalog replace to do**; the earlier "full delete-then-replace, could drop calendar.*" risk does not apply. (b) The 12 existing admin roles are **functional today**: 10 hold exactly the canonical 5 `community.*` grants + 1 member (just a non-canonical id like `hoa-admin`); `camera-club-admin` is **already canonical** (nothing to do); `cedar_commons_hoa_admin` in the **duplicate underscored group** `loom_communities_cedar_commons_hoa` is **incomplete** (only 1 of 5 `community.*`, 4 total) — a stale dup to investigate/remove; `masjid-admin` is **over-granted** (5 `community.*` + 23 extra). So step 4 reduces to: optional cosmetic id-rename of 10 roles (careful assignment-preserving migration — create canonical role, re-assign the 1 admin `fan_id`, delete the old), fix the cedar dup, trim masjid. **Nothing is broken; fresh communities already get canonical admins via steps 1-3.** Decision for the user: do the cosmetic migration + clean the 2 anomalies, or leave the working roles as-is. No live-data mutation performed. | blocked on user (low stakes) | 2026-09-02 |
| 🅿️ Deferred to Cedar's next regeneration | `new-ticket` | **The duplicate Cedar group (`loom_communities_cedar_commons_hoa`, underscored) — decision made 2026-09-06: leave it, clean up at Cedar's next community regeneration.** Confirmed live: it sits beside the real hyphenated `loom_communities_cedar-commons-hoa`, has 2 active members (`fan_alice`, `fan_bob`, joined 2026-08-13), and its `cedar_commons_hoa_admin` role holds only 4 mismatched permissions rather than the standard 5 `community.*`. **User's call:** `fan_alice`/`fan_bob` are legacy fan ids that should be deleted when Cedar Commons HOA is next regenerated, not as a standalone DB cleanup now. Not touched this session. Revisit when Cedar's community package is next regenerated through the Skill. | user decision 2026-09-06 | 2026-09-02 |
| ⬜ Superseded — see row 257 (already closed) | `new-ticket` | **STALE as of 2026-09-06 — same as row 259 above: the 10-role rename this row calls blocked was already done and closed as row 257 the same day.** Only the duplicate Cedar group remains genuinely open, now tracked as its own row below (⛔ Blocked, awaiting a decision on deleting a group with 2 live members). ORIGINAL: **Option A step 4 (migrate 12 existing admin roles) is a LIVE-MEMBER-DATA migration — holding for a go-ahead.** Read-only query of `loom_app_access`: there are **12 group-scoped admin roles, each with exactly 1 member assigned** in `group_membership_role` (the seed root admin per community). Most ids are irregular vs the new canonical `<handle>-admin`: `ad-off-admin`, `hoa-admin`, `chess-admin`, `portability-admin`, `garden-admin`, `masjid-admin`, `social-admin`, `book-admin`, `soccer-admin`, `tabletop-admin` (10 irregular); `camera-club-admin` is **already canonical**; and there is a **duplicate underscored group** `loom_communities_cedar_commons_hoa` (2 roles, 2 members) beside the hyphenated `loom_communities_cedar-commons-hoa`. The blunt reinstall/apply path would let the sweep DELETE each irregular role (my sweep-spare only protects the canonical id), orphaning all 12 admin members. **Safe plan (needs go-ahead):** (5) full catalog replace = current 111 perms ∪ 5 `community.*`, verified additive; then per community a TARGETED migration — create canonical `<handle>-admin` w/ `community.*`, RE-ASSIGN the admin `fan_id` from the irregular role to the canonical one, then delete the irregular role — never a blunt reinstall; special-case the cedar duplicate. Core Option A (steps 1-3) is done + deployed; this is cleanup of pre-existing data with real destructive potential, so it stops here for the user. | blocked on user | 2026-09-02 |
| ✅ Closed | `new-ticket` | **Option A STEPS 2+3 done (`2818e32` install logic, `a6bdd8c` deploy): the system `<handle>-admin` role is created at install, and app-access `0.3.7` is live.** `CommunityPermissionDeriver` parses `governance.adminRole` (gated on `isSystemDefault`), resolves the id from `idTemplate` by substituting `<communityHandle>`, and exposes `Derivation.systemAdminRole` — id-suffix and permission list come from the vocabulary as **data**, never hardcoded. `installCommunityPackage` creates `<handle>-admin` if absent, grants exactly the 5 `community.*` via `setRolePermissions`, unions them into `ensurePermissionsExist` (missing catalog → `unknown_permission_id`, fails loud), and — the load-bearing invariant — **spares it from the undeclared-role deletion sweep** beside `PLATFORM_ADMIN_ROLE_ID`; idempotent across reinstalls. Verified with my own oracle: read the full diff for all 6 invariants; re-ran the app-access module myself under JDK 21 / Testcontainers → **66 passed, 0 failures, 0 errors, 0 SKIPPED** (AppAccessIntegrationTest 20 incl. new reconcile + catalog-missing tests). Deployed to k3s: rollout finished, pod 1/1, serves `0.3.7`, no node-stall collateral; three-source audit clean; spec parity 8/8 + vocab 1/1. **Next: step 5** (publish the permission catalog so `community.*` exists — additive) must precede **step 4** (migrate the 11 irregular stopgap admin roles to canonical `<handle>-admin` — potentially touches member assignments, evaluate for a user checkpoint) and **step 6** (live prove admin creation + reinstall-preserves + instance/`deliver_reminder`). | verified, committed, deployed | 2026-09-01 |
| ✅ Closed | `new-ticket` | **Option A STEP 1 done (`02ab58b7`): `community.*` is a generated vocabulary family.** The generator emits a `governance` section beside the archetype families — `permissionPrefix: community`, the 5 actions/permissions, and a machine-readable `adminRole` object (`isSystemDefault: true`, `idTemplate: "<communityHandle>-admin"`, `grantedPermissions: [the 5]`) that step 2's installer consumes as data. Vocabulary **111 → 116**, exactly the 5 `community.*`. Verified with my own oracle: `--check` exits 0 (file matches generator), the section reads as specified, judges **486** passed 0 skipped 0 failures by exit status. permissions.md §7 (committed earlier) documents the model; sync test passes. **Next: step 2** — `installCommunityPackage` creates the `<handle>-admin` role, grants it `community.*` from this governance section, and spares it from the undeclared-role deletion (backend Java). The vocabulary twin in loom-backend is behind (111) until step 2 rebuilds app-access; parity check flags it until then, expected. | verified, committed | 2026-09-01 |
| ✅ Closed | `needs-verification` | **Option A chosen 2026-09-01, and it is smaller than feared: only `community.*` (5) is held-but-unreproducible.** Measured — of the 84 permissions any role actually holds, exactly **5 are absent from the 111-id generated vocabulary**, and all 5 are `community.*`. Every other deployed-only catalog id (`approval_queue.*`, `discussion_thread.*`, `notification_inbox.*`, `payment_checkout.*`, `equipment_loan.borrow`, `event_rsvp.manage_attendees`, `table.publish`, …) is held by **no role**, so a vocabulary-driven catalog rebuild that drops them breaks nothing. **Consequence: the "Group 1 resolver-completeness" prerequisite is DROPPED from the critical path** — completing the resolver for those ~16 unused actions is optional cleanup for future packages, not a blocker. Option A is now purely: (A1) add `community.*` as a generated governance family so the catalog rebuild includes it; (A2) the deriver grants `community.*` to each community's admin role on install. Still to pin: the governance-role model, because `permissions.md §7` documents the **abandoned** single-app-level-`admin` model while the deployed reality is per-community `<prefix>-admin` roles (the user's 2026-08-30 decision). | measured, de-scoped | 2026-09-01 |
| ⬜ Superseded → [GAP-permission-catalog-publishing](Tickets/GAP-permission-catalog-publishing.md) | `stale-mechanism` | **STALE, corrected 2026-09-07 against deployed code and the live DB — both mechanisms this row feared are gone or were never real.** (1) The admin-stripping fear predates backend `2818e32` (in deployed `0.3.9`): install now derives the vocabulary's `governance.adminRole` (`isSystemDefault`, `<communityHandle>-admin`, exactly the 5 `community.*`), creates it, grants it, and shields it from the removal sweep. Live: all 11 communities hold that role with exactly 5; no role named `admin` exists. (2) The "via FK, dropping them cascades or blocks" claim is wrong: `role_permission` has **no foreign key to `permission`** at all — a catalog replace can neither strip nor block a grant; it only reports orphans. **What is real and remains**: the live catalog (127 ids, published 2026-08-26 from a hand-maintained file) lacks the 9 `calendar.*` + `event_rsvp.send_reminder` added to the vocabulary on 08-31, and carries 21 retired ids (all with zero grants). Root cause is a parallel hand-maintained artifact plus a manual publish step; the fix is structural and is the ticket above (generate the catalog from the resolver, publish on backend startup, gate the twin in `build.sh`). User's design ruling recorded there. One open ruling carried: whether the admin role also carries workflow permissions (it holds only the 5 today; `masjid-nur-admin` would be trimmed 28→5 on re-install). **ORIGINAL:** Declaring community admins as package roles stops the DELETION but not the STRIPPING — the calendar apply still needs an install-merge change. Traced the deriver 2026-09-01: `CommunityPermissionDeriver.derive()` starts each declared role from an **empty** permission set and adds only what its workflows grant; `installCommunityPackage` then **replaces** the role's stored permissions with that derived set. `DerivedRoleInput` carries only `roleId`/`label` — a package cannot declare a role's permissions — and the deriver produces **no `community.*` permission at all** (verified with a control). So the five governance permissions each admin holds (`community.view/invite/manage_members/manage_roles/manage_settings`), which exist only because the 2026-08-30 provisioning hand-granted them, have **no derivation path**. Running install with admin declared would keep the role but set its permissions to the archetype-derived subset — dropping all five `community.*`. The two decisions are therefore: **(1)** declare admins as package roles (stops deletion, confirmed by user), and **(2)** — newly surfaced — change install so it **merges** rather than replaces: replace only the permissions the deriver actually manages (the archetype vocabulary) and preserve any a role already holds that the deriver has no authority over (`community.*`). Option (2) is the principled fix and matches this session's recurring rule — a tool must not silently mutate what it has no authority to produce. Alternatives (re-grant after install; give governance a derivation path) are more fragile or larger. **Until (2) is decided, the calendar apply must not run**, or it strips governance grants exactly as the provisioning apply would have deleted the roles. | traced, needs the merge decision | 2026-09-01 | **WIDER THAN THE ROLE GRANTS — the CATALOG has the same shape, confirmed 2026-09-01.** Even with `0.3.6` deployed (it bundles the 111-id vocabulary incl. `calendar.*`), the live `permission` catalog still holds **127 ids and no `calendar.*`** — deploying the image does not write the catalog; only `replacePermissionCatalog` or `install` does, and both are **vocabulary-driven**, so they would replace the catalog with the 111 generated ids and **drop the ~21 deployed-only stopgaps** — including the five `community.*` governance permissions the admin roles hold (via FK, dropping them cascades or blocks). So the deployed authorization state (catalog **and** grants) was hand-assembled and diverges from the generated tooling in both dimensions, and every tooling path (`install`, `replacePermissionCatalog`) clobbers the hand-assembled parts. **This reframes the decision** from "make install merge" to a governance question: `community.*` was deliberately excluded from the archetype vocabulary because governance is not archetype-derived — so it needs its **own generated source** the catalog build includes, or the provisioning ops must be made merge-safe against permissions they do not own. Until one is chosen, no vocabulary-driven catalog/grant operation is safe to run, and `calendar.*` cannot reach a role because it is not in the catalog for a grant to reference.
...
| ✅ Closed | `new-ticket` | **FIXED & VERIFIED LIVE: `setAppAccess` and `revokeAppAccess` have no authorization — the app-level half of the escalation class.** The agent correctly refused to guard these with a group-scoped check and reported it: there is **no platform/app-administrator predicate** to apply, and inventing a group-admin stand-in would be wrong for endpoints that are not group-scoped. So a plain member can still set or revoke any fan's **app-level** access and roles via `PUT`/`DELETE /v1/apps/{appId}/access/{fanId}`. **Fix needs a design decision first**: what authority governs app-level access — a platform-admin role, a service-only credential, or something else. Until then this is the highest open security item. Same one-request probe confirms it (not yet run, to avoid granting app-level admin even briefly). | needs an app-admin authority | 2026-09-01 | **FIXED AND VERIFIED LIVE 2026-09-01 (`2486841`, deployed `0.3.6`/`adb90e0`).** `setAppAccess`/`revokeAppAccess` now call `CallerActor.requireServicePrincipal`, which refuses any token carrying a `fanId` claim (member tokens have one; service tokens do not — the agent deliberately did not key on `azp`, not an authorization boundary). **Proof, not report**: a member `setAppAccess` granting itself `platform_admin` now returns `403 service_principal_required` (was `200`), zero rows left; the group fix still holds under `0.3.6` (`403`). Suite 64 tests (was 60), 0 failures. **The whole confirmed escalation class is now closed** — group half `0.3.5`, app-level half `0.3.6`. The broader provisioning endpoints remain a separate, unaudited question.
| ✅ Closed (swept 2026-09-03) | `new-ticket` | **✅ FIXED & VERIFIED LIVE — group half; app-level half tracked below on `setGroupMembership` — the same class the 2026-08-30 fix closed elsewhere, on a path it did not cover.** Found 2026-09-01 auditing before the passport re-key. `PUT /v1/apps/{appId}/groups/{groupId}/memberships/{fanId}` (`setGroupMembership`, service line 614, controller 300) sets any fan's `state` and `roleIds` in any group, and is guarded by **neither** `requireGroupAdministrator` (only `issueInvite` and `decideGroupMembership` call it) **nor** `CallerActor.resolve` (only `requestGroupMembership`, `decideGroupMembership`, `issueInvite`, `getInvitation`, `redeemInvite` call it). `JwtSecurityConfiguration` authorizes the route with `anyRequest().authenticated()` and no scope or role. **So any member with a valid token can grant themselves — or anyone — any role in any community, `admin` included.** The 2026-08-30 escalation fix bound the actor to the token's `fanId`, but through `CallerActor`, which this handler never invokes. **Not yet exploited to confirm** — recorded as `needs-verification` rather than proven, and the check is a single authenticated `PUT` with a mismatched actor. **Do not close the earlier escalation item as fully resolved until this path is covered.** This is now the highest-priority backend finding, above the calendar deploy. | code-read, needs a live probe | 2026-09-01 | **CONFIRMED LIVE 2026-09-01, then reverted.** Probed on the dev cluster: a token for `fan-garden-member-1` (holding only `garden-member`) sent `PUT /v1/apps/loom_communities/groups/loom_communities_garden-club/members/probe-escalation-throwaway-1` with body `{state: active, roleIds: [garden-admin]}` and `X-Loom-Actor: fan-garden-member-1`. Result: **HTTP 200**, and the row persisted — `group_membership_role` showed `probe-escalation-throwaway-1 → garden-admin`. So a plain member created an **admin** membership for an arbitrary fan, in a community where they hold no admin role, against a target that is not themselves. Reverted immediately via `DELETE .../members/probe-escalation-throwaway-1` (204); both `group_membership` and `group_membership_role` confirmed at **0** probe rows afterward. This is no longer `needs-verification` — it is a proven, unauthenticated-in-effect escalation, and the 2026-08-30 escalation item is therefore NOT fully resolved. The route noun is `members/{fanId}` (a first probe using `memberships/` 500'd as a routing miss, not a handler response). Fix: route `setGroupMembership` through `CallerActor.resolve` and `requireGroupAdministrator`, exactly as `decideGroupMembership` already is.
| ✅ Answered (no code change needed) | `new-ticket` | **The question this row's own body already posed ("is local persistence wanted, or is the local engine deliberately an ephemeral demo shell whose replacement is the remote engine?") is answered: the latter, confirmed live in the code, not assumed.** `app/apps/loom_communities_demo/lib/main.dart` calls `configureLoomRemoteServicesFromEnvironment()` at startup; `LOOM_ENV` defaults to `dev`, so this returns non-null on every real launch and calls `configureEngineNativeCommunityEngineFactoryForProduction(createRemoteEngineNativeCommunityEngineFactoryForConfiguration(...))`, overriding `_EngineNativeCommunityStore`'s engine factory from the local `WorkflowDatabase.memory()` path to the remote, Postgres-backed `workflow-service`. The `WorkflowDatabase.memory()` field still gets constructed (it's a `late final`), but the `engine` getter resolves through the overridden factory, not the local database — matching the already-confirmed "server-authoritative default" row above. `part02_tab_shell.dart:754`'s in-memory database is a separate, narrower case: it backs only `_MessagesEngineStore`, the not-yet-implemented Messages tab (per standing instruction, out of scope). So the local in-memory engine genuinely is what the row's own question called "an ephemeral demo shell" — it only matters for local/offline dev builds (`--dart-define=LOOM_ENV=local`), which is intentional, not a gap. ORIGINAL: **`WorkflowDatabase.memory()` is the app's only engine database.** Both construction sites use it, so nothing survives a restart — and the worse half is that in-memory also means per-device: two members hold private copies and never converge. | re-verified live 2026-09-06 | 2026-08-31 |
| ⬜ Superseded — see the two ✅ Closed rows above (RLS activation, Superuser sweep) | `new-ticket` | **STALE as of 2026-09-06 — a near-duplicate of row 233 (already closed), describing the same "inert" state that was resolved and closed above it.** Re-verified live during the 2026-09-06 tracker sweep: `pg_stat_activity` shows all four services connected as their restricted roles, not the `loom` superuser, and `loom_workflow_app` has `rolsuper=f, rolbypassrls=f` — the enforcement this row calls a future need is already done. ORIGINAL: **Community isolation is a `WHERE community_id = ?` clause** — not schema-per-tenant, not row-level security. It holds only as long as every query remembers it, and nothing in the database enforces it. One omitted predicate is a cross-community data leak that no test would catch. **RLS BACKSTOP BUILT & COMMITTED 2026-09-03 (`0bcaaf32`):** `FORCE ROW LEVEL SECURITY` + a `community_isolation` policy `USING (community_id = current_setting('app.current_community_id', true))` on all 8 workflow-service tenant tables, per-request `SET LOCAL` context threaded through `runWithCommunity`/`runTx`, cross-community publisher exempted. A FOR UPDATE lock regression the session-context refactor introduced was caught by my own verification (pool test failed 3/3 in isolation) and fixed (assertions unchanged; suite 149 passed / 2 skipped / 0 failed). **But it is INERT: every backend service connects as the `loom` Postgres SUPERUSER with `BYPASSRLS`, which ignores `FORCE RLS`** (confirmed live `rolsuper=t, rolbypassrls=t`; app-access/fan-passport/keycloak default `DB_USERNAME=loom` too). Enforcement needs a `NOSUPERUSER`/`NOBYPASSRLS` runtime role + credential rotation — tracked in `Tickets/GAP-rls-restricted-role.md` (decision parked with the user). `postgres_rls_integration_test` skips with that reason under a bypass role and activates under a restricted one. | TODO.md cross-cutting, migrated | 2026-08-31 → 2026-09-03 |
| ✅ Closed (`836100b8`) | `new-ticket` | **Idempotency is reimplemented per repository** — `document_repository`, the bundle repository and the queue each carry their own version. Divergence between them is silent, and the correct behaviour is subtle enough that three independent implementations will not stay in agreement. **UNIFIED 2026-09-03 (`836100b8`):** extracted `runIdempotent<T>` (`lib/src/idempotency.dart`) — lookup → `INSERT ... ON CONFLICT DO NOTHING RETURNING` → reload-on-losing-conflict, all in ONE community transaction; the three `Idempotency-Key` flows (doc create, doc revision, export bundle) now use it; the queue `join` (already atomic) is the reference, left unchanged. **This fixed a real TOCTOU bug the groundwork uncovered:** the three flows previously SELECT-then-INSERT across two transactions, so concurrent identical requests raced and the loser'`'`s unhandled unique-violation (23505) surfaced as HTTP 500 instead of the intended replay. New live concurrency test proves the fix (1 created + 1 replay, same record, no 500, per flow). Contract preserved (201/200 per endpoint; the doc-revision 201-on-replay inconsistency preserved + flagged, not changed). Suite 152 passed / 2 skipped (unchanged) / 0 failed, analyze clean — MY OWN oracle. | TODO.md cross-cutting, migrated | 2026-08-31 → 2026-09-03 |
...
| ✅ Closed (stale→verified) | `new-ticket` | **The change feed is built but not deployed.** `listVisibleChanges` is present in `workflow_service.dart` and the cursor was amended to a keyset pair (spec `033371c3`); it ships with the next workflow-service image. Built ≠ deployed — the four-state vocabulary in §9 exists because this distinction kept getting lost. | TODO.md B5, migrated | 2026-08-31 |
| ✅ Closed (swept 2026-09-03) | `needs-spec-decision` | **`app_group.external_resource_type` / `external_resource_id` are NULL for all 24 groups.** The columns that would map a group to its community exist and were never populated, so there is no stored join between App Access groups and community packages. Decide whether these columns are the mapping or whether the mapping lives elsewhere, before anything is built on them. — **CLOSED 2026-09-03:** fixed: 10 real groups backfilled; install populates externalRef | TODO.md, migrated | 2026-08-31 |
| ✅ Closed (swept 2026-09-03) | `needs-verification` | **Do not derive the community key from the group id.** It looks derivable and is not: workflow-service keys `community_cedar_commons_hoa` (underscored) while groups use the hyphenated handle. Read both; never compute one from the other. | TODO.md, migrated | 2026-08-31 |
...
| ✅ Closed (swept 2026-09-03) | `new-ticket` | `masjid-admin` held **0** `community.*` permissions before today; audit every other role for governance grants that were assumed rather than checked — **CLOSED 2026-09-03:** resolved: migration grants the 5 community.* to canonical admins | admin-role provisioning | 2026-08-31 |
| ✅ Closed (user-approved 2026-09-06) | `new-ticket` | **12 of 24 orphaned groups deleted — user go-ahead given, executed and verified.** A single scoped SQL transaction deleted exactly the 12 approved groups by explicit `group_id IN (...)` list (never a wildcard): the 10 empty duplicates (`ad_free_community`, `camera_club`, `chess_club`, `data_portability`, `garden_club`, `member_social_space`, `mosque`, `neighborhood_book_club`, `riverside_youth_soccer`, `verify_tabletop_club`) and the 2 `b3-e2e-*` test-run artifacts. `DELETE 12` returned, matching exactly. Verified with my own oracle, immediately after: (a) `app_group` now holds exactly 12 `loom_communities_*` rows — the 11 real communities plus the deliberately-kept Cedar duplicate (deferred to its own regen, see the row above); (b) every one of those 12 has the **exact same active-member count as before the delete** (3,3,4,2,4,4,3,2,3,3,4,3 — byte-for-byte identical, zero collateral damage); (c) `app-access` still serves post-delete (`401` = up + auth-enforcing). The two `fan-b3-allowed-*` fan accounts themselves were not targeted — only their `group_membership` rows, which cascaded away with their group. | user-approved 2026-09-06 | 2026-09-06 |
| ⬜ Superseded — see the ✅ Closed row above | `new-ticket` | **Full breakdown of the 24 orphaned groups, re-verified live 2026-09-06 — awaiting a go-ahead for the disposable ones.** The 24 = 11 real communities (untouched, 2-4 members each) + **10 completely empty duplicate/leftover groups** (`ad_free_community`, `camera_club`, `chess_club`, `data_portability`, `garden_club`, `member_social_space`, `mosque`, `neighborhood_book_club`, `riverside_youth_soccer`, `verify_tabletop_club` — zero members each, zero risk to delete) + **2 synthetic `b3-e2e-<timestamp>-<id>` test-run artifacts** (1 member each, and the member's own `fan_id` is literally `fan-b3-allowed-<matching-timestamp>` — both the group and its member are disposable fixtures from an automated test run dated 2026-08-15) + **the Cedar duplicate** (deferred separately, see the row above — leave until Cedar's next regeneration). `app_role_group_fk` cascades, so deleting a group takes its roles with it. **Root cause of the 10 underscored duplicates, found via `app_group.created_at`/`external_resource_id`:** all 10 were created within the *same second* (`2026-08-26 01:16:31`) with `external_resource_type`/`external_resource_id` both **unset** — an early, incomplete provisioning run that derived group ids from `display_name` (spaces→underscores) instead of the real hyphenated community handle, and never linked them to a real package. ~1 hour later (`02:14:40`–`02:37:04`), a second, correct run created the real hyphenated groups one at a time, each properly linked (`external_resource_type: community`, e.g. `external_resource_id: community_ad_free_community`) — but nobody deleted the first batch's leftovers. Decision needed: delete the 10 empty + 2 e2e groups (12 total) now? ORIGINAL: Orphaned underscored groups (24 groups for ~11 communities) are unreachable but still read as product data. `app_role_group_fk` cascades, so deleting a group takes its roles | group-spelling analysis, re-verified live 2026-09-06 | 2026-08-31 |
| ✅ Closed (swept 2026-09-03) | `needs-verification` | The four governance permissions `community.view`, `manage_roles`, `invite`, `manage_settings` are held by **nobody**; only `manage_members` was granted — **CLOSED 2026-09-03:** resolved: canonical <handle>-admin roles hold all 5 community.* (DB-verified) | admin-role provisioning | 2026-08-31 |
...
| ✅ Closed (re-verified live, 2026-09-05) | `new-milestone` | **Row (c) below is stale — re-checked live on the real emulator, not reproducible as phrased.** The 2026-08-22 finding ("Garden `garden-event-rsvp` renders its medium surface for `spring-workshop` but tapping exposes no expanded detail") described the in-memory-engine-only walkthrough era; the remote-backend default landed 2026-08-26 (`c1158739`), four days later, and seed data like `spring-workshop` only loads for the in-memory engine (`part25`'s `_EngineNativeCommunityStore._initialize` bails for anything else) — so testing this at all required rebuilding with `--dart-define=LOOM_ENV=local` alongside the usual preload flag, a device-state change worth remembering for the next live check. With that build: the full detail card (title, 2/2 going, capacity bar, attendees split Going/Pending, fact chips, edit form, all 6 actions) renders **unconditionally** on the calendar tab — proven by a before/after pixel diff of the tap that shows only the date-chip selection changing (rows 541–640) and the entire agenda row + detail card pixel-identical (rows 641–2400). Cross-checked against the Home tab's `bindingKind: "summary"` binding for the same workflow, which renders the identical full surface — confirming this is how the shell renders these bindings everywhere, not an instance-specific gap. Neither `calendar.md` nor `event-rsvp.md` (453 lines) mentions an expand/drill/disclosure pattern at all. **Corrected framing:** `garden-event-rsvp` renders its detail inline and unconditionally; no tap-to-expand mechanism exists or is required for this surface. **New finding surfaced by the same check, independently confirmed via my own screenshot** (not just the dispatch's report): several numeric-field labels on this same edit form render with their `{value}` template token stripped instead of substituted — `"Default reminder:  hours before"` (should read "24 hours before"), a bare `"minutes"` label (should read "120 minutes"), matching the community JSON's own `labelTemplate` declarations (`"Default reminder: {value} hours before"`, `"{value} minutes"` at lines 301/355 of the shipped Garden Club package) — so the JSON is correct and the gap is in how an edit-context field label renders a `labelTemplate` meant for read-only tile/detail display. Not fixed here: the correct behavior needs checking against how `labelTemplate` is meant to behave in an editable-form context specifically (vs. the display contexts it clearly works for elsewhere) before changing rendering code, to avoid guessing at intended behavior and introducing a regression. Filed as a fresh, separate open item below rather than folded into this retraction. | live-verification dispatch + independent screenshot re-check | 2026-09-05 |
| ✅ Closed (`9397aa85`) | `new-ticket` | **Edit-form field labels render a stripped `{value}` labelTemplate token instead of substituting it — FIXED.** Decision made: edit-context labels never substitute the live value (correct — the input box already shows it); the fix is that a template with a *non-trailing* `{value}`/`{value.length}` token now falls back to the humanized field key instead of leaving broken static text with a value-shaped hole. Trailing-token templates (`"Label: {value}"` → `"Label"`, the vast majority of the corpus) are completely unaffected. `reminderOffsetHours`/`durationMinutes` now render `"Reminder Offset Hours"`/`"Duration Minutes"` instead of the broken text. Verified independently, not the dispatch's self-report: reviewed the diff directly (one function, 11 insertions/5 deletions) and re-ran `loom_communities_app_shell` myself — 377 passed/2 skipped/0 failed, matching the dispatch's own count. No existing test encoded the broken behavior. No community JSON touched — every `labelTemplate` string was already correct. | live-device re-check of row above, then implementation-agent dispatch | 2026-09-05 |
| ⬜ Superseded (see closed row above) | `new-ticket` | **Edit-form field labels render a stripped `{value}` labelTemplate token instead of substituting it.** Found while re-verifying the row above: Garden Club's `garden-event-rsvp` edit form shows `"Default reminder:  hours before"` (double space where `24` belongs) and a bare `"minutes"` label (should read `120 minutes`) — both match declared `labelTemplate`s (`"Default reminder: {value} hours before"`, `"{value} minutes"`) that clearly work in other (read-only tile/detail) render contexts. Purely cosmetic — the actual field values (`24`, `120`) are shown correctly in the adjacent editable input boxes, nothing is lost or wrong, only mislabeled. Needs: (1) confirm whether this affects every `editableFields` label using `{value}` syntax or just numeric/duration fields, by checking a few more communities' edit forms; (2) decide the correct behavior — should an edit-context label substitute the field's *current* live value the same way a display-context label does, or should edit-form labels simply drop the `{value}` token/braces and render the surrounding text only? — before writing a fix, since guessing wrong here risks a UI regression elsewhere `labelTemplate` is used correctly today. | found via live-device re-check of row above | 2026-09-05 |
...
| ✅ Closed (spec `6d0acfea` 2026-08-31, authoring guidance `92c1eed0` 2026-09-04) | `needs-spec-decision` | **RESOLVED — there was no contradiction to settle; `send-reminder` and `deliver_reminder` are two different things and the spec now says so.** User ruling 2026-09-04: *"send-reminder and deliver_reminder are two different things — one is the buttonless deliver-to-everyone automatic, the other is the manual button."* So none of the three options below was taken: the id `send-reminder` was never meant to map to `deliver_reminder` at all. `6d0acfea` (2026-08-31) had already split them — `event-rsvp` gained a distinct `send_reminder` action for the role-guarded manual button, leaving `deliver_reminder` for the platform path — which means Chess's `chess-organizer` and Soccer's `soccer-coach` guards were correct all along and keep their buttons. This row was simply stale, written 2026-08-22 against the pre-split mapping. `92c1eed0` (2026-09-04) closes the user's follow-on ask by adding the author-facing half the spec still lacked: a "Which one do I write?" rule (ask whether a clock or a person decides it goes out now), the concrete JSON shape for each path, the note that both in one workflow is normal, and the two mistakes to avoid. Also corrected the `deliver_reminder` row's "observed transitions" cell, which listed `deliver-reminder` as though authors write one — measured: **zero of eleven shipped packages declare it and none should**; real delivery is `LoomReminderSweeper` sweeping `dueNotifications(asOf:)`, per the engine's own comment. Bundle mirror refreshed byte-identically (`cmp`-verified); action ids and `###` headings untouched so the spec-sync set-equality parse is unaffected. **ORIGINAL (stale, kept for context):** `deliver_reminder` applies cleanly to two of four candidates; Chess and Soccer expose a contradiction inside permissions.md itself and are deliberately UNAPPLIED. Applied 2026-08-22: Tabletop `event-rsvp-response/send-reminder` (a0184b0c) and Book Club `book-meeting-rsvp-response/send-reminder` (6b42a22d). Both are guarded by `actorEqualsField: fanId` alone with `dueAt`/`notificationTitle` inputs -- the platform-delivery shape -- so id and guard agree. **Chess (`chess-club-night/send-reminder`, guard `allowedRoleIds: [chess-organizer]`) and Soccer (`soccer-practice-rsvp-response/send-reminder`, guard `allowedRoleIds: [soccer-coach]`) name a human role.** permissions.md:152 maps the id `send-reminder` to `deliver_reminder`, but the same entry says a `deliver_reminder` transition **names no role, so section 1 grants it to nobody and it renders as no button anywhere**. Those two rules cannot both hold for a transition guarded to `chess-organizer`. Applying it would either hide a button an organizer legitimately needs, or grant `deliver_reminder` to a role and break the stated mechanism. **permissions.md:166 predicts exactly this**, saying guard shape gets 8 of the corpus 10 reminder transitions right and *breaks on the other two* -- Chess and Soccer are demonstrably those two. **Options:** (a) treat guard shape as decisive, leave Chess and Soccer as member actions, and narrow the spec table to say the id maps to `deliver_reminder` only when the guard names no role; (b) treat the id as decisive and also strip their `allowedRoleIds`, which changes who can act; (c) give the two a distinct id so nothing has to disambiguate. A generated Chess package is ready and deliberately unapplied at `/home/fahd/.codex-skill-authoring-scratch/chess_reminder/`; Soccer was never dispatched. **Also corrects this tracker:** an earlier row says five transitions move and names Camera `set-reminder` among them. permissions.md:151 lists `set-reminder` under `set_reminder` explicitly, so Camera stays and the count was never five. | corpus measurement against permissions.md | 2026-08-22 |
...
| ✅ Closed (Loom `db471847`) | B25 finding — UI auth gap | **CLOSED — the one real defect the full 10-community B25 grind found is fixed, tested, and independently re-verified.** Live walkthrough of Export and Migration found a Member persona could focus and type into an Owner-only `redactionValidationResult` field. Root cause (`docs/Build Plan V2/Evidence/B25/root-cause-editguard-export-migration-2026-09-04.md`): `GenericWorkflowInstanceCard` never received a role and never checked `state.editGuard`, so any guarded `editableFields` rendered as an enabled editor for every role. **Confirmed NOT a privilege escalation**: the local engine's write path (`_passesGuard`/`evaluateGuard`) independently resolves role from the account's actual membership and would reject the write with `WorkflowAuthorizationError` — an existing test (`cal_calendar2_guard_enforcement_test.dart`) already covers this. Fix: threaded `roleId` into the card from both `EngineNativeArchetypeCard` construction sites, added the `editGuard` check, invalidated cached edits on role change, added a production-path regression test. My own oracle: re-ran both suites independently — `loom_communities_app_shell` `+373 ~2 All tests passed`, `loom_workflow_engine` `+312 ~5 All tests passed` — matching the dispatch agent's report exactly. | this effort | 2026-09-04 |
| ✅ Closed (Loom `de70b43d`) | Shell UI bug | **`Local package details` disclosure header no longer white-on-white — shell-wide fix, every community.** Found and root-caused during the Platform Social B25 walkthrough (1.00:1 contrast measured on-device, header fully invisible in both collapsed and expanded states). Removed the four hardcoded `Colors.white` overrides at `part01_local_extension_screen.dart:1826-1832`; the header now resolves color from the surrounding theme. Added a stable key + a widget test asserting the header is never `Colors.white` in either state. My own oracle: re-ran `loom_communities_app_shell` independently — `+374 ~2 All tests passed` (baseline 373 + this fix's new test, 0 failed). A companion fix for the role-radio bug from the same ticket was built, then independently caught as a regression before committing and reverted separately (see the next row). | B25 Platform Social walkthrough | 2026-09-04 |
| ✅ Closed (Loom `3ad79072`) | Shell UI bug | **The role radio in "Account role and permissions" is a genuine, full functional no-op under a session — the B25 finding was correct and complete. The apparently-conflicting marketplace test's pass is a timing false positive, not evidence the switch works.** Root-caused after a first fix attempt (disable non-account-role rows) broke that test and was reverted (see above). Confirmed mechanism: `_activeFanId`/`_activeRoleId` (`part01_local_extension_screen.dart:214-225`) always prefer `currentSession.account.*` over the dialog's `_selectedFanId`/`_selectedRoleId` whenever a session exists, so a signed-in member who taps Organizer is rebuilt as the SAME member account/role — every downstream consumer (tab set, legacy workflow cards, tab/instance-scoped create actions, `EngineNativeMarketplaceSurface`'s engine `fanId`) reads that same unchanged `activeActorIdentity`. **The marketplace test's green result is a race, not a role change**: the no-op tap still calls `setState`, which re-parses the package experience, which makes `EngineNativeBindingDispatcher` see a non-identical definitions map and clear+reload all bindings; the test's `_selectMarketplace` only waits for the marketplace ROOT to reappear, not for each card's async `availableTransitionsAsync` to resolve, so it samples the reloading Catan card mid-flight (borrow action absent because still loading, not because Jordan/member became Organizer). Re-ran the exact test on unmodified code to confirm this explanation. **Confirms the intended identity model is genuinely single-role-per-account**: `RemoteLoomAuthApi._onlyRoleId` explicitly throws unless exactly one role is returned, and the Loom skill's own docs call this picker a "local testing harness" (production role comes from server-authoritative membership). **Recommended fix** (NOT the previously-reverted "disable other rows" approach): for an authenticated session, render the account's role as a non-interactive summary + a "Switch account"/"Sign in as a specific person…" action instead of fake selectable radios; preserve the direct role-select fallback ONLY for the sessionless legacy test-harness path. Also fix the marketplace test itself: its organizer step should use the existing `_selectActorIdentity` sign-in helper (not a raw tap on the role radio) and should wait for the card's load to settle before asserting, plus add a new regression test proving identity strip + tab set + a workflow action all change together on a real account switch. **IMPLEMENTED AND VERIFIED (`3ad79072`):** exactly the recommended fix, no scope creep. Authenticated sessions now see a non-interactive account/membership summary + informational "available community roles" (no `onTap`) + a "Switch account" action reusing the existing sign-in flow; the sessionless legacy path is byte-identical to before (confirmed by direct diff read). Fixed 4 existing tests that drove the old radio directly under a session to use the proper sign-in helper (pure interaction swaps, zero assertions weakened — confirmed line-by-line). The marketplace test now performs a real switch, asserts session/identity postconditions, adds a direct engine-level assertion (`organizerTransitionIds` excludes `borrow`), and waits for the reload to settle before checking the UI — strictly strengthened, not weakened. New regression test proves all three effects change together on a real switch (session, tab set 5→6, Calendar create FAB) and proves the dead radio key no longer renders and the "available role" row is non-interactive (`onTap == null`, tap has zero effect). My own oracle: read every diff personally, then independently re-ran the suite — `+375 ~2 All tests passed`, matching the agent's report exactly. | B25 Book Club / Camera Club / Platform Social walkthroughs; root cause `4931867a`; fix `3ad79072` | 2026-09-04 |
| ✅ Closed (3/5 fixed live, 2/5 accepted as-is — user decision 2026-09-04) | B25 multi-persona write extension | **4 communities' B25 rows are unreachable with a single fresh persona — extend the rig to seed instances / drive real multi-persona write sequences to close them.** **Cedar dues CLOSED live 2026-09-04** (no Skill/JSON change, no platform service): package read first revealed the "zero seeded instances" premise was wrong (2 seeds exist, addressed to an unreachable seed fan id — same shape as Camera Club below) and that the package itself documents a real completion path (`record-offline-payment` reaches `paid` directly from `due`, bypassing the `NEEDS IMPLEMENTATION` checkout service). Two live personas (Homeowner + Board) drove a genuine create→paid sequence, submitted for real, verified visible to both sides. Same-shape analysis suggests Camera Club's Critique (create is available to both Member and Organizer — the old blocker was about unreachable SEEDED critiques, not about creating new ones) and Chess Club's disputes (a seeded match sits in a disputable state but its participants aren't sign-in-able; a fresh persona could dispute their OWN newly-created match instead) are likely closeable the same way — in progress. **Book Club/Garden Club Messages — RESOLVED, no action (user decision 2026-09-04): "Messages is a system tab that is not implemented yet. So you can ignore these tabs and keep them as it's."** Messages is a platform-level system tab, not a per-community capability communities declare — the same finding held for Export and Migration's Messages tab too ("injected by the app shell, not declared by the package"). Not a community gap to close; leave as-is. This closes the item: 3 of 5 original isolated gaps fixed via genuine live writes, the remaining 2 explicitly accepted as a known platform-level incompleteness, not a defect. Across the full 10-community grind, every "empty surface" resolved to correct role-gated or identity-scoped behavior EXCEPT five isolated, unseeded workflow gaps: **Cedar Commons HOA** — `hoa-dues-payment` has zero seeds (`seedDataFiles: []`, "Declared for proof, never seeded"), its create is board-only (`byRoleIds: ["hoa-board"]`), and payment processing is flagged `NEEDS IMPLEMENTATION (platform service)`. **Neighborhood Book Club** — Messages has no writer, no seed, and is empty under both Member and Organizer. **Chess Club** — result disputes are promised by tab blurbs but have no seed and no create control under Organizer, Player, or Owner. **Camera Club** — Critique has 2 seeded instances but both are owned by the un-sign-in-able `camera-club-member` fan id (identity-scoped, unreachable by any persona this build can create), and Messages is unseeded (0 `message` occurrences in the package, confirmed against a working cross-community control). **Garden Club** — Messages, same shape as Book Club. None of these are rig failures or architecture problems — they are package-completeness gaps. Resolution needs one of: (a) seed a reachable instance via the community-authoring Skill, (b) a walkthrough that creates its own board/organizer-role persona and DRIVES a real create-to-terminal-state sequence (not cancel-without-submit) to populate the surface, or (c) explicit acceptance as known-incomplete pending a platform service (Cedar's payment processing). Evidence: `docs/Build Plan V2/Evidence/B25/{cedar-commons-hoa,book-club,chess-club,camera-club,garden-club}-agent-walkthrough-2026-09-0{3,4}.md`. | full B25 grind, all 10 communities | 2026-09-04 |
...
- [x] `new-ticket` — **ANSWERED 2026-09-06, no ticket needed: the local engine is deliberately the ephemeral demo shell, and the remote engine already is its replacement in the real app.** `part25_engine_native_community_store.dart:227`'s `WorkflowDatabase.memory()` is only ever actually used when `main.dart`'s `configureLoomRemoteServicesFromEnvironment()` returns null (i.e. `--dart-define=LOOM_ENV=local`, an explicit dev-only opt-in) -- with the real default (`LOOM_ENV=dev`), `main()` calls `configureEngineNativeCommunityEngineFactoryForProduction(createRemoteEngineNativeCommunityEngineFactoryForConfiguration(...))` at startup, which overrides the engine factory to the remote, Postgres-backed `workflow-service` before any community store is installed. So in the real app, loans/RSVPs/custody/due-dates persist server-side, not in-memory, and this was already independently confirmed by the "server-authoritative default" verification elsewhere in this tracker (2026-09-03) -- this row was simply never closed to match. `part02_tab_shell.dart:754`'s in-memory database is unrelated: it backs only the not-yet-implemented Messages tab's `_MessagesEngineStore`. `WorkflowDatabase.file(path)` remaining uncalled is therefore correct, not a gap -- on-device file persistence is for the deliberate local/offline dev path only.
- [x] `needs-verification` — **Same correction applies: this concern is about the local-only dev path (`LOOM_ENV=local`), not the real app.** Two members holding private in-memory copies is real for that explicit opt-in build, but the real app defaults to the shared, server-authoritative remote engine, so multi-party lending/queueing/RSVP-capacity correctness is not degraded there. Any B25 walkthrough run against the default build already exercises the multi-party-correct path.
