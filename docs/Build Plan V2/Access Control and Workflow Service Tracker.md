...
| 🔴 Open | `needs-decision` | **MAJOR: community seed data (`workflowInstances` declared in every package) never reaches the remote backend — confirmed structurally, not per-community.** `_initialize()` in `part25_engine_native_community_store.dart:290` has `if (local is! LocalWorkflowEngineApi) return;` — seeding only ever runs against the local engine. `RemoteWorkflowEngineApi` has no `seedInstances` method; `workflow_service.dart` has no seed/install endpoint. Every community, opened for real against the deployed cluster, starts genuinely empty regardless of its JSON's seed content — confirmed live for Cedar (4 rows total in the DB, all either created live or created by me manually; zero from the package's seed block). **This resolves the open "why did those 16 walkthroughs show populated seed data" question from the earlier Postgres-outage finding**: seeing seed content is definitionally a local-engine signature, independent of whether the backend is healthy. Full writeup, blocked work, and three open questions (is this a gap or intentional, where would a fix belong, does it outrank the smaller tickets it blocks): [root-cause-seed-instances-never-reach-remote.md](Evidence/B25/root-cause-seed-instances-never-reach-remote.md). Not fixed, not decided — found while scoping the Cedar document-content ticket, which this blocks. | autonomous backend loop, this session | 2026-09-07 |
| 🔴 Open | `needs-decision` | **UPDATED, bigger than first reported: all 16 live-verification dispatches spanning 2026-09-03 evening to 2026-09-05 morning correspond to ZERO rows in the live database today** (the entire `workflow_instances` table holds only 3 pre-existing Cedar rows from 2026-08-26). The original, most rigorously-checked finding — 3 specific rows (Cedar dues payment, Camera Club critique, Chess Club dispute) whose claimed server timestamps fall inside a confirmed ~25-hour Postgres outage — still stands and is the best-evidenced subset, but is a lower bound, not the full extent. Full evidence, corrected timeline (an earlier "whole-VM reboot" theory in this doc was checked and retracted — it was actually one pod, `postgres-0`, unresponsive for ~25 hours while the VM and k3s stayed healthy), and four open questions: [root-cause-postgres-outage-live-write-timestamps-2026-09-06.md](Evidence/B25/root-cause-postgres-outage-live-write-timestamps-2026-09-06.md). Not closed, not reopened, not acted on. Found by accident while scoping an unrelated ticket (Cedar document-content upload) — worth noting because it means this class of gap is easy to miss unless someone specifically checks a claimed write against the database rather than the rendered UI. | autonomous backend loop, this session | 2026-09-06 |
