...
| ⬜ Open | `new-ticket` | **11 app-shell test failures** — baseline `+227 -11`, unchanged with the deletions applied *and* reverted. **Correction 2026-08-14: these are not "unrelated to this effort", as this row originally claimed.** Decomposed by running them: **7** are the unimplemented eager response-row fan-out (`v3_milestone_a11_event_rsvp_archetype_test.dart` ×6, plus `v3_milestone_calr2g_live_package_test.dart`) — the row above; **2** are a genuine Garden Club tab-derivation defect (`cjm8`), where the fixture declares `role: "any"` with `byPersonaIds: [garden-member, garden-coordinator]` and the product doc's persona table lists Marketplace for Member, yet `appShellTabsFor` drops the tab for `garden-member` and a seeded `home` instance fails to render for `garden-coordinator` — **do not "fix" these by relaxing the assertions**, the fixture and product doc are the specification and weakening the test would convert a product-doc violation into a silent one; **2** remain unexamined (`cjm9` hydration, `phasee` purchase proposal, the latter already tracked separately). One genuinely stale reference was found and fixed (`bab66564`): `154493e6` renamed `mulch-day-shift` → `mulch-delivery-shift` without updating the test — but that did **not** turn the test green, so the defect stands. Nine of eleven are this effort's own subject matter being half-built, not a prerequisite chore. | independent verification during cleanup; decomposed 2026-08-14 | 2026-08-14 |
