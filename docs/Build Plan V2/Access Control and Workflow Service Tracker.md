...
| 🔵 Queued | Shell UI bug | **The role radio in "Account role and permissions" silently no-ops while signed in — every community.** Found in the Book Club and Camera Club B25 walkthroughs, root-caused during Platform Social (reproduced 3x). Selecting a different role in the identity picker dismisses the dialog with no error and no visible change; the active role never moves off the signed-in account's own role. Root cause: `part01_local_extension_screen.dart:221-224` — `_activeRoleId` reads `session?.account.roleId ?? _selectedRoleId`, and the account's own role always wins whenever a session exists, which is every path that can reach this dialog (the community entry gate requires sign-in first). The picker's `onTap` does write `_selectedRoleId` (line 1183), but that state is unreachable. The radios render enabled with no disabled state and no explanatory text, so the control advertises an effect it cannot have. The only real route to a different role is "Sign in as a specific person…" → Create New Account → Role dropdown. Fix: either make the picker actually switch the active role (if that's a supported concept for this app's single-role-per-account model), or disable/relabel the control so it stops claiming a capability that doesn't exist. | B25 Book Club / Camera Club / Platform Social walkthroughs | 2026-09-04 |
