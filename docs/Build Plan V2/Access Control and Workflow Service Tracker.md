...
|---|---|---|---|---|
| ⚠️ Design conflict (user) | `needs-decision` | **A 2026-08-14 decision ("event-rsvp response rows become canonical") DIRECTLY CONTRADICTS solved-pattern #24 (2026-09-03).** Pattern 24 forbids per-response `*-response` state machines (responses are DATA lists on the event/ballot); the older decision made response *rows* canonical. This older decision is the likely root of the masjid/book-club `*-response` regen breakage. **Reconcile before the next regen pass:** which stands — responses-as-data (pattern 24, the shipped packages' model) or response-rows-canonical (the 2026-08-14 milestone)? The shipped packages use responses-as-data, so pattern 24 is consistent with production; the 2026-08-14 milestone row (§8) should likely be retired. | 2026-09-03 |
