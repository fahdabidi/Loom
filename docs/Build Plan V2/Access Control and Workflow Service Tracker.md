...
| 🟡 Pre-existing (not Thread 1/2) | `new-ticket` | **2 UX-judge + 1 app-shell test failures predate batch v4 and are unrelated to Threads 1-2** (whose dirty files never touched those packages). (a) UX judges: locked provenance mismatch for the **AdFree `.jsonc`** — expected SHA `fa6b66…`/82,521 B vs actual `513e03…`/87,980 B (the ad-free regen was adopted but the locked checksum/reference wasn't reconciled). (b) app-shell: B25 interaction-model **asset conformance** differs at offset 23,493. Neither dispatch modified any assertion, locked Markdown, or community JSON to mask these. Investigate + reconcile separately from the architectural threads. | 2026-09-03 |
