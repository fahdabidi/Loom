...
- **Current v4 pass:** `b25-v4-pass-1` has collected schema v4 evidence through
  ...
  workflow/persona scorecards are not complete. Pass 1 closes with schema v2 tickets that include
  affected screen/coverage IDs, screenshot paths and hashes, visible text excerpts, current surface
  classifications, exact UX failures, target production surfaces, likely files/widgets, and concrete
  screen/workflow/persona acceptance criteria. The next worker iteration must start from
  `docs/Build Plan V2/Evidence/B25/b25-remediation-plan-b25-v4-pass-2.md`, which consumes the
  committed pass-1 tickets. B25 remains reopened. Iteration commit: `647c38f`; pass-result
  clarification commit `c5799e6`; detailed ticket schema update `f617625`.
- **Commit:** Historical iteration 3 implementation `ccc3f40`; v4 pass-1 ticket/planner closeout
  `5d4e313`; detailed ticket schema update `f617625`.
