...
| B25 | `TBD` | Reopened | Product-UX v4 pass 29 failed with 1 blocker, 10 unresolved major findings, 5 major Product Docs reconciliation findings, 1 blocker + 4 major LLM Vision findings, 4 failing lifecycle scorecards, and 9 remediation tickets. Pass 30 must start from `b25-remediation-plan-b25-v4-pass-30.md`, replace remaining generic action/review checklist surfaces, route non-payment flows away from payment-summary containers, build export/portability workspaces, make persona handoffs production-grade, reduce repeated-card visual fatigue, and add durable result states for donor visibility, book vote, photo walk RSVP, and message stream before another full B12-B20 recapture. |
