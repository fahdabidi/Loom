...
|---|---|---|---|---|
| ⬜ Open | `needs-live-validation` | **The `deliver_reminder` path is UNPROVEN, not broken — correcting my own finding from earlier today.** I recorded this as a defect ("the sweep returns nothing when every input says it should not") and that was wrong. Reading `dueNotifications` in `local_workflow_engine_api.dart` shows Cedar derives its reminder as `if(reminderEnabled == true, subtractHours(combineDateAndTime(eventDate, eventTime), 24), null)` — and **all three instances have no `reminderEnabled` field at all**, so the formula yields null, there is no `dueAt`, and `items: []` is the correct answer. They are leftover probe instances ("Gate probe", "live chain probe") from earlier endpoint testing that never enabled a reminder. **What IS true**: the query API is live and correct (200; `400 invalid_as_of` on a bad `asOf`), the stored definitions are current (6 carry a reminder key, matching the 6 shipped packages), and the sweep handles both the computed and stored `dueAt` shapes deliberately. **What is still unproven**: no instance anywhere has ever had a reminder scheduled, so the create→sweep→deliver chain has never run end to end. Proving it needs an instance with `reminderEnabled: true` and an anchor inside the lead window — not a bug fix. | corrected after reading the implementation | 2026-08-31 |
