...
|---|---|---|---|---|
| ⬜ Open | `needs-debug-agent` | **Instance creation is refused for a fan who holds the granting role — narrowed 2026-08-31, cause still open.** `POST /instances` returns **403 `workflow_create_refused`** for `fan-hoa-member-1`. Four candidate causes ruled out by measurement: **(1) the group mapping is correct** — `LOOM_COMMUNITY_GROUP_IDS` in `workflow-service-config` maps `community_cedar_commons_hoa` → `loom_communities_cedar-commons-hoa`, the hyphenated group where the membership actually lives; **(2) the membership exists** — `fan-hoa-member-1` holds `hoa-member` there; **(3) the role grants create** — `hoa-member` has **20** permissions including `event_rsvp.create`; **(4) the catalog is populated** — 13 create permissions of 127. Remaining hypothesis, untested: `hoa-facility-reservation` resolves to an archetype whose `.create` permission `hoa-member` does **not** hold — it has `event_rsvp.create` and `form_entry.create` but **not** `calendar.create`, and `hoa-board` holds five including `document_library.create` and `export_wizard.create`. **CORRECTION to my own note last tick**: I wrote that no role↔permission table exists and permissions are derived at runtime. Wrong — `role_permission` is a real table; I had truncated `dt` with `head -14` and it was row 14. `AppAccessService.checkAccess` queries it through `rolePermissionRepository`. | traced live | 2026-08-31 |
