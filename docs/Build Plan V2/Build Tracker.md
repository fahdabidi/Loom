...
|---|---|---|---|---|
| ⬜ Open | `needs-decision` | **The whole `calendar.*` fix is now PREPARED AND DRY-RUN — one go-ahead executes it.** Done and verified: vocabulary synced (`ac4b881`); `loom/app-access:0.3.4` built and confirmed to carry **106 ids incl. all 9 `calendar.*`** by extracting `BOOT-INF/classes/permissions-vocabulary.json` from the jar. **The re-install is not hand-rolled** — `loom_app_access_provisioning` ships `derive_app_access_provisioning_plan.dart` (read-only) and `apply_app_access_provisioning.dart`, which is **dry-run unless `--apply` is explicit**. Plan derived from the shipped assets: **10 communities, 82 workflows** — matching the 82 stored definitions exactly — with `calendar` appearing **once**, as expected for `hoa-facility-reservation`. Dry-run apply confirms it *"will POST … No groups, roles, permissions, or catalog"* written, and the payload carries `hoa-facility-reservation` with `cardSurfaceFamily: "calendar"` and `createRoleIds: ["hoa-board", …]` — so applying grants exactly the missing permission to exactly the roles each package declares. **Remaining, needing a go-ahead**: bump the manifest to `0.3.4`, roll out, **commit the bump**, then `apply_app_access_provisioning.dart /tmp/plan_clean.json --apply`. **Gotcha for whoever runs it**: `dart run` prepends `Running build hooks...` to stdout, so a redirected plan file is **invalid JSON** until stripped — the plan must be cleaned before the apply will parse it. | prepared and dry-run | 2026-08-31 |
