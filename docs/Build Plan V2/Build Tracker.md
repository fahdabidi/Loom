...
|---|---|---|---|---|
| ⬜ Open | `needs-debug-agent` | **The `deliver_reminder` path does NOT work end to end: every input is present and the sweep still returns nothing.** Asked 2026-08-31 to confirm the event-rsvp backend is live before relying on it, and the answer is half yes. **Live and correct:** `GET /v1/communities/{id}/notifications/due?asOf=` answers **200**, rejects a non-ISO-8601 `asOf` with `400 invalid_as_of`, and is implemented at `workflow_service.dart` `_dueNotifications` → `engine.dueNotifications(asOf:)`. **But it returns `items: []` when it should not.** Every input is in place: Cedar's shipped `hoa-facility-reservation` declares `"reminder": {anchorDateField: eventDate, anchorTimeField: eventTime, …}`; the **stored** definition has it too (6 stored definitions carry a reminder key, matching the 6 shipped packages that declare one — so publishing is current, not stale); and all 3 instances in the service are that workflow with `eventDate` 2026-09-15 / 2026-10-01 and `eventTime` populated. Queried at `asOf` **+365 days** — far past any lead time — and still empty. **So scheduling produces nothing to deliver, and `deliver_reminder` has no working scheduler behind it.** Cause not yet determined: state gating (`open`/`reserved`), an anchor-resolution bug, or the sweep's query. **This is the automatic half of the reminder decision taken today** — the manual `send_reminder` path is a plain role-gated transition and is unaffected. | measured live | 2026-08-31 |
