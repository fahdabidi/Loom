...
| B25 | Reopened | B24 | [Independent Production UX Review](./Phases/Phase%20B25%20-%20Independent%20Production%20UX%20Review.md) | A post-implementation outside-in product UX review critiques the actual app experience, rejects exposed workflow machinery, requires fresh screenshot-backed schema v4 evidence, and only passes when primary workflows use domain-native production surfaces. | `b25-v4-pass-2` ran the full B25 chain after consuming the pass-1 remediation plan. It fixed the broad evidence-persona problem by excluding the non-production B12 harness, inferring concrete personas/personaIds for every reviewed row, and recording manual visible-text review source for 196 screen rows. Pass 2 still failed: 0 blockers, 3 unresolved majors, 7 blocking criteria failures, failed holistic direct-question pass, and failed workflow/persona direct-question pass. The remaining coverage collector gap is now 6 workflow/persona rows missing complete entry/action/result screenshots, and all primary surfaces remain unverified as domain-native by the independent judge. Current pass closeout artifacts are pass-2 evidence, workflow/persona coverage matrix, independent judge output, production judge scorecard, schema v4 remediation tickets, pass-2 scorecard, and pass-3 remediation plan. Required next: start `b25-v4-pass-3` from `b25-remediation-plan-b25-v4-pass-3.md`, repair the 6 remaining coverage rows, then remediate the unverified/domain-native UI surfaces using the ticket reference patterns before recapturing and rerunning judges. | Historical v3 implementation `ccc3f40`; v4 pass 1 evidence `647c38f`; pass-result clarification `c5799e6`; ticket/planner closeout `5d4e313`; coverage/judge-tool update `e46cbaa`; detailed ticket schema update `f617625`; work-item split update `42e7cdf`; reference-pattern ticket update `6d01a22`; v4 pass 2 pending commit |
...
| B25 | Reviews and certifies the finished UX independently. | Outside-in product UX review, schema v4 evidence, screenshot freshness audit, non-boilerplate screen critique, domain-native primary-surface gate, holistic direct-question scorecard, workflow/persona direct-question scorecards, production UX judge scorecard, detailed remediation tickets, iteration scorecards, remediation evidence, final pass/fail decision; failed-pass tickets feed the next pass's remediation planner. | The UX passes only when an independent screenshot-first review and the production UX judge find no unresolved blocker or major design issues, every screenshot is fresh and traceable, every primary workflow is domain-native, the holistic and workflow/persona direct-question passes are green, every failed pass has template-complete remediation tickets and a convergence scorecard, and any minor issues are accepted or tracked. A remediation pass may not begin implementation until the prior pass's tickets have been converted into a remediation plan. |
...
B25 must also run the Production UX Judge as two direct-question passes: one holistic product UX pass
for the whole app/community experience and one workflow/persona pass for every reviewed
workflow/persona pair. Both must be green. The holistic pass guards overall product coherence,
navigation, modern visual quality, and community-centered information architecture. The workflow/persona
passes guard task clarity, domain-native surfaces, natural actions, validation/result states, receiver
states, and unauthorized/read-only behavior.
...
| B25 | `b25_evidence_collector.dart`; `production_ux_judge.dart`; `b25_iteration_scorecard.dart`; next-pass `b25_remediation_planner.dart` | Current pass: `independent-production-ux-review.json/.md`, `product-ux-screen-review-matrix.md`, `production-ux-criteria-scorecard.json/.md`, `b25-remediation-tickets-<run-id>.json/.md`, `b25-iteration-scorecard-*.json/.md`, plus `holisticQuestionAnswers` and `workflowPersonaScorecards` in review JSON. Next pass kickoff: `b25-remediation-plan-<prior-run-id>.json/.md`. | Evidence was not collected by the deterministic collector, any B25 pass criterion has a blocking failure without a template-complete remediation ticket, either direct-question pass is missing/partial/unsupported, any primary workflow surface is not domain-native, or the iteration scorecard is missing for the pass. A remediation pass fails before implementation if the prior pass's tickets were not sent to the planner. |
...
| B25 | None | Reopened | Prior product-UX v3 pass superseded by v4 criteria requiring per-community production UX blueprint, schema v4 evidence, screenshot freshness, visible-text extraction, non-boilerplate critique, domain-native primary workflow surfaces, holistic and workflow/persona direct-question passes, production UX judge scorecard, and per-pass iteration scorecards. |
...
- **Deliverables:** Per-community production UX blueprint, independent UX review report, complete
  ...
  workflow/persona direct-question scorecards, production UX judge scorecard, B25 iteration scorecards,
  B25 remediation tickets, remediation loop log, severity-ranked findings, annotated screenshot references, resolved-finding
...
- **Completed when:** The reviewer has walked the actual app in the visible Android emulator across all
  ...
  any workflow/persona direct-question pass is missing or not green, or if `production_ux_judge.dart`
  produces any blocking criterion failure. Each review/remediation pass must produce a B25 iteration
  scorecard before the pass is committed or the next remediation loop starts.
...
- **Evidence to record:** Independent review report, product UX screen review matrix, findings table,
  ...
  workflow/persona direct-question scorecards, production UX judge scorecard, B25 iteration scorecards,
  B25 remediation tickets, annotated screenshots or screenshot paths, remediation diffs/evidence, retest output for fixed
  findings, final pass/fail statement, manifest/test stamps, B25 phase gate, analyze, boundary lint,
...
- **Gate evidence:** Historical v3 checks passed:
  ...
  pending.
- **Current v4 pass:** `b25-v4-pass-2` has collected schema v4 evidence through
  ...
  `docs/Build Plan V2/Evidence/B25/b25-iteration-scorecard-b25-v4-pass-2.md`, and produced the next
  remediation plan at
  `docs/Build Plan V2/Evidence/B25/b25-remediation-plan-b25-v4-pass-3.md`. The pass failed with
  ...
  Pass 2 fixed the broad evidence-persona gap by excluding the non-production B12 evidence harness,
  assigning concrete personas/personaIds to every reviewed screen and coverage row, and recording
  manual visible-text review source for 196 screen rows. The remaining unresolved major findings are
  `B25-WORKFLOW-PERSONA-COVERAGE-INCOMPLETE`, `B25-WORKFLOW-PERSONA-UX-FAILED`, and
  `B25-HOLISTIC-UX-FAILED`. The coverage gap is now specific: 6 workflow/persona rows are missing full
  entry/action/result screenshot coverage. The workflow/persona direct-question pass is still false
  ...
  start from `b25-remediation-plan-b25-v4-pass-3.md`, repair the 6 coverage rows first, then implement
  domain-native UI/surface fixes from the pass-2 tickets before recapturing evidence and rerunning the
  full B25 chain.
- **Commit:** Historical iteration 3 implementation `ccc3f40`; v4 pass-1 ticket/planner closeout
  ...
  reference-pattern ticket update `6d01a22`; v4 pass-2 pending commit.
