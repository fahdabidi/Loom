...
|---|---|---|---|---|
| ✅ Closed | `new-ticket` | **Item 1l closed and the permission-replacement danger with it (`04faa1bf`, `555ed140`).** Docs first, since §4 is normative: `permissions.md` gained a `create` row for `equipment-loan`, `documentLibrary`, `exportWizard` and `searchAiAnswer`, then the resolver gained `'create'` for each — vocabulary **106 → 110**. A follow-on fixed `send_reminder`, which I had added to the docs on 2026-08-31 without the resolver, leaving the judges suite **red for three ticks** until I ran it; `archetype_resolver_spec_sync_test.dart` caught exactly what it exists to catch. Vocabulary now 111. **Consequence that matters**: every permission any role holds is now derivable except the five `community.*` governance ids, which correctly are not archetype-derived — so an install would no longer strip permissions. **All five suites re-measured by me**: judges **485** (0 skipped), app shell **371** (+2), engine 312 (+5), workflow service **148** (+1, *with* PG credentials — uncredentialed it reads 139 (+10) and looks green while skipping the integration tests), demo 160. CLAUDE.md's baseline table updated. | verified independently | 2026-09-01 |
| ✅ Closed (swept 2026-09-03; **SUPERSEDED IN PART 2026-09-09 — read the update at the end of this cell before acting on the warning**) | `needs-decision` | **🛑 DO NOT RUN `apply_app_access_provisioning --apply`. It would DELETE 16 roles, including all 11 community admin roles.** Found 2026-09-01 while verifying the calendar fix. `installCommunityPackage` does not merely reset permissions — it **deletes every group-scoped role the package does not declare**: `rolesBefore.filter(!PLATFORM_ADMIN_ROLE_ID.equals(...)).filter(!declaredRoles.containsKey(...))` → `setRolePermissions(..., List.of())` then `appRoleRepository.delete(removedRole)`. Only the single reserved platform-admin id is spared. **SUPERSEDED IN PART, 2026-09-09, by a live probe rather than another code read — and the half that is wrong is the headline.** The generated community admin role is **no longer at risk**: install now spares `<handle>-admin` and re-grants it exactly the five `community.*` permissions, because Part A made those grants archetype-derivable. Proven against a disposable handle (`zz-install-probe`): installed a minimal package, added an UNDECLARED role holding `community.view` as a canary, re-installed, and the response said it outright — `{"rolesRegistered":["zz-member"],"removedRoleIds":["zz-canary"]}` — with the generated admin still holding exactly 5. Probe roles deleted afterwards and `check_role_parity.sh` re-run clean. **The half that still stands:** the sweep really does delete every group-scoped role the package does not declare, holders or not, bypassing `deleteRole`'s holder protections. **So the rule is conditional, which no code read had settled: install is safe when the package declares every role that ought to exist in that group, and destructive otherwise.** The live corpus now satisfies that — `check_role_parity.sh` reports zero declared-but-unprovisioned roles, and the only live-but-undeclared roles are the underscored `cedar_commons_hoa` duplicate and the `tabletop-club` fixture. The danger was never the admin; it is any hand-provisioned role a package has forgotten. See the settled row in `TODO.md`. ORIGINAL MEASUREMENT FOLLOWS: **Measured**: packages declare **23** roles; **39** group-scoped roles exist; **16 would be destroyed** — `ad-off-admin`, `book-admin`, `camera-club-admin`, `chess-admin`, `garden-admin`, `hoa-admin`, `portability-admin`, `soccer-admin`, `social-admin`, `cedar_commons_hoa_admin`, `cedar_commons_hoa_member`, the two `b3-event-creator-*` e2e artifacts, and three `tabletop-*` roles. **That is the entire admin-role provisioning of 2026-08-30**, which took 11 roles and 35 accounts through the real approve flow. Also unrecoverable by re-running: the five `community.*` governance permissions are **not** in the generated vocabulary (correctly — they are not archetype-derived), so a re-install cannot restore what it removed. **The dry run does not reveal this**: it prints the requests it *would POST*, not what the server deletes on receipt. **The real question**: admin roles are created outside the package but live in the package's group, and install treats "not declared" as "remove". Options — declare admin roles in the packages; give them a reserved id like the platform admin; or scope deletion to roles the installer itself created. — **CLOSED 2026-09-03:** addressed: install creates+spares system admin; migration done safely | caught pre-execution | 2026-09-01 |
| ✅ Closed (swept 2026-09-03) | `new-ticket` | **DO NOT APPLY THE PROVISIONING PLAN YET — it would revoke working permissions. Caught 2026-09-01 before recommending it.** The apply is **not additive**: `installCommunityPackage` calls `SetRolePermissionsRequest`, which at `AppAccessService:1027` does `deleteById_AppIdAndId_RoleId` **then** saves the derived set — so each declared role's permissions are **replaced**. And the deriver can only grant what its vocabulary names. **`hoa-board` holds `document_library.create` and `export_wizard.create` today; the generated vocabulary contains neither** (0 for both), so applying would drop them. This is exactly the "deleting to match the generated set would revoke working ability" risk I flagged, arriving through a path I had judged safe. **The real blocker is item 1l**: the generator omits `.create` for **four** bespoke archetypes — `document_library`, `equipment_loan`, `export_wizard`, `search_ai_answer` — verified by checking `.create` presence per family across the vocabulary. **Corrected sequence**: (1) 1l's stated blocker, the `permissions.md` §4/§6 contradiction, **was resolved today** by adding `send_reminder`, so 1l is now unblocked; (2) make the generator emit `.create` for those four; (3) regenerate, sync, rebuild, deploy; (4) **then** apply, when the derived set is a superset of what roles already hold. Everything already built stays valid — `0.3.4` and the plan simply come after step 2. — **CLOSED 2026-09-03:** addressed: install non-destructive for admin; canonical admin migration done | caught pre-execution | 2026-09-01 |
| ✅ Closed (swept 2026-09-03) | `needs-live-validation` | **`loom/app-access:0.3.4` is BUILT and verified to carry the calendar family — deploy and package re-install await a go-ahead.** Built from the synced vocabulary (`ac4b881`). **Verified inside the image, not inferred**: extracted `BOOT-INF/classes/permissions-vocabulary.json` from the jar — **106 ids including all 9 `calendar.*`**, against deployed `0.3.3`'s 97 and zero. Cluster unharmed by the build: 6/6 pods, load 3.10, and all four services still *serving* (app-access 401, fan-passport 401, workflow-service 404, keycloak realm 200) rather than merely Running. **Remaining, and deliberately not done**: bump `deploy/k8s/app-access.yaml` to `0.3.4`, roll out, **commit the manifest bump** (it has drifted twice), then re-install the community packages so `CommunityPermissionDeriver` grants `calendar.*` per each package's declared roles. Only that last step changes who can act. — **CLOSED 2026-09-03:** superseded: app-access now 0.3.8 deployed | built and verified | 2026-08-31 |
| ✅ Closed (swept 2026-09-03) | `new-ticket` | **The `calendar.*` gap is a THIRD twin-file drift, and the fix is a normal deploy — not hand-written grants.** `CommunityPermissionDeriver` loads `permissions-vocabulary.json` **from the classpath**, so app-access uses whatever copy was bundled at build time. The backend's copy at `services/app-access/src/main/resources/permissions-vocabulary.json` holds **97 ids and zero `calendar.*`**; Loom's generated artifact holds **106** including all nine. That 97 is exactly the "shared" count measured against the deployed catalog, which confirms the deployed state is simply the stale copy plus 30 separately-seeded ids. **Same shape as the OpenAPI spec twins**: a generated artifact living in two repos with nothing comparing them — `check_spec_parity.sh` covers the specs and nothing covers this. **The fix is therefore the designed path, not bespoke SQL**: sync the vocabulary Loom → backend, rebuild and redeploy app-access, then re-install the community packages so `CommunityPermissionDeriver.derive()` assigns `calendar.*` to the roles each package declares. My earlier proposal to insert 9 rows and hand-grant them would have worked around the mechanism instead of using it, and would have drifted again on the next install. **Still needs a go-ahead** — re-installing packages rewrites `role_permission` — but it is a routine deploy rather than a bespoke auth mutation. **Follow-on**: extend parity checking to this artifact, since it has now drifted exactly as the specs did. — **CLOSED 2026-09-03:** fixed: vocabulary twin synced + rebuilt into image | root-caused | 2026-08-31 |
| ⬜ Open (**MY RE-CHECK BELOW WAS WRONG — read the 403 correction at the end of this cell**) | `needs-decision` |**RE-CHECKED 2026-09-09 — half of this row is stale and the other half needs a different question asked.** **Stale half:** the re-open note says "deployed catalog has `community.*` but **0** `calendar.*` (DB-verified)". Live today, all **nine** are deployed — `calendar.cancel`, `.create`, `.deliver_reminder`, `.edit`, `.propose_change`, `.record_outcome`, `.reopen`, `.set_reminder`, `.view` — in a 137-id catalog. The additive insert this row proposed **was done**. **Live half:** **zero roles hold any of them**, while sibling archetypes hold 72 `.create` grants between them, so the per-role grant step described here was deliberately never applied — consistent with this row gating it on an explicit go-ahead. **But the framing "the catalog gap is the whole of what blocks creation" does not survive checking, and that changes what the go-ahead is even for.** Verified by reading the code, not assuming: `roleHasPermission` (`part12_actor_identity_and_tabs.dart:567`) is **entirely package-derived** — it walks the experience definition's bound workflows, their `readGuard`s and their transition `allowedRoleIds` — and **never reads App Access `role_permission` rows at all**. `part25:477` names it "the app-shell-owned `roleHasPermission` policy". The workflow service likewise authorizes through the engine's guards (`_mayAct`), and a grep finds no `role_permission`/`permissionIds` consultation in it. **So an ungranted `calendar.*` does not, on this evidence, stop anyone creating a calendar workflow in the app.** That also matches the earlier finding that `portability-member` correctly holds zero permissions because it appears only in `readGuard`s. **The open question this leaves, which should be answered before any grant is applied:** if nothing in the app or the service enforces these grants, what does consume them — and was the original "every calendar-archetype workflow was uncreatable by anyone" symptom actually an *install-time derivation failure* (the deriver rejecting an id it had never heard of, so a community's roles never provisioned at all) rather than a runtime permission check? Those are different faults with the same description, and the catalog insert would have fixed the first while the grants address neither. **Do not apply the grants as a creation fix until that is settled** — it would be treating a bookkeeping asymmetry as an authorization gap. **MY 2026-09-09 RE-CHECK WAS WRONG, AND A LIVE 403 PROVED IT THE SAME DAY. The `calendar.*` grants ARE the creation blocker, exactly as this row originally said.** I wrote above that "an ungranted `calendar.*` does not, on this evidence, stop anyone creating a calendar workflow", on the grounds that `roleHasPermission` is package-derived and never reads `role_permission`. **That reasoning checked the wrong layer.** `roleHasPermission` is the app shell's own **tab-visibility** policy. Creation is authorized somewhere else entirely: `workflow_service.dart:572` resolves `permissionId = resolver.permissionId(family, 'create')` for the workflow's archetype family, calls `_appAccessClient.checkAccess(fanId, appId, permissionId, groupId)` at `:592`, and `if (!allowed) return _createRefused(request)`. **So the service gates creation on the App Access grant, and with zero `calendar.*` grants every calendar-archetype workflow is uncreatable by anyone — the original 2026-08-31 claim, verbatim.** **The proof is live, not a code read:** the Cedar `hoa-facility-reservation` walkthrough on 2026-09-09 filled the "Reserve a facility" form and got **HTTP 403** (`status=403 error=http_403`), for `hoa-member` and `hoa-board` alike, with `event_rsvp.create` allowed for both as a control — so roles are correctly provisioned and the grant layer is the gap. **My advice was actively harmful and is withdrawn:** I wrote "do not apply the grants as a creation fix until that is settled". The grants ARE the fix, they were always the fix, and this row's proposed additive change should proceed. **What I would keep from the re-check:** the catalog half really was done (all 9 ids are deployed in the 137-id catalog), so only the per-role grant step remains — and it still changes who can act in the live authorization system, which is why this row gates it on an explicit go-ahead. **The lesson, recorded in CLAUDE.md: a capability has to be true at every layer, and I checked one layer and generalised from it.** `roleHasPermission` and the service's `checkAccess` answer different questions, and I let the first one's answer stand for both.
| ⬜ Open | `new-ticket` | **ROOT-CAUSED 2026-08-31 — the entire `calendar.*` permission family is missing from the deployed catalog, so no `calendar`-archetype workflow can be created by anyone.** Ran `ArchetypeResolver` directly against Cedar's package: `hoa-facility-reservation` resolves to family **`calendar`**, `origin: declaredBespoke`, no conflicting families — so create demands **`calendar.create`**. In the deployed catalog that permission **does not exist** (0 rows), **nobody** holds it, and **no `calendar.*` permission is granted to any role**. The generated vocabulary has all eight (`calendar.create`, `.edit`, `.cancel`, `.reopen`, `.set_reminder`, `.deliver_reminder`, `.propose_change`, `.record_outcome`). **This is a live product blocker, not a harness artifact**: no Cedar member or board member can create a facility reservation. It also blocks the reminder chain twice over, since `calendar.deliver_reminder` is missing too. **This is item 1k made concrete** — deployed catalog out of sync with the generated vocabulary — and the divergence runs both ways: deployed has **127** ids, generated **106**. Fix is to reinstall the catalog from the generated artifact and grant the calendar family to the roles that need it. | root-caused live | 2026-08-31 |
...
| ✅ Closed | `new-ticket` | **ROOT CAUSE, why no authenticated walkthrough has ever succeeded: Keycloak advertises `localhost` in its discovery document.** `loom-backend/deploy/k8s/keycloak.yaml:97` sets `KC_HOSTNAME: "http://localhost:30082"`, so `GET http://192.168.56.10:30082/realms/loom/.well-known/openid-configuration` returns `"authorization_endpoint":"http://localhost:30082/realms/loom/protocol/openid-connect/auth"`. The app uses `openid_client`'s `Flow.authorizationCodeWithPKCE`, which builds the browser URL from the **discovered** endpoint rather than the configured base — so it correctly reaches Keycloak to discover, then sends Chrome to `localhost:30082`, which on the device is the device. **Observed end to end on `emulator-5554`**: Garden Club → entry gate (`LoomAuthNotLoggedInException`, as designed) → "Continue to secure sign-in" → `LoomProductionLoginScreen` → Chrome custom tab showing **`localhost:30082`** and a blank page. Every step of the app's own flow worked; only the advertised hostname is wrong. Fix is `KC_HOSTNAME: "http://192.168.56.10:30082"`, then re-apply and **commit the manifest** — an uncommitted bump has drifted twice. ORIGINAL row left as written above. **STALE ROW, closed 2026-09-07**: this was actually fixed and verified the same day it was found (see the two adjacent rows immediately below/after this one in the file, `79919b2`, `needs-live-validation` closed 2026-08-31) -- the row was simply never flipped. Re-verified live 2026-09-07, independent of the tracker text: `kubectl get deploy keycloak -n loom -o jsonpath='{...KC_HOSTNAME...}'` returns `http://192.168.56.10:30082`; the manifest on disk (`~/loom-backend/deploy/k8s/keycloak.yaml`) matches; the real discovery document's `authorization_endpoint` returns the LAN IP, not `localhost`; `git status` on `loom-backend` is clean (no drift). No action needed -- documentation-only closeout. | driven on device | 2026-08-31 |
...
| ✅ Closed | `needs-debug-agent` | **Garden walkthrough stall — CLOSED 2026-09-09 as stale.** A full live walkthrough completed 2026-09-08 (`Evidence/B25/garden-club-tool-loan-live-write-2026-09-08.md`): real OAuth sign-in as `loom-garden-member-1`, `garden-tool-loan` created from nothing through the UI to `published`, `delist` driven to the terminal state, Postgres row read back after each step and agreeing with the UI. "No escalation was needed." | TODO.md, migrated | 2026-08-31, closed 2026-09-09 |
...
| ✅ Closed (`0da2d948`) | `needs-skill-dispatch` | Chess: strike the 2 duplicate rows (`chess-local-install-open`, `chess-route-home`); keep `chess-match-result` | row-duplication analysis | 2026-08-31 | **Done 2026-09-07**, no Skill dispatch needed -- this was a doc-table edit, not community JSON. Both rows removed from `docs/references/communities/chess-club-product-experience.md`'s B25 addendum table, `chess-match-result` untouched. |
| ✅ Closed (`0da2d948`) | `needs-skill-dispatch` | Social Space: re-point 4 rows at `platform-message-thread` / `platform-connection` with decisions that differ | row-duplication analysis | 2026-08-31 | **Done 2026-09-07**, no Skill dispatch needed. The 4 generic rows in `member-social-space-product-experience.md` merged into 2 real rows, each with distinct decision text and actions verified against the shipped package's actual transition ids. |
| ✅ Closed (`0da2d948`) | `needs-skill-dispatch` | Garden: split `garden-tool-loan-giveaway` into the two shipped workflows | row-duplication analysis | 2026-08-31 | **Done 2026-09-07**, no Skill dispatch needed. `garden-club-product-experience.md`'s row split into `garden-tool-loan` and `garden-tool-giveaway`, affordances verified against the shipped package's real transition ids (loan: join-queue/confirm-pickup/mark-damaged/report-lost; giveaway: claim/pause/resume/delist only). |
...
- [ ] `new-ticket` — **`CommunityExportFake` still returns its fabricated checksum string.** Nothing in the app reaches it, but it backs `loom_fake_backend` tests. Retiring it belongs with the wider fake-retirement work in step 3, not here **RE-VERIFIED 2026-09-01: still present, and confirmed test-only.** The string is live at `community_ops_fake.dart:349` inside `CommunityExportFake` (lines 307–356): `'checksum_${communityId}_${documents.length}_${redactProtectedData ? 'r' : 'full'}'` — the exact canned form solved-patterns cites as the archetypal fabrication. **Reachability traced rather than assumed**: `CommunityOpsServicesFakeBackend` constructs it, and that aggregator's only users are four *test* files (`loom_communities_demo/test/workflow_test_harness.dart` plus three in `loom_fake_backend/test/`). No app-code path reaches it — `loom_demo/lib/main.dart` registers `MigrationExportFake`, a different class. So this is a test fixture that looks like a checksum, not a live fabrication, and the severity is correspondingly lower than the wording suggests. It still wants removing, because a fixture that mints plausible-looking evidence is what trains a reader to trust the shape.
...
- [ ] `new-ticket` — **the alternate leg proves an ENGINE postcondition, never a VISIBLE one.** `_expectShippedInstanceState` / `_expectShippedInstanceDataChanged` both ran and genuinely passed for the two rows above, so `b25ActionProofStatus: pass` was emitted while nothing a person could see had changed. This is the gap that let a false pass through, and it is deliberately a SEPARATE ticket from the rendering fix — the product and the definition of proof-of-the-product must not move in one dispatch. Require a visible postcondition after the engine one: for `stateChanging`, the target state's declared label visible on the source instance; for `sourceInstanceEffect`, a changed key's rendered value or an explicit acknowledgement, failing loudly with the changed keys and their display contexts if every one is excluded by the active context **CONFIRMED 2026-09-01 by reading the helper.** `_expectShippedInstanceState` at `loom_communities_demo/integration_test/workflow_ui_evidence_test.dart:2494` resolves the signed-in fan, then polls `engine.queryInstances(tabId:, fanId:)` until the instance appears and checks `targetState`. Every assertion is against the **engine's** persisted state; nothing in it inspects rendered widgets or visible text. So an alternate-affordance row can pass while the member sees nothing — which is the same defect as the sibling row "a completed action can leave no visible result", and the two should be fixed together. This is why the production bar's proven count means less than it reads: a row proven this way establishes the engine moved, not that anyone could tell.
- [ ] `needs-live-validation` — **3 of 79 B25 rows proven — Camera Club is COMPLETE** (`photo-walk-rsvp`, `critique-submission`, `gear-loan-request`), walkthrough + UX judge, 2026-08-25, verdict at [Evidence/B25/verdicts/camera-club-b15-ux-verdict-2026-08-25-pass4-PASS.md](Evidence/B25/verdicts/camera-club-b15-ux-verdict-2026-08-25-pass4-PASS.md). It took four judging passes and two fixes — a product one so completed actions render a result, and a capture one so result frames are scrolled to their subject. The judge was told to discard its prior verdicts and re-examined the row it had already passed three times. Soccer's `soccer-team-roster` passes the walkthrough but is **not** judged, so it does not count. Next reachable: Ad-Free (6 rows, B16, persona contradiction now converged), Data Portability (9, B16), Soccer (8, B14) — see [Access Control and Workflow Service Tracker.md §8](Access%20Control%20and%20Workflow%20Service%20Tracker.md) **RE-MEASURED 2026-08-31: treat as `unknown`, not proven.** The three Camera Club rows carry **no package identity** in their evidence, so nothing can tell whether they were proven against the package shipping today — and Camera Club's regeneration was subsequently *rejected, not installed*. The denominator is wrong too: 12 of the 79 rows name a workflow their package does not ship, and 9 of those 12 are not community workflows at all.
...
- [ ] `needs-verification` — 3 engine tests skip for want of a deployed backend: 2 need `LOOM_POSTGRES_PASSWORD` (k3s PostgreSQL port-forward), 1 needs a real fan JWT plus a live community id. These are the concrete acceptance gate for Phases B/C/D — un-skipping them is how those phases get proven rather than asserted **RE-MEASURED 2026-09-01: it is four, not three, and the credentialed baseline is `316 (+1)`.** Run plainly the engine reports **312 passed, 5 skipped**; with a port-forward and the `postgres-credentials` secret it is **316 passed, 1 skipped**. CLAUDE.md's baseline table listed only the uncredentialed number without saying so — now corrected to show both, as the workflow-service row already did.
