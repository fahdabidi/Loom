...
- [ ] `needs-skill-dispatch` — **Cedar's 5 remaining orphan findings** (`hoa-architectural-request`'s `reviewerFanId`/`reviewDate`/`reviewTime`/`boardComments`/`ownerNotificationState`, all declared `writableBy: "effect"` with no effect anywhere writing them) — change each to `writableBy: "platform"`, matching solved-patterns §16. Targeted edit, not a regeneration — same pattern as this session's `hoa-document-access-request` dispatch.
