...
| ⬜ Open | `needs-skill-dispatch` | **Outstanding Skill dispatches**: DataPortability (6 prefill) and AdFree (4 orphan) never dispatched; Camera Club's regeneration was **rejected, not installed** (it moved the reminder off the per-member path); Book Club is **knowingly damaged and held** (`c0e0355b` removed its per-member `send-reminder`); Garden Club is regenerated AND **installed** — **row corrected 2026-09-08, it was stale.** This row was written 2026-08-31 and the regeneration landed after it: the shipped asset is byte-identical to the Skill output at `~/.codex-skill-authoring-scratch/garden-club-regen/garden-club-package.jsonc` (Sep 2) apart from a single added `"skillVersion": "3.6.0"` line (stamped Sep 5). `cmp` reports exactly one differing hunk, the stamp. Verified by diff, not by the row. | TODO.md, migrated | 2026-08-31 |
| ⬜ Open (**CAUSE IDENTIFIED 2026-09-08 — it is the seeding, not the packages**) | `new-ticket` | **Six dead queue transitions** — `join-queue`/`leave-queue` in Book Club, Camera and Garden. All six are declared as this row says (verified today). **But "no reachable caller" is a consequence of single-holder seed data, not a package defect.** Garden's `join-queue` guard is `allowedRoleIds: ["garden-member"]` plus the formula `if(ownerFanId == $actor, false, availabilityState == 'reserved' || availabilityState == 'onLoan')` — so it needs a `garden-member` who is **not the owner** of the item. Garden has exactly **one** `garden-member`, who becomes the owner the moment they create a listing, so no caller can ever satisfy it. The same-day sweep found this is corpus-wide: **36 of 37 role assignments have exactly one holder** (only `cedar-commons-hoa:hoa-board` has 2), so every two-party same-role interaction is dead everywhere. **DONE 2026-09-08 — second members seeded for Book Club, Camera and Garden, verified in the database, so all six transitions now have a reachable caller with no package change** — which also means the packages should NOT be edited to "fix" this, since that would converge by removal against a seed-data artifact. Corroborated from the other direction by the 2026-09-08 Garden walkthrough, which saw "Join queue" render for the owner and be correctly refused by the engine. | TODO.md, migrated | 2026-08-31 |
| ⬜ Open (**SUPERSEDED — see the corrected §8 row; do not act on this one**) | `new-ticket` | **This row is stale as written.** It was corrected 2026-09-07 in the §8 queue entry (`low-priority`, "already substantially covered by existing infrastructure"), which supersedes it: `git` cannot represent a read-only mode (it stores only `100644`/`100755`, and all 11 assets are `100644`), so the bit is a purely local working-tree property no clone or CI runner ever has — and the repo-carried enforcement this row asks for **already exists** as `community_package_provenance_test.dart`, which genuinely failed on 2026-09-07 when a package landed before its manifest was regenerated. The residual gap is narrower than this row implies: the test proves a file matches its own manifest entry, not that the Skill produced the change. **One measurement added 2026-09-08:** the guard has in fact lapsed right now — all 10 packages are `-rw-rw-r--` (664) on Linux, not 444 — which is the predicted failure, and is precisely why the durable protection has to be the provenance test rather than the mode bit. Also: this row says "eleven packages"; there are **10** (the 11th tracked file in that directory is not a package). ORIGINAL: All eleven packages are 444 again, but nothing restores the bit after a lift-copy-restore cycle that exits early, and the guard is the only thing standing between the Skill-only rule and a silent hand edit.
...
- [ ] `new-ticket` — **the six dead queue transitions are still dead.** `join-queue`/`leave-queue` in Book Club, Camera and Garden have zero effects. The service exists; nothing points at it. Needs a Skill regeneration per community plus an app-shell client, neither started **RE-EXAMINED 2026-09-01 — the wiring now exists, so "dead" needs re-testing rather than assuming.** The six are confirmed present: 3 `join-queue` + 3 `leave-queue` across Book Club, Camera and Garden. But the caller chain that was missing is now in place: `part02_tab_shell.dart` → `EngineNativeMarketplaceSurface` (part36) → `joinItemQueue`/`leaveItemQueue` → `part46_item_queue_client.dart` → the service. And the transitions declare the right action (`"action": "join_queue"`). **What I cannot confirm from source alone** is that a transition actually routes to that surface at runtime — that needs exercising on device, which is blocked behind the `calendar.*` creation refusal. So: re-test rather than re-report. Note the first search here returned nothing because I grepped `joinQueue`/`leaveQueue`; the methods are `joinItemQueue`/`leaveItemQueue`. **RESOLVED AS A PREMISE ERROR 2026-09-08, and it uncovered four real defects that are NOT this row.** I scoped this with the root cause agent (new session key `phase-queue-transitions` — new phase, new key) after building a confident theory that the queue actions were dead on non-marketplace tabs. **My theory was wrong, and the agent killed it by reading the code.** The chain I believed: `join_queue` declares `"to": null` with no effects, `_applyQueueAction` lives in `part36_engine_native_marketplace_surface.dart`, that surface is returned only for `case 'MarketplaceTabSurface'`, and Garden binds `published` to `home` as well — therefore the Home button fires a null-target, zero-effect transition and silently does nothing. **Two links were false.** (1) `_applyQueueAction` does not belong to `EngineNativeMarketplaceSurface` at all — **I attributed it to the class the FILE is named after.** Verified myself: in that file `EngineNativeMarketplaceSurface` spans lines 10–385 and `_applyQueueAction` sits at line 594, inside `_EquipmentLoanArchetypeCardState` (412–1024). Garden's Home tab resolves to `EngineNativeListSurface`, which dispatches `equipment-loan` to that **same** `EquipmentLoanArchetypeCard` — so Home and Marketplace share one queue handler and one set of queue exclusions. (2) `"to": null` with no authored effects is **not** a no-op: the engine preserves state with `transition.to ?? row.currentState` and then runs **archetype bookkeeping**, whose `equipment-loan` action map adds/removes the actor in the legacy `queuedFanIds` list — and `_applyArchetypeBookkeeping` can introduce that field even when the schema omits it. **Book Club's queue transitions are marketplace-only**, so the overlap I posited does not exist there at all. **The four defects it did find, each verified by me in source before recording:** **(A) LIVE — the queue button ignores its own guard.** `_declaredActionFor` (part36:507) matches only `candidate.action == action && candidate.from.contains(currentState)` — **no guard evaluation, confirmed by reading it.** Garden's `join-queue` guard is `if(ownerFanId == $actor, false, ...)`, so an owner is offered "Join" on their own available listing; the server evaluates the guard properly and returns 403. **(B) LATENT, not live — the replica wrapper defeats service-queue detection.** `_usesServiceItemQueue` requires `engine is RemoteWorkflowEngineApi`, and `LoomReplicaFallbackWorkflowEngineApi` **`implements WorkflowEngineApi`** rather than extending the remote class (verified: `part49:316`), so when it wraps the engine the check is false and **every tab, marketplace included, silently takes the legacy `queuedFanIds` path instead of the authoritative service queue.** I checked whether this is live and it is not, in a default build: the wrapper is applied only when `offlineReplicaCoordinator != null`, which requires `configureLoomOfflineReplicaSupportForProduction`, which runs only `if (_offlineReplicaDirectory.isNotEmpty)` — and that is a `String.fromEnvironment` defaulting to empty. The agent was careful to say it was not asserting the tested device enabled it; that caution was right. Note also the wrap passes `communityId: extensionId`, which is the identifier-space confusion this file already warns about. **(C) Book Club's declared queue audit is skipped on the service path.** Its queue transitions DO have effects — appending `joined-queue`/`left-queue` to `libraryActivityAudit` — and the service queue endpoints authorize against the transition but never execute its effects, so the audit entries are simply lost. **(D) A 403 refusal renders as "Queue unavailable."** `_applyQueueAction` funnels every exception into `_queueUnavailable = true`, discarding a distinction the client already carries (`isUnavailable`, part46:264). A genuine empty queue renders "Queue length: 0 / You are not queued", and there is even a 503 widget test asserting the unavailable text — so failure and empty ARE distinguished; what is wrongly collapsed is **refusal** into failure. Combined with (A), the user-visible bug is: press a button you were never eligible for, get "Queue unavailable", and nothing explains it. **The fix belongs in application/service code, not in package placement — keep the existing Home/Admin bindings.** The queue service should expose permitted queue actions evaluated with the same guards, roles and membership as its mutation endpoints, and `_queueToggle` should consume that; **reverting to the engine's `_actions` is explicitly NOT sufficient**, because engine availability is computed from the retired instance-local list, which is why the card bypasses it in the first place. **Reference docs are inconsistent with the implementation and need correcting too:** `equipment-loan.md:37` claims buttons follow evaluated availability, and `platform-services.md:201` still describes queue position via the legacy list. **So this row is closed as written and replaced by A–D above; the six transitions are not dead.** **SCOPE REFINED 2026-09-08 by my own reading of all three packages, not from the agent's report — defect (A) hits ALL THREE communities, each with a different visible symptom, and (C) is Book Club only.** Garden's `join-queue` guard is `if(ownerFanId == $actor, false, availabilityState == 'reserved' || availabilityState == 'onLoan')`, so an **owner is offered Join on their own listing**. Book Club's is `if(ownerFanId == $actor, false, if(availabilityState == 'available', false, true))` — the same owner exclusion **plus** an available-item exclusion, so it manifests both ways. Camera Club's is only `if(availabilityState == 'available', false, true)` with no owner clause, so its symptom is different again: **any member is offered Join on an item that is actually available**, where the correct affordance is Borrow. In all three the button is offered because `_declaredActionFor` never evaluates the guard, and in all three the server refuses — which defect (D) then renders as "Queue unavailable". **(C) is Book Club alone:** only its queue transitions carry `effects`, appending `{"event": "joined-queue"/"left-queue", "fanId": "$actor", "at": "$timestamp"}` to `libraryActivityAudit` (verified at lines 610–620 of the shipped asset). Garden's and Camera's queue transitions declare no effects at all, so nothing is lost there. This matters for the fix: **the Book Club audit gap cannot be closed by firing an extra generic transition after the queue call** — that would write to a second store with a partial-success window between them. It needs the queue service to record the audit itself, which is a separate change from A and D and is NOT in the dispatched ticket. **DO NOT "FIX" THE BYPASS ITSELF — it is deliberate and correct, and the code says so.** Verified locally 2026-09-08: `_actions` IS populated live from the engine's available-actions call, and `_action`/`_actionFor` read it, so `equipment-loan.md:37`'s claim that buttons are driven live off `availableTransitionsAsync` is **accurate for every button except the queue pair**. That pair deliberately bypasses it, and the reasoning is written directly above `_declaredActionFor`: *"The engine's old queue guards read an instance-local list. The remote queue is now durable and cross-member, so it selects the semantic action from the declared machine after the service has told us whether the viewer is in the queue."* That is the right call — engine availability for these transitions is computed from the retired `queuedFanIds` membership. **So the defect is narrower than "it bypasses availability":** in bypassing engine *availability* it also silently dropped *guard evaluation*, which it never needed to drop. The fix adds the guard check back while keeping the bypass. A future session that "restores" `_actions` for the queue actions would reintroduce the legacy-membership dependency this code was written to escape — and the reference doc needs only a narrow caveat naming the queue pair as the intentional exception, not a rewrite. **A AND D FIXED, VERIFIED AND PUSHED 2026-09-08 (`70c1f5fa`). B AND C REMAIN OPEN.** The card now evaluates the transition guard in `_declaredActionFor`, which required threading the viewer's `roleId` into `EquipmentLoanArchetypeCard` — the dispatcher already held it and passed it to sibling cards, and the `equipment-loan` branch was the one omitting it. **The deliberate bypass of `availableTransitionsAsync` is preserved**, per the note above. A 403 now sets a distinct refusal state that **keeps the last queue snapshot visible** rather than blanking it; `isUnavailable` keeps the outage presentation and the existing 503 test is untouched. **Verified by me on a quiet box, not from the dispatch's own report** — necessary because `postgres-0` was force-killed during the dispatch run (restart count 9 → 10), which makes any PostgreSQL-backed result from that window worthless: app shell **403 +2**, engine **313 +1** (skips fell from 5 to 1, i.e. the four PostgreSQL tests actually ran rather than silently skipping), workflow service **153 +1** with BOTH credential sets, demo app **160**, UX judges **501**, bundle mirror **36**. **No assertion was weakened:** every assertion line in the diff is an addition with zero removals, and the two pre-existing test files changed by exactly one line each to supply `roleId`. The three new tests include the **positive control** (an eligible non-owner IS offered Join) — without it a suite asserting only the owner's absent button would pass on an implementation that hid Join from everyone, which is precisely what a role-less `evaluateGuard` would have produced. **STILL OPEN — (C)** Book Club's queue transitions declare `libraryActivityAudit` appends that the service queue path never executes; this needs the **queue service** to record the audit, and must NOT be done by firing an extra generic transition after the queue call (two stores, partial-success window). **(C) SCOPED 2026-09-08 by reading `workflow_service.dart` myself — the service already holds everything the fix needs, and there are two constraints that would be easy to get wrong.** The join handler authorizes with `_mayAct(engine: context.engine, instance: queueInstance, fanId: ..., action: 'join_queue')` and then calls `context.repository.join(...)` directly, never applying the transition — so it **resolves and evaluates the community's own transition and then discards it**, which is exactly why declared effects never run. It has the engine, the instance and the resolved action in hand, so executing the transition's effects is mechanically available rather than requiring new plumbing. **Constraint 1 — the endpoint is idempotent, and a naive fix would double-write.** Before joining, it checks `if (existingPosition != 0)` and returns **200 with the existing entry**; a duplicate request is authorized but does not move the entry. Any audit effect must therefore fire only on the real join path, never on that idempotent replay, or a client retry appends a second `joined-queue` row for a member who never left. **Constraint 2 — guards are evaluated against a SYNTHESIZED instance, not the stored one.** `_withQueueMemberships(instance, entries.where((e) => e.fanId != caller))` rebuilds the legacy membership view from the **service** entries, deliberately omitting the caller so a duplicate request still evaluates as a join. That bridge is good design and must be preserved — but it means the instance the guard sees is **not** the instance any effect would be written against, and conflating the two would write the synthesized queue state back into `instance_data`. **Do NOT implement this by having the app fire an extra generic transition after the queue call** — that writes to two stores with a partial-success window between them, and the app already learned this lesson with `queuedFanIds`. Also note `_currentlyHoldsItem` returns **409 `item_queue_current_holder`**, a real refusal distinct from a guard failure, so whatever records the audit must not treat every non-200 alike. **STILL OPEN — (B)** `LoomReplicaFallbackWorkflowEngineApi implements WorkflowEngineApi` rather than extending `RemoteWorkflowEngineApi`, so wrapping flips `_usesServiceItemQueue` false and every tab silently takes the legacy path; dormant only because the offline-replica directory is a `String.fromEnvironment` defaulting to empty, so it is a live defect waiting on a configuration change rather than a theoretical one.  **A FOURTH OCCURRENCE EXISTS OUTSIDE THE DISPATCHED SCOPE — found 2026-09-08 by my own exhaustive sweep, not by the dispatch, because my ticket scoped only `part36`.** Enumerating every `is`/`is!`/`as RemoteWorkflowEngineApi` in the app shell gives 11 sites: one in `part25_engine_native_community_store.dart` that **already unwraps correctly** (the precedent the fix copied), nine in `part36` (all addressed), and **one in `part43_document_uploads.dart:120` that is untouched and is arguably the worst of them.** `loomDocumentUploadBlocker` returns the blocker string *"Uploading needs a connected community. This build is running on the local engine."* whenever `engine is! RemoteWorkflowEngineApi`. Under the replica wrapper that test is false while the engine genuinely IS remote — so enabling offline replica support would **block document uploads outright** and explain it with a statement that is not true, sending anyone who reads it to look for a build/configuration problem that does not exist. Note this function's own doc comment argues that a clear explanation beats a silent no-op, which is right — and is exactly why a wrong explanation costs more here than a silent fallback does elsewhere. **The follow-up fix should be a single shared helper rather than a fourth copy of the unwrap:** the dispatch necessarily produced three identical private `_remoteEngine` getters inside `part36` (one per card class), and `part25` holds a fourth inline copy, so the unwrapping rule now lives in five places and `part43` is the one that missed it. A public `resolveRemoteWorkflowEngine(WorkflowEngineApi)` used by all of them would collapse those and make the next occurrence impossible to get wrong. **This is a scoping lesson, not an agent failure** — the agent was told "this card, or any sibling archetype card in the same file", and it did exactly that, correctly finding the document-library and export-wizard siblings I had not anticipated. The population to check was the whole package. **(C) SCOPED TO A DECISION 2026-09-08/09 — recommendation is (a), execute the declared effects through the shared engine ATOMICALLY with the membership change — and the scoping found a LIVE, SEPARATE defect that must be fixed first.** Root cause agent, session key `phase-queue-transitions` (resumed — same phase). **It corrected my framing twice, both verified by me.** (1) Book Club's `libraryActivityAudit` field is **not** a pre-regression product requirement — it was added *after* `c0e0355b`; the earlier product doc requires audit support more generally. So the fix must satisfy an audit requirement, not necessarily that exact field. (2) Book Club is the only **shipped app asset** declaring effects on queue transitions, but **not the only package**: Tabletop (a reference package) has four effects that write **legacy queue membership**, and executing those unchanged would reintroduce the very storage model the service replaced. So this is a class with a hazard, not a single bug — any "execute declared effects" implementation must suppress legacy queue bookkeeping on the service route. **THE LIVE DEFECT, verified by me in source, not taken on report: an exception inside an item-queue handler commits its writes while returning HTTP 500.** `_handle:123` wraps the entire handler in `_communityTransactionRunner(communityId, ...)`, and `_withItemQueueRequest:1892` catches exceptions **inside** that boundary and returns a 500 `Response`. A returned Response is *normal completion* to the transaction runner, so PostgreSQL commits anything already written. Concretely: if anything throws after `repository.join` has inserted — the `listForItem` re-read, position computation, response construction — **the member is queued and the client is told the request failed.** Note `_database.transaction` does not rescue this either: `database.dart:884` just executes the callback when an outer executor exists and creates **no nested rollback boundary**. **This is a prerequisite for (a)** — composing effects into the same transaction is unsafe until failures actually escape it — and it is worth fixing on its own regardless. **The recommended design for (a), recorded so it is not re-derived:** lock the instance row first (`readInstanceForUpdate`) so two members appending to the same audit list cannot overwrite each other; drive effects off **`joined.created`** (the repository already reports whether this caller inserted, `item_queue_repository.dart:138`) rather than `existingPosition`; change `remove` from `Future<void>` to report whether it actually deleted a row, so a repeated leave is distinguishable from a genuine one; run effects **only when membership actually changed**, so a duplicate join or absent-member leave appends nothing; apply them against the **locked stored row**, never against the synthesized `_withQueueMemberships` instance (that projection exists only to evaluate guards and must never become the base map written to `instance_data`); reuse the engine's **extended** executor (`_applyExtendedEffects`) rather than bespoke append logic or the simple `applyEffects`, so hydration, computed expressions, branches, related-instance updates and instance creation all behave normally; and **suppress only the legacy `queuedFanIds` archetype bookkeeping** on this route. Also: `_mayAct` currently reduces to `any(transition.action == action)` and **retains no transition**, so effect execution needs a resolver that returns the specific authorized declaration — and if several declarations share an action, require an explicit transition id rather than executing the first or all of them.
...
- [x] `needs-skill-dispatch` — **SUPERSEDED 2026-09-07, verified live against the current validator, not the dated notes below.** Re-ran `/validate` directly against all 11 shipped packages today: **9 of the 10 communities listed here are already clean (0 orphan, 0 prefill)** — Chess, Mosque (Masjid Nur), Youth Soccer, Garden, MemberSocialSpace, CameraClub, AdFree, DataPortability, and Book Club. This work was done sometime after 2026-08-27 and never recorded here — the numbers below are stale, not current. **Only Cedar has real remaining findings: 5 `effect_writable_field_has_no_effect` orphans on `hoa-architectural-request`** (`reviewerFanId`, `reviewDate`, `reviewTime`, `boardComments`, `ownerNotificationState`), pre-existing and unrelated to this session's `hoa-document-access-request` addition (confirmed baseline in that feature's own commit message). **Also newly found, not in this row's original scope**: `Loom_Communities_Workflow_Engine_Phase1_TabletopClub_Example.jsonc` carries 4 orphan + 13 prefill findings AND a `skillVersion: "0.0.0"` — behind the current `3.6.0` by the full versioning migration, not a small writer-declaration touch-up. That's a separate, bigger ticket; tracked as its own row below, not folded into this one. **ORIGINAL (stale, kept for context):** Cedar (16 prefill + 6 orphan), Chess (18+6), Mosque (29+7), Youth Soccer (14+4), Garden (13+12), MemberSocialSpace (22+17), CameraClub (9+4), AdFree (0+4), DataPortability (6+0). Briefs are built at `data/<community>_brief.md` with identifiers re-derived from the shipped package, not from notes. Book Club needs one more pass too — its `ownerFanId` is `formEntry` and should be `platform` (its single remaining prefill finding)
- [x] `not-a-defect` — **Cedar's 5 "orphan" findings are a validator false positive, not a package defect — dispatch correctly declined the requested fix.** All 5 fields (`hoa-architectural-request`'s `reviewerFanId`/`reviewDate`/`reviewTime`/`boardComments`/`ownerNotificationState`) ARE written — by `hoa-committee-decision`'s cross-workflow effects using `"relatedInstance": "requestInstanceId"` (e.g. `begin-committee-review` writes `reviewDate`/`reviewTime` at package lines ~354-355; `approve-request`/`request-owner-changes`/`deny-request`/`reopen-decision` write the rest). The validator's `effect_writable_field_has_no_effect` rule apparently doesn't recognize a `relatedInstance` effect targeting a DIFFERENT workflow type as a valid writer for the target's own schema field — it only checks for writers within the same workflow or via `createInstance`. The dispatch tested the requested `effect` → `platform` change, confirmed it silences the warning, and correctly rejected it anyway because it would misdescribe a real JSON-effect-written field as platform-written. Package left byte-identical (SHA-256 confirmed unchanged). **Real next step, if anyone wants the warning gone**: extend the validator's writer-analysis to also treat a `relatedInstance`-targeted `set`/`append` effect as satisfying `writableBy: "effect"` on the TARGET workflow's field, not just same-workflow and `createInstance` writers. Low priority — cosmetic false-positive warning only, blocks nothing, `errorCount: 0` either way.
- [ ] `low-priority` — **`Loom_Communities_Workflow_Engine_Phase1_TabletopClub_Example.jsonc` is unstamped (`skillVersion` absent, treated as `0.0.0`), current is `3.6.0`.** Found 2026-09-07 while re-checking the writer-declaration row above; initially over-flagged as urgent — corrected same day. `docs/references/reference/skill-versioning.md`'s own baseline table already lists this file as **"Tabletop Club (reference fixture, not shipped to the demo app) — unreviewed, check before use"**, confirmed independently: it is absent from `app/packages/core/loom_communities_app_shell/assets/` (the 10 real shipped communities all live there) and only appears in test fixtures. Not part of the 10 shipped communities, not part of the B25 production bar, not user-facing. Its 4 orphan + 13 prefill findings can wait — full migration only worth doing if/when this fixture is promoted to a real shipped community. No action needed now.
...
- [ ] `low-priority` — **the `chmod 444` anti-hand-editing guard is not durable, BUT already substantially covered by existing infrastructure — corrected 2026-09-07, was overstated as a fully open gap.** `git` doesn't track the permission bit, so a fresh clone gets packages writable and the local speed bump disappears — that part still stands. But the repo-carried enforcement this row asks for (option (a): "a test that fails when a community `*.jsonc` changes without a corresponding [manifest] record") **already exists and already caught a real case this session**: `app/packages/tooling/loom_ux_judges/test/community_package_provenance_test.dart` fails if a package's SHA-256/byte count don't match `docs/references/_meta/community-provenance.json`, which is exactly what happens if a file changes without running `tool/update_community_provenance.dart` afterward — this test genuinely failed 2026-09-07 (`community_package_provenance_test.dart`'s "2 passed, 2 failed") when a dispatch's Cedar package update landed before the manifest was regenerated, and the fix was `dart run tool/update_community_provenance.dart`, not touching test code. **Residual, real gap**: the test proves the file matches its OWN manifest entry, not that a Skill dispatch specifically produced the change — a hand-edit followed by re-running the regeneration tool would still pass. That's a smaller, different risk than "silent undetected drift" (which is what this row originally worried about and is now covered), and matches a determined bad-actor scenario the chmod bit never protected against either. Pre-commit hook option (b) remains undecided and would need the user's buy-in on local git workflow tooling — not implemented here.
- [x] `STALE — the service was BUILT AND INTEGRATED 2026-08-27, and proven live 2026-08-30; corrected 2026-09-08` — ~~`new-milestone`~~ **the export checksum service.** This row's premise ("`migration-export-api` has no checksum concept, and nothing produces an export *bundle* for one to hash") was already false when it was migrated here, and [`Build Tracker.md`](Build%20Tracker.md)'s own §9 item 4a says so plainly: *"THE EXPORT CHECKSUM SERVICE IS BUILT AND INTEGRATED. Closed 2026-08-27"* — five commits (spec `33ecde09`, service `79da87b1`, app client `79784c13`, spec auth fix `2aed5d25`, trigger fix `af77cb29`), `export-bundle-api.openapi.yaml` with 4 operations, MinIO-backed storage, and 7 of 8 checksum-bearing workflows generating end to end. **Re-confirmed against the code 2026-09-08, not against notes:** `workflow_service.dart:2152` computes `sha256.convert(payload.bytes)` over the bytes actually served, `:2277` recomputes `sha256.convert(bytes)` fresh on verification (the failure mode Build Tracker item 4b explicitly guarded against — a verifier comparing a stored value to itself), backed by `export_bundle_repository.dart` and covered by `export_bundle_endpoints_test.dart`. **This row existing at all is the exact hazard `solved-patterns.md` §20 names** — a stale open item that outlives its implementation, which the next reader trusts and re-reports as a gap. It was in fact re-reported as unbuilt in a 2026-09-08 session summary before being checked. The one genuinely open descendant is the Garden `checksumVerified` comment (its own row above), not the service.
