...
- [ ] `needs-verification` — **`Loom_Communities_Workflow_Engine_Phase1_TabletopClub_Example.jsonc` is on `skillVersion: "0.0.0"`, current is `3.6.0`.** Found 2026-09-07 while re-checking the writer-declaration row above. Needs the full migration path in `docs/references/reference/skill-versioning.md` applied, not a small field-level fix — its 4 orphan + 13 prefill findings are likely a symptom of the version gap, not independent defects. Scope this properly before dispatching; may overlap with this doc's existing Tabletop tournament-event Phase A.1 migration note elsewhere in this tracker.
