...
| ⬜ Open (**CAUSE IDENTIFIED 2026-09-08 — it is the seeding, not the packages**) | `new-ticket` | **Six dead queue transitions** — `join-queue`/`leave-queue` in Book Club, Camera and Garden. All six are declared as this row says (verified today). **But "no reachable caller" is a consequence of single-holder seed data, not a package defect.** Garden's `join-queue` guard is `allowedRoleIds: ["garden-member"]` plus the formula `if(ownerFanId == $actor, false, availabilityState == 'reserved' || availabilityState == 'onLoan')` — so it needs a `garden-member` who is **not the owner** of the item. Garden has exactly **one** `garden-member`, who becomes the owner the moment they create a listing, so no caller can ever satisfy it. The same-day sweep found this is corpus-wide: **36 of 37 role assignments have exactly one holder** (only `cedar-commons-hoa:hoa-board` has 2), so every two-party same-role interaction is dead everywhere. **RESOLVED BY STANDING POLICY 2026-09-08 (seed several backend users per role): provision more members in these communities and the six transitions become reachable with no package change** — which also means the packages should NOT be edited to "fix" this, since that would converge by removal against a seed-data artifact. Corroborated from the other direction by the 2026-09-08 Garden walkthrough, which saw "Join queue" render for the owner and be correctly refused by the engine. | TODO.md, migrated | 2026-08-31 |
