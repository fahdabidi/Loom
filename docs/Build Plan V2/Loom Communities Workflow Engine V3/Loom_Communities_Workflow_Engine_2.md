...
| 1 | 1.16 Dashboard prioritization | `[x]` **CLOSED 2026-07-13** — independently re-derived the same conclusion (no shared urgency model across 11 categories) before reading the report; documented no-build decision + a real user-facing copy fix, analyze clean + 26/26 app-shell tests, commit `aa197bc` (see phase doc) |
| 1 | 1.17 formEntry checkbox + relative-time-picker control | `[x]` **CLOSED 2026-07-13** — independently verified: analyze clean + 27/27 app-shell tests, commits `10750b7` + `3fa2091`; verification agent wrote the widget test directly after two dispatch rounds ran out of turn budget (see phase doc) |
| 1 | 1.18 Tournament + Voting feature (flagship) | `[x]` **CLOSED 2026-07-13** — Stages 1/2a/2b + 3 tickets (attendance+RSVP `8a62c4d`; candidate popup `98dc4e9`+`a3732cb` regression fix; deadline+reminder `24827de`+`e89f206`), 33/33 app-shell tests; **live emulator walk done** on `PantryVision_Manual_API_36` — hand-installed a real Tabletop Club package via the app's actual Add Community flow, confirmed on-device: deadline/reminder banner, formula-backed attendance, candidate detail popup, real tie→runoff creation, cross-instance winner propagation, and the eligibility guard genuinely hiding Vote buttons for a non-RSVP'd persona (screenshots in `.codex-logs/m18-live-walk/`). One known gap deliberately left open (organizer-authored creation flow — a CRUD concern, not one of the capability gaps this milestone existed to prove) (see phase doc) |
| 1 | 1.19 Archetype Implementation Standard finalized | `[x]` **CLOSED 2026-07-13** — 15/17 archetype rows `[x]` with citations (test file + count + commit each); `paymentCheckout` honestly left `[ ]` (not started); `votePoll` `[x]` with an explicit caveat on 1.18's one remaining gap (organizer creation flow). Done directly by the verification agent — documentation/audit, not implementation code (see phase doc) |
| 1 | 1.20 Human sign-off gate (manual) | `[~]` **REOPENED 2026-07-13** — the user's own first hands-on walk of every tab (not just Ballot) found real defects and, deeper, that the app has no runtime path for the engine-native workflow schema (every "real" feature is a hand-written Dart engine-store per feature). The hardening pass that closes this (milestones 1.21, 1.27-1.33) is tracked in its own doc: **[Loom_Communities_Workflow_Engine_3.md](./Loom_Communities_Workflow_Engine_3.md)**. 1.20 stays open until that pass closes and the user re-reviews live. |
