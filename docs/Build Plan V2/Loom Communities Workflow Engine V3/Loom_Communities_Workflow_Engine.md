...
| 1 | 1.18 Tournament + Voting feature (flagship) | `[~]` In progress — Stages 1/2a/2b closed; **ticketed-dispatch process adopted (2026-07-13, hard gate — no self-written code)**; attendance+RSVP ticket closed first attempt (30/30, `8a62c4d`); candidate detail popup ticket closed (`98dc4e9`) — independent verification caught a real cross-test store-contamination regression + a missing-pump test bug, fixed via a follow-up ticket (`a3732cb`), now 31/31; deadline+reminder ticket closed in two rounds (code `24827de`, tests `e89f206`), now 33/33. Remaining: organizer-authored creation flow, live emulator walk (see phase doc) |
