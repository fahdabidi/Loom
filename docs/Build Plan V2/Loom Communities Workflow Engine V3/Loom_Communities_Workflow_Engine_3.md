...
| # | Milestone | Depends on | Status |
|---|---|---|---|
| AS.1 | Dismissible community-description card. From the original multi-part Calendar feature request (pre-spec-design phase): a dismissible ("X" to close) card summarizing the community — name, description, current persona/role — appearing at the top of each tab, App Shell-wide (not Calendar-specific). **Never formally approved as deferred** — it fell out of the "Calendar tab only, then other tabs" instruction by inference, not by an explicit decision at the time (corrected 2026-07-20). **Grounded 2026-07-23** (correcting the earlier wrong pointer to the community-catalog browsing screen): the actual unconditional, undismissable card is the hero identity+role banner rendered at the top of every tab's scroll content in `part01_local_extension_screen.dart:1050+` (avatar, name, tagline, persona/role card), plus the "No sponsored message right now" banner immediately above it (`:1010-1048`) — both confirmed live in the user's own walkthrough (screenshot showing the Tabletop Club hero card with no way to close it). Plan: add a close ("X") `IconButton` to the hero card's header row, backed by a simple per-session, in-memory dismissed-state set in `_LocalExtensionScreenState` — matching the existing pattern already used for `_selectedTabIdByPersonaId` (in-memory per session, not persisted to device storage) — keyed by `communityId` so dismissing it in one community doesn't hide it in another. | none | `[ ]` Not started — planned, awaiting user approval to dispatch |
| AS.2 | **Dispatcher-level scroll-position preservation on mutation refresh (universal, all tabs) — investigation only so far, per explicit user direction (2026-07-23): "we want a universal global fix across all tabs... if the card is expanding it should expand pushing the view downwards, not causing the view to completely reset to the top."** Root-caused via direct code tracing, no code changes: any successful transition/mutation on a Calendar or Giving tab instance (`EngineNativeCalendarSurface`'s `_applyTransition` → `widget.onInstanceChanged`, `part28_engine_native_calendar_surface.dart:925-994`) calls back into `EngineNativeBindingDispatcher._startLoad()` (`part27_engine_native_binding_dispatcher.dart:107-120`), which immediately sets `_bindings = null` — collapsing the **entire tab's content** to a bare zero-size `SizedBox` (`:240-245`) while the requery runs. That transient collapse shrinks the outer `SingleChildScrollView`'s (`part01_local_extension_screen.dart:998-1000`) `maxScrollExtent` below the user's current offset; Flutter clamps the scroll position down (to 0 in the general case) and it does not restore once content regrows — this is why it reproduces on *any* button in the tab, not just Calendar RSVP actions. **Fix direction**: `_startLoad()` should keep rendering the previous, still-valid `_bindings` in place while the requery runs in the background, swapping to fresh data only once it arrives — nothing should ever collapse to empty mid-mutation. Fixes this once, in the one shared dispatcher, for every tab built on it (today: Calendar, Giving) rather than a per-archetype patch — matching the "universal, global" bar explicitly requested. Verification must include reproducing the exact repro steps (RSVP action on Calendar, dues-payment action on Giving) as direct before/after screenshot evidence proving scroll position holds, not just a widget test asserting `ScrollController.offset` unchanged (though that test should also exist). | none | `[ ]` Investigated, root cause confirmed — not yet scoped as an implementation ticket, awaiting user approval to proceed |
