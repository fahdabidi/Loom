...
| CALR.3 | Real event creation: "+ New event" (event-rsvp only — split from tournament creation to keep milestone size manageable, given CALR.2 alone took eight rounds) consuming `creatable` + `editableFields`. `creatable`/`CreatableSpec` has been fully parsed/validated since GAP-2 but has zero real consumers anywhere in the app — this milestone builds the first one. Calls `createInstance` for the event and `createInstances` for its bulk per-member response rows. | CALR.1, CALR.2 | `[x]` Creation flow done and verified (commit `3aebf07`, plus `3572f5b`/`8e3ba85` fixing a vacuous test and a viewport regression — 88/88 tests). **The FAB affordance itself is not yet closed** — see CALR.3f/3g below; CALR.3e (a naive nested-`Scaffold` FAB conversion) broke 18 tests (`Scaffold` needs bounded layout constraints; the calendar's real host wraps content in an unbounded `SingleChildScrollView` — confirmed true in the live app too, not just tests) and was reverted (`8d889d2`) before landing. |
| CALR.3f | **Foundational fix, blocks 3g**: give the currently-selected tab bounded height in `_TabNativeRenderer` (`part02_tab_shell.dart`) instead of one shared unbounded `SingleChildScrollView` around all tab content. Each tab can still scroll internally — it needs to do so within its own bounded region, not at the outer level. Touches a file every tab depends on; must be regression-tested against all ~20 tab surfaces, not just Calendar. | CALR.3 | `[ ]` Not started |
| CALR.3g | Generic creatable-action FAB, **replacing** (not supplementing) CALR.3's Calendar-specific `workflowType == 'event-rsvp'` check and its `_EventRsvpCreationDialog`. New shared mechanism alongside `EngineNativeBindingDispatcher` (not bolted onto Calendar specifically) — Calendar becomes its first real consumer, same as it's already a consumer of the dispatcher itself. Detects every `creatable` binding matching the current tab + viewer persona; resolves `multiActionStyle`/`tabActionStyles` (new grammar, see spec note below) to pick `speedDial`/`stacked`/`singleFirst` rendering; generalizes the creation dialog to work off any workflow's `instanceDataSchema`/`editableFields`, not just event-rsvp's. | CALR.3f | `[ ]` Not started |
| CALR.3b | Tournament creation: "New tournament" (tournament-event), same pattern as CALR.3 but for `tournament-event`'s own schema — deliberately deferred out of CALR.3. Its `creatable` FAB should land via CALR.3g's generic mechanism, not a second bespoke implementation. | CALR.1, CALR.3g | `[ ]` Not started |
| CALR.4 | Ballot creation: a second `creatable` affordance on the tournament-event's own card ("Create ballot for this tournament"), invoking creation with `context: {eventId}`, resolving `{context.eventId}` for real. | CALR.1, CALR.3b | `[ ]` Not started |
