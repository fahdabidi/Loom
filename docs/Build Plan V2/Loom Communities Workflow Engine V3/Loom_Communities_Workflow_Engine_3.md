...
| CALR.5d | **Live emulator walkthrough + full regression**, per §7's Verification Standard (full-tab card audit, screen-validation evidence matrix, random regression re-check) — **plus the side-by-side screenshot comparison against the three Google Calendar reference screenshots this milestone's own layout direction requires** (date-rail placement, slim-card density, grid-then-list vertical composition), not just a functional exercise of Day/Week/Month/Pending and the RSVP actions. Exercises the full acceptance methodology: create randomized events as organizer → switch to a real individual member account (CALR.4b) → RSVP → verify correct Day/Week/Month/Pending scoping and filterableFacets narrowing, live. | CALR.5a, CALR.5b, CALR.5c | `[x]` **Done.** Live walk on `PantryVision_Manual_API_36` (Android 36 emulator, WSLg-visible), `loom_communities_demo` installed fresh via `flutter run`, Tabletop Club sideloaded through the real "Add local community" dialog using a freshly-built, comment-stripped copy of the frozen fixture (screenshots in `.codex-logs/loom-calr5d-*.png`). **Layout confirmed matching the Google Calendar reference** (`#06`-`#08`): month grid at top, date-strip below it, scope control (Day/Week/Month/Pending) and facet row (`Full events`/`Has waitlist`/`Number accepted: N`) beneath that, then the agenda list with the date rendered once on the left and that day's event chips stacked to its right — Summer Tournament and Friday game night both correctly grouped under their shared `2026-07-10` date rail. **Accordion expand-in-place confirmed both ways** (`#08`-`#11`): tapping Summer Tournament's row expanded its full detail card directly beneath it; tapping Friday's row collapsed Summer Tournament and expanded Friday's own card in its place — proving the single-selection swap, not just initial render. **Pending scope confirmed empty for the organizer persona** (`#12`, correct — nothing genuinely pending for Alex Chen right now) and **Marketplace confirmed regression-clean** (`#14`, Catan listing renders correctly) as the random regression check. **Full acceptance methodology exercised end-to-end**: switched from Organizer to a real individual member account via CALR.4b's actual "Sign in as a specific person…" flow (`#16`-`#19`, not a test-hook bypass) — signed in as Priya N. (`tabletop-member-05`), confirmed her Summer Tournament view correctly differs from the organizer's (member sees `8/8 going` + `Can't make it` only, no generic RSVP actions, since she's already committed and the tournament is full), confirmed her real per-individual Friday RSVP row shows `Going` exactly matching the frozen fixture's seeded `resp-friday-member-05` state (`#22`-`#23`), then **live-mutated her response to `Maybe`** (`#25`) and confirmed the capacity formulas recomputed correctly in real time (`11/20 going` → `10/20 going`, `9 seats left` → `10 seats left`) — proving the whole CALR.5a/5b/5c pipeline (scope filtering, generic `responseTable` Pending resolution, `filterableFacets`, and the accordion layout) operates correctly against real engine mutations, not just static fixture data. Emulator and app left running and visible for the user's own manual inspection. |
