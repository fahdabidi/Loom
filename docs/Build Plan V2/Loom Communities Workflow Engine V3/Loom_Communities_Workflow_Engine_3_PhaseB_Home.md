...
| B.4 | Ballot: cross-instance eligibility guard, end-to-end | `[x]` **Done 2026-08-04.** Dispatched via ticket `data/v3_ticket_phaseb4_eligibility_guard_negative_test.md` (commit `466648f7`) — no production change needed, the guard was already fully engine-enforced (Milestone 1.4/CALR.1). New widget test calls `engine.applyTransition('cast-vote', ...)` directly for `tabletop-organizer` (not in `goingPersonaIds`) and confirms it genuinely throws (`StateError: Transition cast-vote is not available for tabletop-organizer`) and creates no `tournament-vote` row, contrasted against the same call succeeding for an eligible member. One fix-round (`603bde12`) was needed on the test's own assertion mechanics — `expectLater(tester.runAsync(...), throwsA(...))` doesn't correctly observe an error surfaced through `runAsync`; independent verification caught this by reading the actual failure output, which showed the engine already throwing the exact right error while the test's own assertion reported it as unmet — replaced with an explicit try/catch inside `runAsync` capturing the error into a local for a direct `expect`. Independently verified: `flutter analyze` clean; full `loom_communities_app_shell` suite **168/169**, the sole failure being the same pre-existing, separately-tracked `v3_milestone_a11_event_rsvp_archetype_test.dart` flake — zero new regressions. |
| B.5 | Ballot: deadline + `dueNotifications`-driven reminder banner | `[x]` **Done 2026-08-04.** B.3's card already rendered `deadline`/`isExpiringSoon` straight from the engine-computed fields (`dueAt = subtractHours(deadline, ...)`, `isExpiringSoon = isPast(dueAt)`) — never a bespoke store — but that only proved correctness for one frozen snapshot. This milestone proved genuine liveness: ticket `data/v3_ticket_phaseb5_reminder_liveness.md` (commit `cf2489ec`), a `loom_workflow_engine`-level test using `LocalWorkflowEngineApi`'s injectable `clock:` parameter (the same mechanism Milestone 1.4's own `dueNotifications` test used) — seeds one ballot instance, queries it with the clock set before the computed `dueAt` (`isExpiringSoon == false`), advances only the clock past `dueAt` with zero data mutation, queries again (`isExpiringSoon == true`, same `dueAt` both times) — proving the SAME stored data produces a different, correct result purely as a function of time. No production/app-shell change needed (nothing was broken); done in one round, no fix needed. Independently verified: `dart analyze` clean (21 pre-existing, unrelated `prefer_const_constructors` info hints, unchanged); full `loom_workflow_engine` suite **193/193** (192 baseline + 1 new test), zero regressions. |
| B.6 | Ballot: close-vote → **real runoff** on tie, **cross-instance winner write** otherwise | `[x]` **Done 2026-08-04.** Found and fixed a real, previously-undiscovered **engine bug**, not just a UI gap: this was the first time anything actually exercised `close-vote`'s `branch`/`isTie` condition against the real, redesigned (query-backed `ballots`/per-row `tournament-vote`) grammar through a live transition — it failed unconditionally, tie or not (`Bad state: branch "if" formula must evaluate to bool`). Root cause: `LocalWorkflowEngineApi._applyExtendedEffects` never hydrated query-backed source fields (`_hydrateSourceFields`) before evaluating computed-field formulas for effect/branch conditions — every other read path in the file already did this at 3 other call sites; this one (live transition effect evaluation) was missed. Fixed with a 3-line change (commit `fc635b40`): hydrate a local copy before computing, while leaving the persisted `data` itself un-hydrated (query-backed fields must never be written back to storage). This affects every transition whose effects reference a query-backed-derived computed field, not just this one ballot — full regression run across both packages found zero fallout elsewhere. Widget-level proof (commit `70c98a84`, fix-round `89b93fc5` for two unrelated test-authoring bugs — a missing interleaved-pump wait pattern and an incorrect actor persona for the guard's literal role check) exercises the real `Close vote` button for both outcomes: a clear winner (seeded 2/1/1 tally) writes `selectedGame` onto the event instance; a genuine 3-way tie (engineered via real `cast-vote` calls) creates a real runoff `tournament-ballot` instance holding exactly the tied candidates and leaves the event's `selectedGame` untouched. New engine-level regression test (`v3_milestone_phaseb6_engine_hydration_test.dart`) proves the same two scenarios directly against `applyTransition`. Independently verified: `dart analyze`/`flutter analyze` clean; `loom_workflow_engine` **195/195**; `loom_communities_app_shell` **170/171** (171 = 169 prior baseline + 2 new tests), sole failure the same pre-existing, separately-tracked `v3_milestone_a11_event_rsvp_archetype_test.dart` flake. |
| B.7 | Tournament attendance card + published announcements on Home | `[x]` **Done 2026-08-04** (commit `3b08d388`, fix-round `62d4194c`). `VotePollArchetypeCard` extended with a second internal branch (`repeater == null && workflowType == 'tournament-event'`) rendering a bespoke attendance/quorum card — title, `Accepted: N / M` (the already-existing `accepted`/`minimumAttendance` formula fields, not hand-computed), and the existing `quorumMet` formula rendered as a status line. Every other non-repeater `votePoll` binding still correctly falls to the generic card. Announcements: confirmed via a real `applyTransition('publish', ...)` call (no bespoke publish UI exists or was built — out of scope) that a published announcement's title/body genuinely appear on Home afterward, rendered by the existing generic fallback in `detail` display context (a small, targeted `EngineNativeListSurface` change so `notificationInbox`-family bindings show their full body, not just tile-context fields) — building a bespoke `notificationInbox` archetype widget was explicitly deferred as a separate, larger, out-of-scope effort. One fix-round needed: B.7's own change legitimately produces two `VotePollArchetypeCard` instances on Home now (ballot + attendance), which broke a pre-existing B.2/B.3-era test's stale `find.byType(VotePollArchetypeCard), findsOneWidget` assertion — not a regression, a correctly-outdated assumption; re-scoped to the ballot's own instance key. Independently verified: `flutter analyze` clean; full `loom_communities_app_shell` suite **172/173**, sole failure the same pre-existing, separately-tracked `v3_milestone_a11_event_rsvp_archetype_test.dart` flake. |
