...
- Current `docs/Build Plan V2/Evidence/B25/b25-component-doc-context-<run-id>.json`, generated by
  `b25_component_doc_context.dart` for this B25 run.
- The card-surface catalog index at
...
- The tab renderer contracts guide at
  `docs/Build Plan V2/Skill/components/card-surfaces/tab-renderer-contracts.md`.
- Every card-surface component guide referenced by the product doc's
  `### B25 Card Surface Registry Mapping`, such as
  `docs/Build Plan V2/Skill/components/card-surfaces/calendar.md`,
  `docs/Build Plan V2/Skill/components/card-surfaces/messages.md`,
  `docs/Build Plan V2/Skill/components/card-surfaces/equipment-loan.md`,
  `docs/Build Plan V2/Skill/components/card-surfaces/documents.md`, or
  `docs/Build Plan V2/Skill/components/card-surfaces/workflow-status.md`.
...
For standalone Skill runs, use the equivalent bundled Skill component docs under
`<extension-workspace>/components/card-surfaces/**` when the native Loom repo path is not available.
Do not judge tabs, surfaces, theme, or presentation-state usage from memory or generic app design
assumptions. Load and cite the component docs that define the applicable contracts.
The reviewer must read those component docs every B25 run, even when hashes are unchanged from a
prior pass. Prior-run summaries, tickets, or remembered guidance are not valid substitutes for
current-file review.
...
## Component And Tab Contract Loading Rules

Before answering any pass/fail question, the LLM reviewer must load and record these component
contract inputs:

1. `components/card-surfaces/README.md` for the available surface families and catalog rules.
2. `components/card-surfaces/app-shell-navigation-theming.md` for tabs, persona visibility, pinning
   policy, minimized/medium/expanded presentation states, theme, typography, density, and community
   list customization.
3. `components/card-surfaces/tab-renderer-contracts.md` for Calendar, Messages, Marketplace,
   Documents, Workflow Status, and custom tab renderer expectations.
4. Every `components/card-surfaces/<surface>.md` file named by the community product doc's
   `### B25 Card Surface Registry Mapping`.

For every loaded component doc, copy the current metadata from
`b25-component-doc-context-<run-id>.json` into the LLM output and then add the LLM's own semantic
review:
...
- `reviewRunId`
- `semanticSummary`: what this doc currently requires, in the reviewer's own words
- `semanticImplicationsForCommunities`: how those requirements affect community tabs, card surfaces,
  theme/customization, evidence, and tickets
- `changedSincePriorRun` / `semanticDeltaSincePriorRun`, when prior B25 component-doc metadata is
  available and the hash or git commit changed

If a component doc hash or last-commit SHA changed, the reviewer must not only state that it changed;
it must explain the semantic change and identify which product docs, tab mappings, card-surface
selections, screenshots, or implementation tickets need updates. If the doc did not change, the
reviewer must still summarize current semantics and confirm it reread the file for this run.

If a workflow references a card-surface family that has no loaded component doc, create a major
`component-doc-missing` finding. If the product doc maps a workflow to a surface/tab that contradicts
the loaded component docs, create a `component-contract-mismatch` finding. If screenshots implement a
surface or interaction not covered by the loaded component docs, create a
`component-doc-expansion-needed` finding so the docs are updated before the UI is judged complete.
...
9. **Component contract fit.** Cross-check every selected `cardSurfaceFamily`, API contract,
   required interaction, and renderer/fake-backend support row against the loaded component docs.
   Fail if the chosen card surface is too generic for the community job-to-be-done, if a better
   documented surface exists, if required interactions are omitted, or if the implementation appears
   to use a pattern the component docs do not support.
10. **Tab renderer and customization fit.** Cross-check Section 3.1 and screenshots against
   `app-shell-navigation-theming.md` and `tab-renderer-contracts.md`. Fail if Calendar, Messages,
   Marketplace, Documents, Workflow Status, or custom tabs use generic workflow lists instead of the
   documented tab-native renderer contract, or if community theme/typography/presentation knobs are
   declared but not visibly applied.
11. **Product-doc expansion from screenshots.** If screenshots show interactions such as `Change
...
12. **Implementation remediation from docs.** If the product doc clearly requires richer domain content
...
`cardSurfaceFamily`, `componentDocPaths`, `status`, `directQuestionAnswers`, `affectedScreenRowIds`,
...
- Does the card-surface registry define the API contract and interactions needed by the visible
  surface?
- Which exact component docs were loaded for this workflow/tab, and do they support the selected
  surface family, tab renderer, interactions, theme/customization choices, and evidence expectation?
- Does the selected card-surface contract fit this community job-to-be-done better than the available
  alternatives in the loaded catalog? If not, which surface should replace it and why?
- Does the tab use the loaded tab-renderer contract for Calendar, Messages, Marketplace, Documents,
  Workflow Status, or a declared custom tab, instead of falling back to a generic workflow-card list?
- Do color, typography, density, pinning, presentation-state, and community-card choices follow
  `app-shell-navigation-theming.md`, and are any deviations documented in the product doc?
...
```json
{
  ...
  "reviewedComponentDocPaths": [],
  "componentDocReview": {
    "status": "pass|fail",
    "contextPath": "docs/Build Plan V2/Evidence/B25/b25-component-doc-context-<run-id>.json",
    "reviewedThisRun": true,
    "docs": [
      {
        "path": "docs/Build Plan V2/Skill/components/card-surfaces/app-shell-navigation-theming.md",
        "sha256": "<sha256-from-context>",
        "gitLastCommitSha": "<git-log-sha-from-context>",
        "gitStatus": "clean|<git status --short line>",
        "reviewedThisRun": true,
        "reviewRunId": "<run-id>",
        "semanticSummary": "<what this doc currently requires>",
        "semanticImplicationsForCommunities": "<how communities/tabs/surfaces/theme evidence must react>",
        "changedSincePriorRun": false,
        "semanticDeltaSincePriorRun": "<required when changedSincePriorRun=true>"
      }
    ]
  },
  ...
  "findings": [
    {
      ...
      "affectedComponentDocPaths": [],
      "componentContractId": "<surface-or-renderer-contract-id>",
...
- component docs reviewed, including tab renderer, App Shell customization, and applicable
  card-surface docs;
...
Missing `reviewedComponentDocPaths`, missing relevant card-surface docs, or pass-shaped
`tabRendererResults[]` rows that do not cite component docs are major reconciliation findings.
Missing `componentDocReview.docs[]`, missing `reviewedThisRun=true`, missing semantic summaries, or
hash/git metadata that does not match `b25-component-doc-context-<run-id>.json` is also a major
finding and will fail `production_ux_judge.dart`.
