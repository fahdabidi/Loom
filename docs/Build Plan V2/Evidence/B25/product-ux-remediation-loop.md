...
Status: reopened under `b25-production-ux-v4`; iteration 10 repaired screenshot persistence, recaptured
fresh phase-split B12-B20 screenshot evidence, and reran the full B25 judge chain. The latest review
fails with 4 unresolved major findings based on fresh screenshot-backed evidence. The latest remediation
plan is `b25-remediation-plan-b25-v4-pass-8.md`.
...
`b25-v4-pass-4` is now historical under the stricter visual inspection gate. Screenshot persistence is
repaired as of pass 7. B25 cannot close again until the affected UI is remediated, screenshots are
refreshed again after remediation, the visual auditor passes, the independent UX judge passes, and the
production UX judge has zero blocking criteria.
