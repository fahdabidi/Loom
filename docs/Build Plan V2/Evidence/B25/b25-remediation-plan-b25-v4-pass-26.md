...
| Status | `open` |
| Source tickets | `../docs/Build Plan V2/Evidence/B25/b25-remediation-tickets-b25-v4-pass-25.json` |
| Ticket count | 9 |
...
| --- | --- | --- | --- | --- | --- |
| `b25-v4-row-001-garden-event-rsvp-0` | Garden Club | `garden-event-rsvp` | member | B13_ext_garden_club_garden-event-rsvp_start | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-002-garden-event-rsvp-1` | Garden Club | `garden-event-rsvp` | member | B13_ext_garden_club_garden-event-rsvp_action | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-003-garden-event-rsvp-2` | Garden Club | `garden-event-rsvp` | member | B13_ext_garden_club_garden-event-rsvp_complete | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-004-plant-exchange-submission-0` | Garden Club | `plant-exchange-submission` | member | B13_ext_garden_club_plant-exchange-submission_start | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-005-plant-exchange-submission-1` | Garden Club | `plant-exchange-submission` | member | B13_ext_garden_club_plant-exchange-submission_action | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-006-plant-exchange-submission-2` | Garden Club | `plant-exchange-submission` | member | B13_ext_garden_club_plant-exchange-submission_complete | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
...
| `b25-v4-row-017-soccer-team-roster-1` | Riverside Youth Soccer | `soccer-team-roster` | coach | B14_ext_youth_soccer_soccer-team-roster_action | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
...
| `b25-v4-row-022-hoa-facility-reservation-0` | Cedar Commons HOA | `hoa-facility-reservation` | member | B14_ext_hoa_hoa-facility-reservation_start | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-023-hoa-facility-reservation-1` | Cedar Commons HOA | `hoa-facility-reservation` | member | B14_ext_hoa_hoa-facility-reservation_action | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-024-hoa-facility-reservation-2` | Cedar Commons HOA | `hoa-facility-reservation` | member | B14_ext_hoa_hoa-facility-reservation_complete | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-025-hoa-export-evidence-0` | Cedar Commons HOA | `hoa-export-evidence` | owner | B14_ext_hoa_hoa-export-evidence_start | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-026-hoa-export-evidence-1` | Cedar Commons HOA | `hoa-export-evidence` | owner | B14_ext_hoa_hoa-export-evidence_action | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-027-hoa-export-evidence-2` | Cedar Commons HOA | `hoa-export-evidence` | owner | B14_ext_hoa_hoa-export-evidence_complete | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
| `b25-v4-row-028-mosque-donor-visibility-0` | Masjid Nur | `mosque-donor-visibility` | donor | B14_ext_mosque_mosque-donor-visibility_start | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
...
| `b25-v4-row-030-mosque-donor-visibility-2` | Masjid Nur | `mosque-donor-visibility` | donor | B14_ext_mosque_mosque-donor-visibility_complete | LLM required fix: Replace or refine this row as part of the affected finding remediation, then recapture a fresh screenshot with a distinct production-native state. Current row verdict is `fail` with severity `major`. |
