...
- [ ] `new-ticket` — **two disagreeing sources of role truth.** The Dart evidence catalog (`part15_evidence_catalog.dart`) says Ad-Free has `ad-off-admin` and Soccer has a bare `owner`; the shipped JSON says `ad-off-owner` and `soccer-owner`. The walkthrough reads the catalog, the validator reads the JSON, and nothing keeps them in sync — the same drift class as the three mirrors gated on 2026-08-24. This can silently invalidate walkthrough results
