...
- [ ] `needs-debug-agent` — **THE WALKTHROUGH DOES NOT INVOKE THE ALTERNATE ACTION for at least 2 of 3 Camera workflows.** It captures a frame named `alternate_action` and another named `result_receiver`, but between them the screen only *scrolls* — the control is shown enabled and never pressed, so no durable consequence exists to prove. Frame sizes corroborate: the passing row's two frames differ by 16%, the two failing rows by 0.7% and 2%. This is a harness defect, not a product one, and it caps what any community can prove — **every row needs an alternate leg**. Highest-priority blocker on the production bar
- [ ] `needs-live-validation` — **1 of 79 B25 rows proven** (Camera Club `photo-walk-rsvp` only). **CORRECTED DOWNWARD 2026-08-24**: "2 of 79" was reported on a first-pass verdict that passed `critique-submission`; re-judging fresh frames, the same judge failed it for the defect above. `gear-loan-request` failed both passes. Soccer's `soccer-team-roster` passes the walkthrough but is **not** judged, so it does not count. Reachable communities remaining: Ad-Free (6 rows, B16 — but its `ad-off-community-checkout` row names the wrong persona), Data Portability (9, B16), Soccer (8, B14) — see [Access Control and Workflow Service Tracker.md §8](Access%20Control%20and%20Workflow%20Service%20Tracker.md)
