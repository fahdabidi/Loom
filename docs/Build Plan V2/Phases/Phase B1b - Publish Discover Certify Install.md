...
Complete `Phase B1b - UX Decisions.md` before implementation work that affects UI, interaction, user-visible state, or workflow copy. The UX Decisions file is a gate artifact and must follow this required format:

1. **Reference Sources Reviewed** - find several reference implementations of real-backend-publish mode, builder signing, certification, package permission review, QR/handle discovery, hosted-install preview, and latest certified open behavior validated through local fakes; record each source, surface/flow reviewed, why it applies, patterns observed, applicability/gaps, and review date.
