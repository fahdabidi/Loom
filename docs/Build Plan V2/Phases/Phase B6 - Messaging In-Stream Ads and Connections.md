...
Complete `Phase B6 - UX Decisions.md` before implementation work that affects UI, interaction, user-visible state, or workflow copy. The UX Decisions file is a gate artifact and must follow this required format:

1. **Reference Sources Reviewed** - find several reference implementations of required Messages and Connections navigation, direct/group messaging, connection invite/block, stream rendering, in-stream ad disclosure, top banner ad behavior, and no-fill states; record each source, surface/flow reviewed, why it applies, patterns observed, applicability/gaps, and review date.
