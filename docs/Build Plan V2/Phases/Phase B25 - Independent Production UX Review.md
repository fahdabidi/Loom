...
- B25 component-doc context at
  `docs/Build Plan V2/Evidence/B25/b25-component-doc-context-<run-id>.json` and `.md`, generated by
  `b25_component_doc_context.dart` immediately before the LLM Product Docs reconciliation. The LLM
  reviewer must reread the current component docs every run, not only when their hashes changed, and
  must copy each required doc's path, SHA-256, git last-commit SHA, git status, review run ID, semantic
  summary, and community/theme/tab implications into `componentDocReview.docs[]`. The deterministic
  production judge cross-checks those fields against the current checkout and fails stale,
  pass-shaped, or summary-only review output.
...
B25 also cannot pass until the LLM Product Docs reconciliation proves it reread the current App Shell,
tab renderer, and referenced card-surface component docs in the same run. The reconciliation must
include `componentDocReview.docs[]` with current hashes, git last-commit SHAs, git dirty status,
semantic summaries, and community implications. The deterministic production judge cross-checks that
metadata against the checkout; stale hashes, missing reviewed-this-run flags, weak semantic summaries,
or missing semantic deltas for changed docs block the phase.
...
11. **Component-doc context capture:** immediately before the LLM Product Docs reconciliation, run the
    component-doc context generator from the current checkout:
   ...
   ```
    ...
    This artifact records current hashes, git last-commit SHAs, and dirty status for the App Shell,
    tab renderer, and card-surface component docs. It must be regenerated every B25 run. A prior
    context file cannot be reused for a new LLM reconciliation pass.

12. **LLM Product Docs to Evidence Workflow Reconciliation:** before the vision/product-quality judge,
    ...
    screenshots, visible-text extracts, screenshot hashes, app commit SHA, review JSON, and the
    B25 component-doc context JSON/Markdown. Do not give the reviewer worker implementation notes or
   ...
   - `componentDocReview.docs[]` proving the reviewer reread each required component doc in this run,
     with matching `path`, `sha256`, `gitLastCommitSha`, `gitStatus`, `reviewedThisRun=true`,
     `reviewRunId`, `semanticSummary`, and `semanticImplicationsForCommunities`.

    If a component doc's current hash or git last-commit SHA differs from the previous B25 run, the
    reviewer must explain the semantic delta and identify any Product Docs, tab/card-surface mapping,
    screenshot coverage, implementation, or ticket updates required by that changed guidance. If the
    hash did not change, the reviewer still must summarize the current semantics and confirm it reread
    the file this run.
...
13. **LLM Vision UX Judge Agent:** send the current product docs, production UX blueprint,
...
14. **Validate and import LLM vision review:** first run `b25_llm_review_freshness_gate.dart` against
...
16. **Production UX judge scorecard:** run the deterministic production judge against the independent
   ...
   incomplete, stale, or non-commit-eligible screenshot evidence. It must fail criterion
   `b25-c17-component-doc-freshness` when the LLM reconciliation did not reread the current component
   docs this run or when its recorded hashes/git metadata do not match the current checkout.
...
| Component Doc Context Tool | Records current App Shell, tab-renderer, and card-surface doc hashes, git last-commit SHAs, and dirty status for the LLM reconciliation gate. |
...
registry. First generate and provide the current B25 component-doc context JSON/Markdown. The LLM must
reread the App Shell navigation/theming doc, tab-renderer contracts, and referenced card-surface docs
every run, validate whether changed docs changed the semantics, and copy the current hash/git metadata
into `componentDocReview.docs[]`. Compare those rows to the current screen matrix, workflow/persona
coverage, visible text, and screenshots. Fail when a documented workflow lacks screenshot-backed
implementation, when screenshots show a workflow or UI interaction missing from the product doc, when
required visible proof is not visible, or when the doc/screenshot pair maps a primary user job to a
generic workflow card instead of a domain-native surface. Write
...
- The LLM reconciliation includes a passing component-doc freshness review: every required App Shell,
  tab renderer, and card-surface doc was reread this run, semantically summarized, checked for semantic
  changes, and hash/git cross-checked by the deterministic production judge.
...
machine-readable review JSON, B25 component-doc context JSON/Markdown, LLM Product Docs to Evidence
Workflow Reconciliation JSON/Markdown,
