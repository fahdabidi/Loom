...
| ✅ Closed (`7061b1e7`) | `fixed` | Cedar Commons HOA: `hoa-member-document` access-request flow has no board-side grant/deny resolution (dead end once requested); traceability overclaims "board sees document access audit" — 5 audit list fields have no display config and render nowhere. Non-blocking fast-follow, judge-identified. | m15-cedarhoa-r2 judge | 2026-08-12 | **FIXED 2026-09-07.** New `hoa-document-access-request` workflow type shipped via the Skill (targeted-edit mode, exactly 3 hunks, zero deletions, verified against the shipped package's diff and independently re-validated: `status=pass, errorCount=0, warningCount=9`, byte-identical to the dispatch's own report). Both copies (`docs/references/communities` and the app-bundled asset) updated and confirmed identical. Product doc's B25 addendum and Card Surface Registry rows added. The audit-field-count half of this row remains genuinely unverified (see below) — not addressed by this fix, since it was about a `hoa-member-document` display-config claim, not the access-resolution mechanism. **HALF CONFIRMED 2026-09-01.** The dead end is real and verified against the shipped package: `hoa-member-document` declares `request-document-access`, `withdraw-access-request` and `share-document`, and **no** grant, deny, decide, approve or reject transition — so a member can ask and withdraw, and the board has no way to resolve it. (`share-document` is the owner sharing outright, not a response to a request.) **The audit half I could not reproduce as written**: the item says "5 audit list fields have no display config", and I find a single `auditLabel` field, referenced by two traceability blocks. Either the five are counted somewhere I did not look, or the number is wrong — recorded as unverified rather than assumed either way. **DESIGN DECIDED, 2026-09-07 (autonomous mode, no new grammar needed):** `request-document-access` currently only appends to `engagementAudit` on the shared document instance — the same instance every requester shares — with no per-requester tracking, and `explicitReaderFanIds` is `writableBy: "platform"` so no transition can even grant it. The fix is not a new field on the document; it's a **separate workflow instance per request**, mirroring the exact pattern this same community already uses for `hoa-committee-decision` resolving `hoa-architectural-request` (per-instance board approve/deny, `transitionRelated`/`relatedInstance` effects syncing the paired instance, `createInstance` firing an `hoa-owner-notification`). A new `hoa-document-access-request` workflow type: member-created on `request-document-access` (now `createInstance`-ing this new type instead of only appending audit), board `grant-access`/`deny-access` transitions on it (guard `allowedRoleIds: ["hoa-board"]`) that append to the *requesting member's* `explicitReaderFanIds` on the related `hoa-member-document` instance via `relatedInstance`, plus an owner-notification on decision. Product doc updated directly (my own authority, not Skill-authored) to specify this — see `docs/references/communities/cedar-commons-hoa-product-experience.md` §5/§6/§7, new `hoa-document-access-request` row added alongside `hoa-member-document`. **Not yet dispatched to the Skill** — queued as the next Cedar Skill-authoring ticket, since this changes both the community JSON and needs the existing `request-document-access`/`withdraw-access-request` transitions updated to target the new workflow type instead of `engagementAudit`-only.
