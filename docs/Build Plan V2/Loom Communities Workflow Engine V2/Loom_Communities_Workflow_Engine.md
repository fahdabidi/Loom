...
| 5 | 5.6 Mosque tab reimplementation | `[r]` **READY FOR RE-VERIFICATION 2026-07-11** - sent-back fix commit `54884eb` changes `assign-care-request` to set `assignedReviewerPersonaId` from `{actor}` instead of hardcoded `mosque-admin`; scratch helper scripts are absent and working tree is clean. Re-run gates: app-shell analyze clean; tooling analyze clean; `milestone_1_3_test.dart` 36/36; app-shell package tests 5/5; `b46_mosque_engine_migration_test.dart` 1/1; workflow engine tests 73/73; full demo suite `flutter test --concurrency=1 apps/loom_communities_demo/test` 129/129. Mailbox cleared before `[r]`; awaiting verification/live screenshot validation. |
