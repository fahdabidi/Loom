...
| 5 | 5.4 Chess Club tab reimplementation | `[r]` **READY FOR VERIFICATION 2026-07-09** - implemented Chess Club end-to-end through the workflow engine. Changed files: `part02_tab_shell.dart` adds `_ChessClubEngineTabSurface`, `_ChessClubEngineStore`, fixture parsing, real `WorkflowDatabase.memory()`/`LocalWorkflowEngineApi` calls, and the result-to-rankings mutation; `part12_persona_and_tabs.dart` adds Matches/Calendar/Rankings/Admin/Documents/Messages tabs; `part16_experience_catalog.dart` replaces the old three-workflow Chess stub with match, result, rankings, calendar, admin, export, documents, and messages workflows; added `Loom_Communities_Workflow_Engine_ChessClub_Example.jsonc` plus package asset; added `b44_chess_engine_migration_test.dart`; extended `milestone_1_3_test.dart` with `Validator - Chess Club migration fixture`. Generality note: `rankingMode` is fixture/template metadata on the existing `table` archetype with no workflow-engine schema change; the cross-workflow effect is declared by the `chess-match-result` transition effects targeting `rankingRows` and implemented in `_ChessClubEngineStore._rankingsEffect`, which calls `LocalWorkflowEngineApi.updateInstanceFields` on the persisted `chess-rankings-table` instance after `submit-result`/`correct-result`. Local validation: `dart analyze packages/core/loom_communities_app_shell` clean; `dart analyze packages/tooling/loom_ux_judges` clean; `dart test packages/tooling/loom_ux_judges/test/milestone_1_3_test.dart` 34/34; Chess fixture validator `status: pass`, `errorCount: 0`, `warningCount: 0`; `flutter test packages/core/loom_communities_app_shell` 5/5; focused `flutter test apps/loom_communities_demo/test/b44_chess_engine_migration_test.dart` 1/1; full `flutter test apps/loom_communities_demo/test` 127/127. Live emulator screenshot validation is prepared by stable tab/action keys for Home, Matches, Calendar, Rankings, Admin, Documents, and Messages. Commit coverage: `e5492c8` (`Implement Phase 5 engine migrations M5.1-M5.4`). |
