...
| 5 | 5.5 Youth Soccer tab reimplementation | `[!]` **SENT BACK 2026-07-10** — code verification found the guardian's "own child only" `protectedDetail` gate is fully hardcoded, not genuinely computed: `_YouthSoccerEngineStore._visible` (`part19_youth_soccer_engine.dart:826`) filters the roster row with a literal `playerName == 'Sofia Rivera'` check, not any guardian-to-player identity relationship — no such relationship field exists anywhere in the schema. This is the exact anti-pattern M5.4 was sent back for (hardcoded literal standing in for computed logic), landing here on the milestone's other explicitly-required focus area. The `b45` test only asserts the hardcoded name appears, proving nothing about generalization. Secondary non-blocking finding: `redactedFields`/`medicalNotes` are genuinely engine-persisted but never rendered. Everything else (guidedProcess step-gating, statusTimeline, paymentCheckout, documentLibrary, notificationInbox, discussionThread, exportWizard) verified genuinely engine-backed; all cited test/analyze commands re-ran clean and matched exactly (35/35, 5/5, 1/1, 128/128, 73/73 — the 128/128 demo suite must be run from the `app/` workspace root, not from inside the package directory, or one CWD-sensitive test produces a false failure unrelated to this milestone). See phase doc for full rejection note and required fix. Live emulator walk was not performed, per protocol. |
