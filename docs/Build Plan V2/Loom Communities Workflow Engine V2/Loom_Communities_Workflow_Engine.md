...
| 5 | 5.6 Mosque tab reimplementation | `[!]` **SENT BACK 2026-07-11** — code verification found the engine wiring genuine throughout (real `WorkflowDatabase`/`LocalWorkflowEngineApi`, real audience resolution via the M2.1 `SurfaceQuery(audienceMemberField: ...)` primitive, genuine `protectedDetail` masking computed from `assignedReviewerPersonaId == personaId`/ownership) but found one narrow defect: `assign-care-request`'s effect hardcodes `"value": "mosque-admin"` instead of using the engine's real `$actor`/`{actor}` dynamic-actor mechanism — the same one this exact fixture correctly uses for `sign-up-volunteer`. Currently behaviorally inert (guard-restricted to the sole `mosque-admin` persona) but a real hardcoded-value defect in the milestone's own scrutiny area. Fresh gates match submission exactly: analyze clean; `milestone_1_3_test.dart` 36/36; Mosque fixture validator pass/0/0; `app_shell` 5/5; engine 73/73; `b46` 1/1; combined demo suite 129/129. Also flagged: 4 untracked helper scripts left in the repo (commit-gate cleanup), and recurring `?`-for-non-ASCII mojibake in the resubmission note. See phase doc for full note — fix is a one-line change, should be a fast round-trip. Live emulator walk was not performed, per protocol. |
