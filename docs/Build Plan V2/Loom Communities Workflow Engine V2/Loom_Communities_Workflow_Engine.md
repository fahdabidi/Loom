...
| 5 | 5.6 Mosque tab reimplementation | `[x]` **CLOSED 2026-07-11** — resubmission independently verified: `git show 54884eb` confirmed the diff is exactly the one line described (`"mosque-admin"` → `"{actor}"`), nothing else touched; scratch scripts confirmed gone. Fresh gates match submission exactly: analyze clean; `milestone_1_3_test.dart` 36/36; Mosque fixture validator pass/0/0; `app_shell` 5/5; engine 73/73; `b46` 1/1; combined demo suite 129/129 (unaffected by the fix, as predicted). Live emulator walk (both personas) directly confirmed both generality claims: a `selected-many`-audience event published as admin was genuinely absent from the member's Calendar (only the `all`-audience event showed); the Care tab's `assign-care-request` flipped live from `Private details: Masked for this viewer` to unmasked with `Assigned reviewer: mosque-admin` via the real `{actor}` token, then `respond`→`close` reached `State: Resolved`. Also verified: member RSVP/donation/donor-visibility/volunteer-signup; admin `contact-volunteer`/announcement `preview`→`publish`; Messages' neutral notification correctly leaking zero care details; Search's `hide-source`. See phase doc for full close-out note. **Phase 5 is now fully complete** (all of 5.1–5.6 closed). |
