...
- Use a short, progressive onboarding sequence with a strong visual header, sticky primary action, and scrollable content instead of a dense setup form.
- Represent interest cold-start as grouped selectable chips because the task is scanning and selecting many lightweight topics across categories.
...
- Treat creator setup as a channel preview plus channel name, handle, description, category, then hosting acceptance. This mirrors common creator products without forcing publishing decisions into Phase 1.
- Keep the Phase 0 catalog visible in the Fan App default surface, but frame onboarding as a social setup card inside the feed rather than a plain button above a list.

## Key UX decisions

- The Fan App opens to a modern home surface: branded shell, setup prompt, horizontal topic rail, and the Phase 0 creator catalog. The direct `Start fan onboarding` action remains available and testable.
...
- Refreshed design-system components for onboarding layout, grouped interest chips, persona/visibility option cards, channel preview/setup fields, bottom navigation, compact role switching, and media-first content cards.
- Added bottom scroll padding to onboarding content so chips and controls do not slide under the sticky action bar on emulator-sized screens.
...
8. Emulator screenshots were captured for the refreshed Fan App home, fan onboarding, and Creator Studio onboarding surfaces during validation.
...
- The generated poster/card visual style is sufficient for Phase 0/1, but Phase 3 should decide whether seed media needs richer generated bitmap assets for discovery.
