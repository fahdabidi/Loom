...
The first build milestone is a **Demo App that demonstrates every MVP user story end-to-end on the Flutter Android emulator**, with physical-phone verification deferred to Phase 14 — both **fan-facing** stories and **creator-authoring** stories. It has two jobs:
...
- The headline loop (author → consume) and the six-step "wow" demo run end-to-end on the Flutter Android emulator, fully offline; the same loop is verified on a physical Android phone in Phase 14 only.
...
One Dart codebase for Android + iOS; builds APK in **WSL Ubuntu**; runs on the **Flutter Android emulator** for Phases 0–13, then on both the emulator and a **physical phone over adb** in Phase 14; statically typed and agent-friendly; hard module boundaries via a **melos** workspace; trivial **stub→real-HTTP** swap via DI. We validate on Android now; iOS later with no rewrite.

**WSL/emulator setup (Phase 0 checklist):** Flutter SDK + Android cmdline-tools/SDK in WSL; launch the Flutter Android emulator in WSL; install/run the debug APK on the emulator. Physical-phone adb setup (USB passthrough with `usbipd-win`, or wireless adb) is a Phase 14 checklist item.
...
| 14 | UX hardening & physical phone validation | both | **FINAL launch + phone** | Immersive discovery; richer media; async states; pagination; physical-phone full-demo validation |
...
| 9 — Export/portability + transparency + full demo | Complete                       | 2026-06-02      | `1db710d`  | [Phase 9 API Review][phase-9-api-review] | Green implementation gates: `melos bootstrap`, Drift generation, `melos run analyze`, `melos run lint:boundaries`, `melos run test` (28 tests), full `melos run test:integration` on `emulator-5554` (39/39), normal-app `flutter build apk --debug -t lib/main.dart`, emulator `adb install -r`, app launch, and screenshot `data/validation/phase9-manual-checkpoint.png`. Added [Phase 9 UX Decisions][phase-9-ux-decisions] and [Demo Runbook](./Phases/Demo%20Runbook.md). Physical Android phone validation and final launch-demo validation moved to Phase 14. |
| 10 — Launch contracts, store & fakes              | Not started                    | —               | —          | —                                        | Adds launch API contracts/models, local-store tables, fake backend behavior, seed data, reset/idempotency tests, and DI registrations for Phases 11-14. |
| 11 — Creator launch funnel                        | Not started                    | —               | —          | —                                        | Creator Launch/Grow UX: announcement templates, link-in-bio preview, QR/capture link, external account context, and cross-post stub. |
| 12 — Fan starter-pack onboarding                  | Not started                    | —               | —          | —                                        | Capture-link landing, one-tap starter pack, idempotent bulk follow, non-empty feed landing, and plural suggested creators. |
| 13 — Conversion analytics & creator utility consoles | Not started                 | —               | —          | —                                        | Aggregate conversion funnel plus catalog import, ad-policy, creator archive-AI preview, and membership setup consoles. |
| 14 — UX hardening & physical phone validation     | Not started                    | —               | —          | —                                        | Immersive discovery, richer generated media, loading/empty/error states, feed-style pagination, full launch-demo run, and physical Android phone validation. |
...
- **P14:** final UX hardening and physical-phone validation: immersive discovery, richer generated media, loading/empty/error states, feed-style pagination, final launch-demo runbook, emulator regression, and physical Android phone sign-off.
...
- App builds in WSL and installs/runs on the Flutter Android emulator for Phases 0–13, fully offline, with the role switcher. Phase 14 additionally installs/runs on a physical Android phone.
