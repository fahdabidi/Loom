...
The first build milestone is a **Demo App that demonstrates every MVP user story end-to-end on the Flutter Android emulator**, with physical-phone verification deferred to Phase 10 — both **fan-facing** stories and **creator-authoring** stories. It has two jobs:
...
- The headline loop (author → consume) and the six-step "wow" demo run end-to-end on the Flutter Android emulator, fully offline; the same loop is verified on a physical Android phone in Phase 10 only.
...
One Dart codebase for Android + iOS; builds APK in **WSL Ubuntu**; runs on the **Flutter Android emulator** for Phases 0–9, then on both the emulator and a **physical phone over adb** in Phase 10; statically typed and agent-friendly; hard module boundaries via a **melos** workspace; trivial **stub→real-HTTP** swap via DI. We validate on Android now; iOS later with no rewrite.

**WSL/emulator setup (Phase 0 checklist):** Flutter SDK + Android cmdline-tools/SDK in WSL; launch the Flutter Android emulator in WSL; install/run the debug APK on the emulator. Physical-phone adb setup (USB passthrough with `usbipd-win`, or wireless adb) is a Phase 10 checklist item.
...
| 10 | Launch audience re-acquisition & onboarding | both | **FINAL launch + phone** | Re-acquisition funnel; starter-pack onboarding; conversion analytics; physical-phone full-demo validation |
...
- **P10:** creator runs the launch re-acquisition funnel (CE-S10), fan joins through starter-pack onboarding (FE-S13), creator reviews conversion yield (CE-S11), stubbed creator utility consoles are completed, UX hardening closes modern-social gaps, and the full launch demo is validated on a physical Android phone.
...
- App builds in WSL and installs/runs on the Flutter Android emulator for Phases 0–9, fully offline, with the role switcher. Phase 10 additionally installs/runs on a physical Android phone.
