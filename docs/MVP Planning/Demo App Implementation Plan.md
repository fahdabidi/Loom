...
| 22 — Fan AI search settings & source connections     | Not started | —               | —          | [Phase 22 API Review][phase-22-api-review] | Planned: Fan Settings to connect AI search agent (Claude/OpenAI/Gemini via MCP, simulated), enable external sources, connect YouTube (simulated), prefer-creators default, query-egress disclosure. |
| 23 — AI search results (merge + title handling)      | Not started | —               | —          | [Phase 23 API Review][phase-23-api-review] | Planned: AI search merges creator (preferred) + external with agent ranking (no paid placement); compliant title handling (external title/thumbnail unaltered + additive accurate-match label + source chip). |
| 24 — Embedded player & AI-driven next                | Not started | —               | —          | [Phase 24 API Review][phase-24-api-review] | Planned: official in-app YouTube IFrame player (unobscured); "Next from your AI search" rail; non-YouTube external open; offline/error states. First phase needing network at play time. |
