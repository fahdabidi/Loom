...
| ------------------------------------------------- | --------------------- | --------------- | ---------- | ---------------------------------------- | ------------------------ |
| 0 — Foundation & scaffolding + role switcher      | Complete              | 2026-05-31      | pending    | [Phase 0 API Review][phase-0-api-review] | Green: `melos bootstrap`, `melos run analyze`, `melos run lint:boundaries`, `melos run test`, `flutter build apk --debug`, `melos run test:integration` on `emulator-5554`, and emulator `adb install -r`. `loom_local_store` uses Drift/SQLite for app runs and in-memory Drift for tests. [Phase 0 UX Decisions][phase-0-ux-decisions] filed. Physical-phone verification is deferred to Phase 9. |
