...
The first build milestone is a **Demo App that demonstrates every MVP user story end-to-end on the Flutter Android emulator**, with physical-phone verification deferred to Phase 20 — both **fan-facing** stories and **creator-authoring** stories. It has two jobs:
...
- The headline loop (author → consume) and the six-step "wow" demo run end-to-end on the Flutter Android emulator, fully offline; the same loop is verified on a physical Android phone in Phase 20 only.
...
| 14    | UX hardening & phone deferral                     | both    | **emulator UX**                                 | Immersive discovery; richer media; async states; pagination; Phase 20 physical-phone deferral |
...
| 20    | Customization showcase & final validation         | both    | FINAL phone + emulator                          | Showcase hardening; runbook/walkthrough updates; emulator regression; physical-phone gate |
...
| 14 — UX hardening & physical phone validation        | Complete    | 2026-06-02      | `e4cb221`  | [Phase 14 API Review][phase-14-api-review] | Green implementation gates: `melos bootstrap`, final post-format `melos run analyze`, `melos run lint:boundaries`, `melos run test` (36 tests), focused `flutter test test/widget_test.dart test/p14_ux_hardening_widget_test.dart`, and `flutter build apk --debug -t lib/main.dart`. Added [Phase 14 UX Decisions][phase-14-ux-decisions], Phase 14 integration tests, and launch-loop runbook updates. Blocked device evidence: WSL `adb devices` reports no attached Android device, `flutter devices` only reports Linux desktop, and `flutter test integration_test/it_p14_*.dart` fails with no supported device. Authoritative physical-phone validation is deferred to Phase 20. |
| 15 — Extensions platform & customization foundation  | Not started | —               | —          | [Phase 15 API Review][phase-15-api-review] | Planned: typed extension registry/runtime/creator-experience contracts, fake backend over Drift/SQLite local store, seeded gaming creators and certified extensions, reset/idempotency coverage, and extension foundation tests. |
| 16 — Config-driven channel renderer                  | Not started | —               | —          | [Phase 16 API Review][phase-16-api-review] | Planned: fan channel renders from `CreatorExperienceConfig`, per-creator theme/module ordering/extension slots, no per-creator UI hardcoding, and renderer-focused UX validation. |
| 17 — Extension modules I: competitive & economy      | Not started | —               | —          | [Phase 17 API Review][phase-17-api-review] | Planned: Clip Arena, Pick'Em, and HypeWars modules using extension runtime sessions/events plus wallet/reward receipt integration. |
| 18 — Extension modules II: collaborative & creative  | Not started | —               | —          | [Phase 18 API Review][phase-18-api-review] | Planned: Quest Log, Build Showcase, and Guild Quest modules using the same extension runtime contracts, collaborative state, moderation-safe placeholders, and creator-configurable module placement. |
| 19 — Creator Studio customize console                | Not started | —               | —          | [Phase 19 API Review][phase-19-api-review] | Planned: Creator Studio controls for theme/banner selection, extension install/configuration, module ordering, preview, and gaming starter-pack assembly. |
| 20 — Customization showcase & final validation       | Not started | —               | —          | [Phase 20 API Review][phase-20-api-review] | Planned: showcase polish across customized creator channels, async/empty/error state hardening, updated validation walkthrough/runbook, emulator regression, and final physical Android phone validation gate. |
...
- **P14:** UX hardening: immersive discovery, richer generated media, loading/empty/error states, feed-style pagination, final launch-demo runbook, emulator regression, and Phase 20 physical Android phone sign-off deferral.
...
- **P20:** customization showcase and final validation: harden customized fan experiences, update validation walkthrough/runbook, run emulator regression, and complete the physical Android phone validation gate.
...
- App builds in WSL and installs/runs on the Flutter Android emulator for Phases 0–20, fully offline, with the role switcher. Phase 20 additionally installs/runs on a physical Android phone.
