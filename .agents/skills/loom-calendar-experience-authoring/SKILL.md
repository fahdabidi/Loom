...
11. [`docs/references/reference/permissions.md`](../../../docs/references/reference/permissions.md) — the `action` field and the closed action vocabulary per bespoke archetype. **Declare `action` on every transition of a bespoke-archetype workflow (`event-rsvp`, `votePoll`, `equipment-loan`, `documentLibrary`, `searchAiAnswer`, `exportWizard`, `table`) using only that family's listed values, and never on a generic-archetype transition** — generic families derive their permissions structurally. Permissions themselves are always derived from what you author; never write a permission, a user, or a membership/join-approval workflow. Domain processes that accompany joining (a waiver, a registration fee, a reviewer decision) remain legitimate workflows; the membership grant itself does not.
